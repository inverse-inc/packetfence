...
my $app = sub {
    ...
    my $sms_module = captiveportal::DynamicRouting::AuthModule::SMS->new(id => 'sms', source => pf::authentication::getAuthenticationSource('sms'), custom_fields => [], app => $application, parent => $or_module);
    $or_module->add_module($sms_module);
    $root_module->add_module($or_module);
    $application->add_module($root_module);
