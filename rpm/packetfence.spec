...
if /usr/local/pf/containers/manage-images.sh; then
    ...
    /usr/bin/mysql -uroot --silent --batch -e "REVOKE ALL PRIVILEGES ON \`test\`.* FROM 'PUBLIC';" 2>&1 > /dev/null || true
    /usr/bin/mysql -uroot --silent --batch -e "REVOKE ALL PRIVILEGES ON \`test_%\`.* FROM 'PUBLIC';" 2>&1 > /dev/null || true
