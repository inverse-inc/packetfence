...
if /usr/local/pf/containers/manage-images.sh; then
    ...
    /usr/bin/mysql -uroot -e "REVOKE ALL PRIVILEGES ON \`test\`.* FROM \"PUBLIC\";" &> /dev/null || true
    /usr/bin/mysql -uroot -e "REVOKE ALL PRIVILEGES ON \`test_%\`.* FROM \"PUBLIC\";" &> /dev/null || true
