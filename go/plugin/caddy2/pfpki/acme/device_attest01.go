...
//  5. Confirm the nonce equals SHA-256(challenge.token). Apple's
//     client (and step-ca, the reference it interoperates with) bind
//     the attestation to the bare token, not to the RFC 8555 §8.1
//     key authorization.
//  6. Confirm the order's permanent-identifier value matches the
//     device's UDID (preferred) or Serial.
//
// On success returns the DER SubjectPublicKeyInfo of the attested key
// (the leaf's public key), which finalize compares against the CSR so
// the certificate is bound to the hardware key that was attested. On
// any failure returns a Go error whose string can go straight into an
// ACME problem document.
func deviceAttest01Validate(prof models.Profile, attestation []byte, challengeToken, expectedIdentifier string) ([]byte, error) {
	if len(attestation) == 0 {
		return nil, errors.New("device-attest-01: empty attestation payload")
	}

	var obj appleAttestationObject
	if err := cbor.Unmarshal(attestation, &obj); err != nil {
		return nil, fmt.Errorf("device-attest-01: CBOR decode: %w", err)
	}
	if obj.Format != "apple" {
		return nil, fmt.Errorf("device-attest-01: unsupported fmt %q (only apple)", obj.Format)
	}
	if len(obj.AttStatement.X5C) == 0 {
		return nil, errors.New("device-attest-01: attStmt.x5c is empty")
	}

	leaf, err := x509.ParseCertificate(obj.AttStatement.X5C[0])
	if err != nil {
		return nil, fmt.Errorf("device-attest-01: parse leaf: %w", err)
	}
	intermediates := x509.NewCertPool()
	for _, der := range obj.AttStatement.X5C[1:] {
		c, err := x509.ParseCertificate(der)
		if err != nil {
			return nil, fmt.Errorf("device-attest-01: parse intermediate: %w", err)
		}
	...
	if _, err := leaf.Verify(x509.VerifyOptions{
		...
		return nil, fmt.Errorf("device-attest-01: verify chain: %w", err)
	}
	...
	// Nonce binding: the extension MUST equal SHA-256(token). Apple
	// sometimes adds a wrapping ASN.1 layer; extractAppleDevice already
	// unwraps it before returning the raw 32-byte digest.
	expectedNonce := sha256.Sum256([]byte(challengeToken))
	if subtle.ConstantTimeCompare(dev.nonce, expectedNonce[:]) != 1 {
		return nil, errors.New("device-attest-01: nonce does not match SHA-256(token)")
	}
	...
	if want == "" {
		return nil, errors.New("device-attest-01: order has empty permanent-identifier")
	}
	if dev.UDID != want && dev.Serial != want {
		return nil, fmt.Errorf("device-attest-01: identifier mismatch: order=%q udid=%q serial=%q",
			want, dev.UDID, dev.Serial)
	}
	spki, err := x509.MarshalPKIXPublicKey(leaf.PublicKey)
	if err != nil {
		return nil, fmt.Errorf("device-attest-01: attested public key: %w", err)
	}
	return spki, nil
}
