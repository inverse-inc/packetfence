...
func (p Path) Matches(base string, caseSensitivePath bool) bool {
	if base == "/" || base == "" {
		return true
	}
...
func (p PathMatcher) Match(r *http.Request, caseSensitivePath bool) bool {
	return Path(r.URL.Path).Matches(string(p), caseSensitivePath)
}
