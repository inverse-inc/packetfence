...
func TestAuth(t *testing.T) {
	...
	teardown := simpleSetup(t,
		...
		&chclient.Config{
			Remotes: []string{
				...
				"0.0.0.0:" + tmpPort2 + ":containers-gateway.internal:$FILEPORT",
			},
			Auth: "foo:bar",
		})
	defer teardown()
	//test first remote
	result, err := post("http://containers-gateway.internal:"+tmpPort1, "foo")
	if err != nil {
		t.Fatal(err)
	}
	if result != "foo!" {
	...
	//test second remote
	result, err = post("http://containers-gateway.internal:"+tmpPort2, "bar")
	if err != nil {
		t.Fatal(err)
	}
	if result != "bar!" {
