...
func (j *Ip4logCleanup) DoRotate(ctx context.Context) {
	...
	for {
		...
		sql := `
            INSERT INTO ip4log_archive (mac, ip, start_time, end_time)
              SELECT mac, ip, start_time, end_time FROM ip4log_history
		...
		if rows_deleted != rows_inserted {
			log.LogWarn(ctx, fmt.Sprintf("When rotating ip4log the number of rows deleted (%d) does not match the number of row inserted (%d)", rows_deleted, rows_inserted))
		}
