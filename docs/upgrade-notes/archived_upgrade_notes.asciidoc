...
[options="compact"]
* moved remediation templates in `html/captive-portal/templates/violations` and
  converted them to Template Toolkit
...
* splitted the httpd services in three: httpd.admin, httpd.portal and
  httpd.webservices
...
modified. Simply update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number.
...
installed, update the file [filename]`/usr/local/pf/conf/currently-at` to match
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new
...
If you have defined any filters in [filename]`/usr/local/pf/conf/vlan_filters.conf`,
ensure to rename all references to the left hand side with the new names on the
right hand side.
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 8.0).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 8.1).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 8.2).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 8.3).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 9.0).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 9.1).
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 9.2):
...
Once completed, update the file [filename]`/usr/local/pf/conf/currently-at` to
match the new release number (PacketFence 9.3):
