...
Introduction
------------
This guide will provide an example for the configuration of an Open SSID (not crypted) and a Secure SSID (802.1x). You will to install wpad and hostapd. This two SSIDs will do radius authentication against PacketFence.

Assumptions
-----------
* You have a configured PacketFence environment with working test equipment
* Where the management IP of PacketFence will be 192.168.1.10 and has s3cr3t as his radius shared secret
...
First update the repos by clicking the button Update lists.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Create the file : /etc/config/hostapd.vlan
----
*           wlan0.#
----
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You will need to modify the hostapd script that come with the package that we previously installed.

Connect with SSH to the AP and run these command:
----
cd /lib/netifd/
mv hostapd.sh hostapd.sh.old
wget https://github.com/inverse-inc/packetfence/tree/devel/addons/hostapd/hostapd-14.07.sh
mv hotsapd-14.07.sh hostapd.sh
wifi
----
...
In order to apply this configuration, when you are connect over ssh on the AP, run the command 'wifi'. It will reload the configuration and broadcast the SSID.

NOTE:  It's known that you can't put 2 SSIDs with the same dae server at the same time. The deauthentication will not work on the 2nd SSID.
...
Here few things you can do/check to see if you configuration is consistant.
