...
To configure the access to the billing portal after the registration :
[options="compact"]
* Go in the 'Captive Portal' tab
* Change 'Redirection URL' to https://PORTAL_HOSTNAME/status
* Check the box 'Force redirection URL' and then save the profile
...
After the registration process you will be redirected to https://PORTAL_HOSTNAME/status where you can elevate your user access and manage your devices
...
NOTE: The upgrade page is based on the billing tiers that have been configured in the section 'Configure PacketFence -> Portal Profile'. You can create as much tiers as you want and add them in the portal profile. Each one should assign a different role that you will map to a traffic shaping profile in the section 'Configure traffic shaping -> Configuring another role'
