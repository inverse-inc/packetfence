...
In `Configuration -> Connection Profiles -> Add profile -> Advanced filter` specify an advanced filter like that:
...
This configuration is really simple, you just have to enable and fill the URL, the username and password in `Configuration -> Node MSE Tab`.
So in our example:
