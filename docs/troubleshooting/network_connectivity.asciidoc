...
5. For VLAN enforcement issues, check switch configuration and trunk port
   settings

6. Verify firewall rules allow required traffic between PacketFence and
   network devices

==== Advanced Network Issues

1. **Large Registration Network Issues**: In large environments, check for ARP
   table overflow symptoms:
...
2. **VLAN Segmentation**: Ensure VLAN reaches from client to DHCP
   infrastructure to PacketFence server
