Ruckus Quick Installation Guide
===============================
:encoding: UTF-8
:lang: en
:doctype: book
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Now that you have a fully functional PacketFence installation, you'll need to add the Ruckus controller to the PacketFence switches database for correct integration.
...
On the left menu, click on WLAN. Click the Create New button, and add the following information. If you do not see the line it means that you should leave the setting to default value:
   ...
   Description: Secure SSID for PoC
   Type: Standard Usage
