= Developer's Guide
////

    This file is part of the PacketFence project.

    See includes/global-attributes.asciidoc
    for authors, copyright and license information.

////
include::includes/global-attributes.asciidoc[]

== About this Guide

This guide helps modify PacketFence for specific needs and covers adding support for new switches.

Find the latest version at https://packetfence.org/documentation/.

=== Other sources of information

<<PacketFence_Clustering_Guide.asciidoc#,Clustering Guide>>::
  Covers installation in a clustered environment.
<<PacketFence_Installation_Guide.asciidoc#,Installation Guide>>::
  Covers installation and configuration of PacketFence.
<<PacketFence_Network_Devices_Configuration_Guide.asciidoc#,Network Devices Configuration Guide>>::
  Covers switches, WiFi controllers and access points configuration.
<<PacketFence_Upgrade_Guide.asciidoc#,Upgrade Guide>>::
  Covers compatibility changes, manual instructions and upgrade notes.
https://packetfence.org/news.html[PacketFence News]::
  Covers noteworthy features, improvements and bug fixes by release.

Package and release tarballs include these files.

== Creating a new Switch via a Template

include::developer/template_switches.asciidoc[]

== Documentation

include::developer/documentation_info.asciidoc[]

== Development environment setup

include::developer/dev_env_setup.asciidoc[]

== Administration GUI development setup

include::developer/admin_gui.asciidoc[]

== Golang environment

include::developer/go/README.asciidoc[]

== Code conventions

include::developer/code_conventions.asciidoc[]

== HTTP JSON API

include::developer/http_json_api.asciidoc[]

== Customizing PacketFence

include::developer/customizing_packetfence.asciidoc[]

== SNMP

include::developer/snmp.asciidoc[]

== Supporting new network hardware

include::developer/supporting_new_network_hardware.asciidoc[]

== PacketFence builds

include::developer/packer/README.asciidoc[]

== Developer recipes

include::developer/developer_recipes.asciidoc[]

== Containers recipes

include::developer/containers_recipes.asciidoc[]

== Running tests

include::developer/tests/README.asciidoc[]

include::developer/integration_tests/README.asciidoc[]

== Contributing

include::developer/contributing.asciidoc[]

//TODO: process the rest. I found quickest way to be:
// paste from docbook then use vim(:1,9s#<\_.\{-1,}>##g) and reformat by hand

// INCLUDES

include::includes/additional-info.asciidoc[]

include::includes/commercial-support.asciidoc[]

include::includes/license.asciidoc[]

[appendix]
== Quick Reference

=== Terms and Technologies

API::
  <<#_http_json_api,HTTP JSON API>>

Authentication::
  <<#_customizing_packetfence,Customizing PacketFence>>

CLI::
  <<#_code_conventions,Code Conventions>>

Code::
  <<#_code_conventions,Code Conventions>>, <<#_customizing_packetfence,Customizing PacketFence>>

Containers::
  <<#_containers_recipes,Containers Recipes>>

Development::
  <<#_development_environment_setup,Development Environment Setup>>

Documentation::
  <<#_documentation,Documentation>>

Environment::
  <<#_development_environment_setup,Development Environment Setup>>, <<#_golang_environment,Golang Environment>>

Git::
  <<#_contributing,Contributing>>

Golang::
  <<#_golang_environment,Golang Environment>>

GUI::
  <<#_administration_gui_development_setup,Administration GUI Development Setup>>

Hardware::
  <<#_supporting_new_network_hardware,Supporting New Network Hardware>>

HTTP::
  <<#_http_json_api,HTTP JSON API>>

Integration::
  <<#_running_tests,Running Tests>>

JSON::
  <<#_http_json_api,HTTP JSON API>>

Network::
  <<#_supporting_new_network_hardware,Supporting New Network Hardware>>

Perl::
  <<#_code_conventions,Code Conventions>>, <<#_development_environment_setup,Development Environment Setup>>

REST::
  <<#_http_json_api,HTTP JSON API>>

SNMP::
  <<#_snmp,SNMP>>, <<#_supporting_new_network_hardware,Supporting New Network Hardware>>

Switch::
  <<#_creating_a_new_switch_via_a_template,Creating a New Switch via a Template>>, <<#_supporting_new_network_hardware,Supporting New Network Hardware>>

Template::
  <<#_creating_a_new_switch_via_a_template,Creating a New Switch via a Template>>

Testing::
  <<#_running_tests,Running Tests>>

// vim: set syntax=asciidoc tabstop=2 shiftwidth=2 expandtab:
