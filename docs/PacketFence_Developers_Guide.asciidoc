...
* `/usr/local/pf/html/captive-portal/content/Gruntfile.js` -- the task runner configuration file
* `/usr/local/pf/html/captive-portal/content/package.js` -- the npm dependencies required to process the Sass files
* `/usr/local/pf/html/captive-portal/content/scss/` -- the directory containing the Sass source files
* `/usr/local/pf/html/captive-portal/content/styles.css` -- the generated stylesheet
...
In order to prepare your environment to process the Sass files, you need to install http://gruntjs.com/[grunt] (the task runner) and additional https://nodejs.org[nodejs] modules, including the various inuitcss components:
  ...
  cd /usr/local/pf/html/captive-portal/content
  npm install
