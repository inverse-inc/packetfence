...
<<dev-setup,initial setup>> but in place of starting `pfcendev`, you can
start:

* `pfcennight`
* `pfdeb9night`

These machines will be auto-configured to install latest builds from development
 repositories.
...
----
cd packetfence/addons/vagrant
vagrant provision --provision-with=config pfcennight
