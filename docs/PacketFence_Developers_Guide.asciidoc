...
Then use this token to query the API. The following example fetches the domains configuration:

 curl -X GET "https://PF_MANAGEMENT_IP:9999/api/v1/config/domains" \
 -H  "accept: application/json" \
...
. Install PacketFence packages from devel (see next section) cleanly and do an initial configuration
. Stop all services:
+
[source,bash]
----
/usr/local/pf/bin/pfcmd service pf stop
systemctl stop packetfence-config packetfence-mariadb packetfence-redis-cache
----
+
...
----
# Debian
apt install -y build-essential libipset-dev pkg-config
...
cd /usr/local/
git clone https://github.com/inverse-inc/packetfence.git pf
cd pf
// mv conf conf-git-pristine
// mv raddb config-raddb-pristine
// ln -s /usr/local/pf-pkg/conf conf
// ln -s /usr/local/pf-pkg/raddb raddb
// chown pf:pf var/control
make devel
mkdir /usr/local/pf/var/ssl_mutex
----
+

. Install (at least) link:https://github.com/nodesource/distributions/#table-of-contents[NodeJS] 8 to build JavaScript files for webadmin
. Compile JavaScript files for web admin ([command]`sudo` is important to avoid permissions issues):
+
[source,bash]
----
...
. Compile JavaScript files for captive portal ([command]`sudo` is important to avoid permissions issues):
+
[source,bash]
----
cd /usr/local/pf/html/common
...
. Fix permissions and start `packetfence-config` and `packetfence-redis-cache` service:
+
[source,bash]
----
cd /usr/local/pf
...
. Install Golang environment and build Golang binaries
+
[source,bash]
----
cd /usr/local/pf/go
...
WARNING: Care should be taken not to commit local configuration files changes and files not in the
repository.


===== Day to day usage

. Update your git copy:
+
[source,bash]
----
git pull
...
. Recompile JavaScript files (see # section)
