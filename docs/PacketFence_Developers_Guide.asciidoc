...
We use Asciidoctor tags for section titles (headings) in place of AsciiDoc
tags, see table below for equivalence:
...
* link:https://www.vagrantup.com/docs/installation/[Vagrant] (>= 1.9.3) provided by Hashicorp
* Virtualbox if you wan to use Virtualbox as a provider for Vagrant (only `pf*` and `ad` machines)
...
include::integration_tests/README.asciidoc[]
