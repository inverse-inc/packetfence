...
[source,bash]
----
vagrant up pfcen7dev
----

.Reboot virtual machine to disable SELinux:
[source,bash]
----
vagrant reload pfcen7dev
----
...
Web admin should be reachable through management IP address of `pfcen7dev`
machine, configured in [filename]`addons/vagrant/inventory/hosts`. Default
passwords are all set to `secret`.
...
<<_virtual_environment_initial_setup,initial setup>> but in place of starting `pfcen7dev`, you can
start:
...
If you want install latest packages built in the pipeline, available on
...
----
cd packetfence/addons/vagrant
CI_PROJECT_NAME="ppa" CI_PAGES_URL="http://inverse-inc.gitlab.io/packetfence" vagrant up pfdeb9night
----
...
----
cd packetfence/addons/vagrant
vagrant provision --provision-with=config pfcen7night
vagrant provision --provision-with=config pfdeb9night
----
...
----
cd packetfence/addons/vagrant
ansible-playbook site.yml --limit pfcen7night
----
...
EOF
ansible-playbook site.yml --limit pfcen7night -e @extra.yml
----
