...
The full API spec documentation for :
...
You should use 9999 TCP port and a secure connection (HTTPS) to ask it.
...
NOTE: You can use API spec documentation mentionned above to have ready to use curl commands with "Try it out" button.

First, get an authentication token with webservices credentials or an admin account:
...
Then use this token to ask API (here get domains config) :
