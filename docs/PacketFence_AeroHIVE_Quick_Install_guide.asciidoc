...
About this Guide
----------------
This guide has been created in order to help sales engineers, product managers, or network specialists demonstrate the PacketFence capabilities on-site with an existing or potential customer.  It can also provide guidelines to setup a proof of concept for a potential PacketFence deployment using AeroHIVE products.

The instructions are based on version {release_version} of the PacketFence Live Image. Also based on version 6+ of the HiveManager.

Assumptions
-----------
* You will need a working PacketFence setup using VLAN enforcement or Role enforcement;
* You have either physical or virtual infrastructure to deploy the AeorHIVE Access Point and Controller;

Quick Deployment
----------------
Step 1: PacketFence Side
~~~~~~~~~~~~~~~~~~~~~~~~
