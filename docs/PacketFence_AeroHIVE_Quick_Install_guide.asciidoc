...
For our customer scenario, you will choose VLAN enforcement.
...
Confirgure the switch like the following:
...
^^^^^^^^^^^^^^^^^^^^^^^^^
By default, the HiveManager might be empty, add your AP on it via it's serial number. If necessra update the firmware of the HiveManager and the AP.
...
Setup the SSIDs
^^^^^^^^^^^^^^^
For this guide, you will show you how to configure one open SSID with RADIUS mac filtering, and one secure SSID using WPA2-Enterprise.
...
Click OK to save the server. You should see the RADIUS server in the list.

Configure the Open SSID
+++++++++++++++++++++++
On the left menu, click on SSIDs. Click the New button, and configure the SSID while following those information.
...
+++++++++++++++++++++++++
On the left menu, click on SSIDs. Click the New button, and configure the SSID while following those inforamtion.
...
Configure SNMP
^^^^^^^^^^^^^^
You now need to configure the SNMP to add a valid read community string. Click on the Configure pane, and on Advanced Configuration, then Management Policies, then SNMP Assignements. Now click on New and follow this configuration.
...
To be able to use the "Roles by switch role" you will need to setup a specific RADIUS attribute. To do so in the Configuration pane got to Networks, click on the network you will use, that will bring you to the "Guided configuration" now choose your SSID and click 'Add/Remove' under User Profile configure now this section as follow:
