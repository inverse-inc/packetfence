...
Configure SNMP
^^^^^^^^^^^^^^
Last step, we need to configure the SNMP to add a valid read community string. Click on the Configure pane, and on Advanced Configuration, then Management Policies, then SNMP Assignements. Now click on New and follow this configuration.
