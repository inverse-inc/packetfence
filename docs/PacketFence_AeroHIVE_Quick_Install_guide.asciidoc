...
^^^^^^^^^^^^^^^^^^^^^^^^^^
First thing to do is to connect the HiveManager. Like we saw in Step 2, make sure to connect the AP on the TRUNK port configured for it.
...
+++++++++++++++++++++++++
On the left menu, click on Advanced Configuration, then Authentication, then AAA Client Settings. We need to add the PacketFence RADIUS server here. Click on the New button, and provide those information:
...
+++++++++++++++++++++++
On the left menu, click on SSIDs. Click the New button, and add the following information. If you do not see the line, it means that you should leave the setting to default value:

   Profile Name/ESSID: DemoOpen
...
On the left menu, click on SSIDs. Click the New button, and add the following information. If you do not see the line it means that you should leave the setting to default value:
   ...
   Profile Name/SSID: DemoSecure
   Decription: Secure SSID for PoC
   Secure: WPA/WPA2 802.1X (Enterprise)
   Key Management: WPA2-(WPA2 Entreprise)-802.1X
   Encryption Options: CCMP(AES)
