...
About this Guide
----------------
This guide has been created in order to provide guidelines to deploy a Aerohive Wireless APs and the associated management in a PacketFence context, whether it is with a standalone server, a cluster or PacketFenc ZEN deployxement.

Aerohive products are a bit different compared to the other vendors.
They support either a local HiveManager (kind of wireless controller) or a cloud-based HVM.
However, the configuration is the same for the local and the cloud-based controller.
Note that all the config are made on the HVM and then pushed to the APs.
...
In this part, iit is recommended that the default VLAN must be the native VLAN.
...
References and Externals Informations
-------------------------------
