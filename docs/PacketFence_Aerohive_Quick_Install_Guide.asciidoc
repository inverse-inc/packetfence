...
+++++++++++++++++++++++++
On the left menu, click on 'Network Configuration', then 'Authentication', then 'AAA Client Settings'. We need to add the PacketFence RADIUS server here. Click on the 'New' button, and provide follow this configuration.
...
Configure SNMP
^^^^^^^^^^^^^^
You now need to configure the SNMP to add a valid read community string. Click on the 'Configure' pane, and on 'Network Configuration', then 'Management Policies', then 'SNMP Assignments'. Now click on 'New' and follow this configuration.
...
On the left menu, click on 'Network Configuration', then 'Authentication', then 'Captive Web Portals'. Click on 'New' to create a Captive Portal, configure it as follow:
