...
Since version 4.5.1, PacketFence polls the OPSWAT GEARS API in order to trigger violations on non compliant devices.
...
You now have to configure the violation that is raised when a device is not compliant. Using the violation 'Generic' should fit your needs for now, and you can modify the template after.
...
You can then hit 'Save' and you now the device will get isolated whenever it's found not compliant.
