...
It is possible to trigger violations based on threatness level of downloaded files. This is possible with OPSWAT Metascan integration in conjunction with Suricata MD5 extraction feature. Without entering in the details, here are the basic steps to make it work.
...
Configure `/etc/syslog-ng/syslog-ng.conf` by adding the following to enable sending MD5 file store log entries to PacketFence:
  ...
  # This line specifies where the files-json.log file is located (make sure the path fits your installation)
  ...
  # This line indicates syslog-ng to use the s_suricata_files source with the d_packetfence destination
...
At this point, Suricata should be able to extract MD5 checksum of downloaded file and send the related log entry to PacketFence.

A configuration of a new pfdetect parser as well as some violations are the only remaining steps to make full usage of the OPSWAT Metascan integration.
