...
On a Ubuntu 12.04 LTS system, do:
...
All the PacketFence dependencies are available through the official Ubuntu repository. 
...
For Ubuntu 12.04 LTS, PacketFence provides a Ubuntu package repository.
...
Like mentioned at the end of the packages installation, fire up a web browser and go to http://@ip_of_packetfence:1443/configurator. From there, the configuration process is splited in six (6) distinctive steps, after which you'll have a working PacketFence setup.
...
In this directory you have 3 importants files, httpd.admin, httpd.portal, httpd.webservice.
httpd.admin is use to manage PacketFence admin interface.
httpd.portal is use to manage PacketFence captive portal interface.
httpd.webservices is use to manage PacketFence webservices interface.

These files have been written in perl language and are completly dynamic, so they activate services only on the interfaces provided for this purpose.

The other files in this directory are managed by PacketFence but with template, so it is easy to modify this files based on your configuration. SSL is enabled by default to secure access.

Upon PacketFence installation, self signed certificates will be create in `/usr/local/pf/conf/ssl` (`server.key` and `server.crt`). Those certificates can be replaced anytime by your 3rd-party or existing wildcard certificate without problems. Please note that the CN (Common Name) needs to be the same as the one defined in the PacketFence configuration file (`pf.conf`).
...
Under Ubuntu 12.04 LTS and RHEL6/CentOS 6, inline enforcement uses ipset to mark nodes as registered, unregistered and isolated.
...
Make sure to set the required configuration parameters on top of the file. Set the username/password to the account created under *Configuration -> Web Services*:
...
When done with the samba install, you need to modify `/etc/krb5.conf`. Here is an example for the `DOMAIN.NET` domain for Centos/RHEL:
...
Note that for Ubuntu 12.04 LTS you will probably have this error:
...
* through the Web GUI, in the *Configuration -> Network -> Floating devices* tab
...
These parameters can also be configured from the *Configuration -> Self Registration* section.

Available registration modes are defined on a per-portal-profile basis. These are configurable from *Configuration -> Portal Profiles and Pages*. To disable the self-registration feature, simply remove all modes from the portal profile definition. 
...
PacketFence administrators automatically have access to the guest management interface. It is also possible to create users that will only have access to this separate interface by setting the access level to *All* as an action.
...
Then make sure to set the required configuration parameters on top of the `/usr/local/pf/raddb/packetfence-soh.pm` file. Set the username/password to the account created under *Configuration -> Web Services*:
