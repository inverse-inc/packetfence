...
Packetfence can also be configured as hybrid, if you have a manageable device that support 802.1x and/or mac-auth you can enable inline mode by a radius attribute (mac address, ssid, port)
...
Introduced in PacketFence 3.6, hybrid enforcement is mix method that permit to use inline mode with vlan enforcement on the same device.
...
Before the version 3.6 of PacketFence, it was not possible to have radius enabled for inline mode. Now with the new hybrid mode, all the devices that support 802.1x or mac-auth can work with this mode. Let see how it works.

Device configuration
~~~~~~~~~~~~~~~~~~~~

You have to configure PacketFence as inline and your switch or access point to use the VLAN assignement techniques (802.1x, mac-auth), but now in the switch configuration, a new configuration parameter appear "Trigger to enable inline mode". This parameter is working like a trigger and you have the possibility to
...
where ALWAYS mean that the device is always in inline mode, PORT specify the ifIndex of the port, MAC an mac address, SSID a ssid name.
An example:

  SSID::GuestAccess,MAC::00:11:22:33:44:55

This will trigger all the nodes that connect on the SSID GuestAccess as Inline (PacketFence will return an empty vlan or the inlineVlan) and the mac address 00:11:22:33:44:55 if it connect on another SSID.
