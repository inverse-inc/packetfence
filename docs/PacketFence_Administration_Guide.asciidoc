...
From the PacketFence's web admin interface, you must configure an Admin Access role that contains the action SWITCH_LOGIN_READ or SWITCH_LOGIN_WRITE and assign this role to an internal user or in an Administration rule in an internal source.
