...
This will trigger all the nodes that connects to the "GuestAccess" SSID to use inline enforcement mode (PacketFence will return an empty vlan or the inlineVlan if defined in switch configuration) and the mac address 00:11:22:33:44:55 client if it connects on another SSID.
