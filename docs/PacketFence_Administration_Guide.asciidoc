...
When using subscription based billing, bla bla bla

This means that you should configure it so the user is be granted access to the network for an infinite value (in the billing tier you configure). 

Then Stripe will contact your PacketFence server when the subscription is canceled.
