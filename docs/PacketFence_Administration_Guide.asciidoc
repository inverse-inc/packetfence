...
image::docs/images/portal-modules/chianed_oauth_sms.png[scaledwidth="100%",alt="Chained OAuth + SMS module"]
...
NOTE: To benefit the OPSWAT Metascan integration, Suricata needs to be built with libnss / libnspr support. Make sure to use JSON output. More information on how to achieve this can be found there: https://redmine.openinfosecfoundation.org/projects/suricata/wiki/MD5
...
It is possible to trigger violations based on threat level of downloaded files using the OPSWAT Metascan integration in conjunction with the Suricata MD5 extraction feature. Without entering in the details, here are the basic steps to make it work.
...
Along with the OPSWAT API key for Metascan Online (they call it 'License Key') and the working Suricata installation, some configuration (PacketFence based AND Suricata based) are also required.
...
Configure `/etc/syslog-ng/syslog-ng.conf` by adding the following to enable sending MD5 file store log entries to PacketFence:
  ...
  ### PacketFence / OPSWAT Metascan integration
...
A configuration of a new 'syslog parser' as well as some violations are the only remaining steps to make full usage of the OPSWAT Metascan integration.
...
Configuration of a new violation can use the following trigger types:
  ...
  Type: metascan
  Triggers ID: The scan result returned by OPSWAT Metascan online
