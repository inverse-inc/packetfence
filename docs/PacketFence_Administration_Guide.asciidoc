...
* Give a temp shell to the `radiusd` user in `/etc/passwd`, and execute `raddebug` as `radiusd`
* Run `radiusd` as root (but please don't!)
* Add the `radiusd` user to the root group (but please don't!)
