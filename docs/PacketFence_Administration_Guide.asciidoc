...
In order to configure this, you will need to install additionnal packets available from the packetfence-extra repository. These packets will allow PacketFence to query your Active Directory for the NT hash of the users.

```
yum install python2-impacket --enablerepo=packetfence-extra
```

Once that is done, you need to configure PacketFence to start caching the credentials
