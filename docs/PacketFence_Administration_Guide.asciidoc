...
RADIUS support (RADIUS Enforcement)::
  PacketFence can be used as a RADIUS server without having registration, isolation, portal features in the case only RADIUS authentication is what matters. This can be particularly useful if you only want to do MAC authentication or 802.1X with your current VLAN or ACL.
