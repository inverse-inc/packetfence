...
Here is the configuration of the syslog regex parser:
    ...
    Detector *: StreamScan
    Enabled: checked
    Alert pipe: /usr/local/pf/var/cds (To create the fifo file, do: mkfifo /usr/local/pf/var/cds)
...
You will need to create a rsyslog configuration to forward all the syslog message sent by StreamScan to the pipe file /usr/local/pf/var/cds

First you need to activate the syslog under the rsyslog configuration file located at /etc/rsyslog.conf, uncomment the two parameters:
