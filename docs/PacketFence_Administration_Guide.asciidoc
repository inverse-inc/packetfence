...
NOTE: If you want to use other RADIUS attributes in your authentication source, add them in 'Configuration->Advanced->Custom RADIUS attributes'. They will then be available in the rules you define.
...
Configure `/etc/syslog-ng/syslog-ng.conf` by adding the followings which will enable sending sguild log entries to PacketFence:
  ...
  # This line tells syslog-ng to send the data read to the IP address of the PacketFence management interface, via UDP to port 514
  destination d_sguil_udp { udp("PACKENTFENCE_MGMT_IP" port(514)); };
...
Configure `/etc/rsyslog.d/security_onion.conf` by adding the followings which will enable reception of Security Onion sguild log entries:

  :programname, isequal, "sguil_alert" |/usr/local/pf/var/security_onion_alerts
  & ~

Make sure the receiving alert pipe (FIFO) exists

  mkfifo /usr/local/pf/var/security_onion_alerts
