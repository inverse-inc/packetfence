...
NOTE: If you want to use other LDAP attributes in your authentication source, add them in 'g->Advanced->Custom LDAP attributes'. They will then be available in the rules you define.
...
To do so, go in 'Configuration -> Advanced Configuration -> Networks -> Fencing', then check 'Passthrough' and add the Identity Provider domain name to the 'Passhtroughs' list.
...
* IP under *Configuration -> Advanced Portal Configuration -> Captive portal*
...
Next, go in the Administration interface under *Configuration -> Policies and Access Control -> Domains -> Microsoft Active Directory Domains*.
...
First configure your domains in *Configuration -> Policies and Access Control -> Domains -> Microsoft Active Directory Domains*.
...
First, create a custom root module for our examples in order to not affect the default policy. In order to do so, go in 'Configuration -> Advanced Portal Configuration -> Advanced Portal Configuration -> Portal Modules', then click 'Add Portal Module' and select the type 'Root'. Give it the identifier `my_first_root_module` and the description `My first root module`, then hit save.
...
Go in 'Configuration -> Advanced Portal Configuration -> Portal Modules' and click the `default_guest_policy`. Add `firstname`, `lastname` and `address` to the 'Mandatory fields' and save.
...
Go in 'Configuration -> Advanced Portal Configuration -> Portal Modules' then click 'Add Portal Module' and select 'Authentication -> SMS'.
...
Next, create a 'Provisioning' portal module by going in 'Configuration -> Advanced Portal Configuration -> Portal Modules'. Set the 'Identifier' to `secure_boarding` and the description to `Board Secure SSID`. Also uncheck 'Skippable' so the user is forced to board the SSID should it choose this option.
...
Go in 'Configuration -> Advanced Portal Configuration -> Portal Modules', then click 'Add Portal Module' and select 'Message'. Set the 'Identifier' to `hello_world` and the description to `Hello World`.
...
Go in 'Configuration -> Advanced Portal Configuration -> Portal Modules', then click 'Add Portal Module' and select 'URL'. Set the 'Identifier' to `token_system`, the 'Description' to `Token system` and the 'URL' to `http://YOUR_PORTAL_HOSTNAME:10000/token.php`.
...
In order to do that, in the administration interface, go in 'Configuration -> Advanced Portal Configuration -> Provisioners'. Then select the 'android' provisioner. Enter the SSID and save.

Now do the same thing for the iOS provisioner.

After, you simply need to add the 'Android' and 'iOS' provisioners to your 'Connection Profile' configuration.
...
In the PacketFence administration interface, go in 'g->Advanced Portal Configuration->Billing tiers'
...
* through the Web GUI, in *Configuration -> Advanced Configuration -> Floating Device*
...
In order to use the passthroughs feature in PacketFence, you need to enable it from the GUI in *g->Advanced Configuration->Networks->Fencing*, enable *Passthrough* and then save.
...
Then add passthroughs in *g->Advanced Configuration->Networks->Fencing->Passthroughs*. They can be of the following format:
...
The proxy passthroughs can be configured in *g->Advanced Configuration->Networks->Fencing->Proxy Passthroughs*.
...
*Configuration -> Advanced Configuration -> Networks -> Fencing* and check *Proxy Interception*.
...
In order to configure DNS enforcement, you first need to go in 'Configuration -> Advanced Configuration -> Networks -> Interfaces' then select one of your interfaces and set it in DNS enforcement mode.
...
The *parked* vs *unparked* state is controlled through violation `1300003` which gets triggered according to the `parking.threshold` setting ('g->Advanced Configuration->Networks->Device Parking').

So, in order to activate the parking, go in 'g->Advanced Configuration->Networks->Device Parking' and set the threshold to a certain amount of seconds. A suggested value would be `21600` which is 6 hours. This means that if a device stays in your registration network for more than 6 hours in a row, it will trigger violation `1300003` and place that device into the *parked* state.
...
* The `Template` attribute will only be used when the user is on the normal PacketFence portal and not the one dedicated for parking. If you want the user to access the non-parking portal, disable 'Show parking portal' in 'Configuration -> Advanced Configuration -> Networks -> Device Parking'
...
Available registration modes are defined on a per-portal-profile basis. These are configurable from *Configuration -> Policies and Access Control -> Connection Profiles*. To disable the self-registration feature, simply remove all self-registration sources from the connection profile definition. Notice however that if your default connection profile has no source, it will use all authentication sources.
...
From the PacketFence's web admin interface, you must configure an Admin Access role (g->Advanced Configuration->Admin access) that contains the action 'Switches CLI - Read' or 'Switches CLI - Write' and assign this role to an internal user or in an Administration rule in an internal source.

DHCP Option 82
~~~~~~~~~~~~~~

PacketFence is able to locate a device on the network even if the switch port is not managed by PacketFence.
To use this feature you need to add all the switches in PacketFence and enable SNMP read (switch and PacketFence side) and enable DHCP option 82 in Configuration -> Advanced Configuration -> Networks -> Network.
...
First of all, you will need to enable the NTLM caching globally by enabling 'NTLM Redis cache' in 'g->Advanced'. You then need to restart radiusd.

Once that is done, you need to configure PacketFence to start caching the credentials. In order to do so, go in 'Configuration->Policies and Access Control->Domains->Microsoft Active Directory Domains' and select the domain you want to cache the credentials for. 
...
After that, you will need to enable the `redis_ntlm_cache` service which is used by PacketFence to store the cached credentials. In order to do so, goviolation.logConfiguration->Main Configuration->Services' and enable 'redis_ntlm_cache' and save the changes.

Next, start the service via pfcmd:

  /usr/local/pf/bin/pfcmd service redis_ntlm_cache start

If you chose to enable 'NTLM cache background job' in one of your domains, you will need to enable the pfmon job that will periodically cache the credentials. This can be configured in 'Configuration->System Configuration->Main Configuration->Maintenance->NTLM cache job interval'. It is advised to set this to half the expiration of the credentials you have set in the domain configuration. This will ensure you have an optimal cache hit. Once done, restart the `pfmon` service.
...
Alternatively, you can configure these parameters from the PacketFence Web administrative GUI, in the *Configuration -> Advanced Configuration -> SNMP* section.
