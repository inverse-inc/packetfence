...
In the Administration interface under *Configuration -> Domains*.
...
* *Workgroup* is the workgroup of your domain.
...
* *Username* is the username that will be used for binding to the server. This account requires administrative privileges.
* *Password* is the password for the username defined above. Note that this password will be in *clear text* in the PacketFence configuration. We suggest you create an additionnal account specificly for this task.

NOTE: In order to troubleshoot unsuccessful binds, please refer to the following file : `/chroots/<mydomain>/var/log/samba<mydomain>/log.winbindd`. Replace `<mydomain>` with the identifier you set in the domain configuration.
...
If you are using multiple domains for authentication, you can define which one you want to use as the default one by creating the following realm
