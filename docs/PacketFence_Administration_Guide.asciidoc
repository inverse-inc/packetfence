...
Here are the different configuration parameters that can be set for each portal profiles. The only mandatory parameter is "filter", otherwise, PacketFence won't be able to correctly apply the portal profile.
  ...
  guest_modes = the availables guest self-registration modes (sms|email|sponsor|google|facebook)
...
The captive portal of PacketFence allows a guest/user to register using his Google or Facebook account.
...
field : https://YOUR_PORTAL_HOSTNAME/o2/google.  Of course, replace the hostname with the values from `general.hostname` and `general.domain`.
...
https://YOUR_PORTAL_HOSTNAME/o2/facebook
