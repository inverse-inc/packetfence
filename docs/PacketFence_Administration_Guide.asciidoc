...
This section assumes that you already have an Active Directory domain configuration both in 'Configuration->Domains' and 'Configuration->Sources'. If you don't, you need to first configure those. Refer to the appropriate sections of this guide for details on how to configure those two components.
...
To overcome this limitation, it is possible to use a Redis driven cache inside PacketFence to reduce the amount of authentications requiring NTLM authentication. Should a user be in the cache, PacketFence will attempt to compare the 802.1x credentials with this one. In the even that the validation fails, a call to ntlm_auth is made. In the event of a cache miss, an ntlm_auth call is made as well. This ensures that even if a user changes his password, his new password is immediately valid for 802.1x EAP-PEAP connections even if the cache contains the outdated entry.
...
In order to use this cache, you will need to install additionnal packets available from the packetfence-extra repository. These packets will allow PacketFence to query your Active Directory for the NT hash of the users.
