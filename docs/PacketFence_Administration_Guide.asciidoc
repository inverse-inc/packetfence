...
PacketFence is distributed in two different format: packages for Linux Debian and Red Hat Enterprise, and as a virtual machine image named Zero Effort NAC (ZEN).
...
PacketFence ZEN has been tested under VMWare ESXi 4.0 & 5.0 with 8GB RAM dedicated to the virtual machine. It might work using other virtualization products.  You need a 64-bit capable CPU on your host.
...
Through this Administration Guide's examples, we use the following assumptions for our network infrastructure:
