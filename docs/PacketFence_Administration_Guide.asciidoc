...
In the PacketFence administration interface, go in 'Configuration->Advanced Portal Configuration->Billing tiers'
...
These parameters can also be configured from the *Configuration -> -> Advanced Portal Configuration -> Device Registration* section.
...
In order to use the passthroughs feature in PacketFence, you need to enable it from the GUI in *Configuration->Advanced Configuration->Networks->Fencing*, enable *Passthrough* and then save.
...
Then add passthroughs in *Configuration->Advanced Configuration->Networks->Fencing->Passthroughs*. They can be of the following format:
...
The proxy passthroughs can be configured in *Configuration->Advanced Configuration->Networks->Fencing->Proxy Passthroughs*.
...
The *parked* vs *unparked* state is controlled through violation `1300003` which gets triggered according to the `parking.threshold` setting ('Configuration->Advanced Configuration->Networks->Device Parking').

So, in order to activate the parking, go in 'Configuration->Advanced Configuration->Networks->Device Parking' and set the threshold to a certain amount of seconds. A suggested value would be `21600` which is 6 hours. This means that if a device stays in your registration network for more than 6 hours in a row, it will trigger violation `1300003` and place that device into the *parked* state.
...
From the PacketFence's web admin interface, you must configure an Admin Access role (Configuration->Advanced Configuration->Admin access) that contains the action 'Switches CLI - Read' or 'Switches CLI - Write' and assign this role to an internal user or in an Administration rule in an internal source.
