...
Gaming devices registration::
  A registered user can access a special portal-page from within the network to allow him to register a gaming device of his own. This registration will require login from the user and then will register gaming devices with pre-approved MAC OUI into a configurable category.
...
Users have the possibility to register gaming devices (Microsoft XBOX/XBOX360, Nintendo DS/Wii, Sony PlayStation and so on) right from a special portal page. By accessing this page, users will be prompted to login as if they were registering themselves. Once they log in, the portal will ask them to enter the gaming device MAC address that will then be matched against a predefined list of authorized MAC OUI. Once done, the gaming device will be registered with the user's id and can be assigned into a specific category for easier management.
...
This URL is accessible from within the network, in any VLAN that can actually reach the PacketFence server.
