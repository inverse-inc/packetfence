...
OpenID Connect
^^^^^^^^^^^^^^
Using OpenID Connect is a bit deifferent than other OAuth2 sources. The reason behind that is because you will setup your own OpenID Connect source or depend on a provider for it. Details like token path, authorize path or API URL are specific to your setup. To start create your own or get a host please go here: http://openid.net/connect/. 
...
OpenID connect have different ways to configure it, make sure to create a client ID and a client secret to work with PacketFence.
