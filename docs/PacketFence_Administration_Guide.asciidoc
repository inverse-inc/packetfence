...
Under RHEL 6.x / Centos 6.x, you must exclude perl-Apache-Test from rpmforge and openfusion repository. You also need to exclude perl-Net-DNS and perl-Moose from the openfusion repository.
...
Edit `/etc/yum.repos.d/openfusion.repo` and add to the section [of] the line:
  ...
  exclude = perl-Apache-Test*,perl-Net-DNS*,perl-Moose*
