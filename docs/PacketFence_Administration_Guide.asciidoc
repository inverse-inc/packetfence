...
The captive portal of PacketFence allows a guest/user to register using his Google, Facebook, LinkedIn, Windows Live, Twitter, Kickbox, Instagram or Github account.
...
You can keep the default configuration, modify the App ID & App Secret (Given by Instagram on the developper plateform) and Portal URL (https://YOUR_PORTAL_HOSTNAME/oauth2/callback).

Also, add the following Authorized domains : *.instagram.com, *.fbcdn.net, *.akamaihd.net (May change)
...
CAUTION: By allowing OAuth through Instagram, you will give Instagram access to the users while they are sitting in the registration VLAN.
