...
In some cases, you may want to present a different captive portal (see below for the available customizations) according to the SSID, the VLAN, the switch IP/MAC or the URL the client connects to. To do so, PacketFence has the concept of portal profiles which gives you this possibility. 
...
Google
^^^^^^
In order to use Google as a OAuth2 provider, you need to get an API key to access their services.  Sign up here : http://code.google.com/apis/console.  Make sure you use this URL for the "Redirect URL"
...
This filter will return an error code (501) if the user agent is Dalvik and the URL doesn't contain _/generate_204.
