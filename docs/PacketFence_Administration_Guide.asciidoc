...
DNS enforcement allows you to control the network access of the device is using the pfdns service on PacketFence.

The architecture of DNS enforcement is as following : 
- DHCP and DNS is provided by the PacketFence server
  - DHCP leases will provide the IP of your network equipment as the gateway and the IP address of the PacketFence DNS server to resolve names.
- Routing is provided by another equipment on your network (Core switch, Firewall, ...)
- If a user should be shown the portal, the pfdns service will return a pointer to the captive portal, otherwise pfdns will resolve the name externally and use it in the reply.
...
Once this is done, you need to restart the pfdns service.
