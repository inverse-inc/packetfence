...
Device Sensor for Cisco hardware
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Device Sensor is a way to receive information about the device connected on a switch/WLC.
With this feature PacketFence receive the DHCP/LLDP/CDP/HTTP information in the accounting packet.
In order to enable you need to configure the switch/WLC and you have to enable it on the PacketFence side too.

To do that go in the Switch configuration and in the Radius Tab enable RadiusFingerprint.

image::docs/images/radius_fingerprint.png[scaledwidth="100%",alt="Radius Fingerprint"]

Since PacketFence receive information about LLDP and CDP, it's possible to detect that the device connected is VoIP capable.
In order to update the information in PacketFence, go in the Switch configuration and in the Definition Tab, enable VoIPAccountingDetect.

image::docs/images/VoIPAccountingDetection.png[scaledwidth="100%",alt="VoIP Accounting Detection"]
