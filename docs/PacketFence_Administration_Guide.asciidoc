...
RADIUS support (RADIUS Enforcement)::
  PacketFence can act as RADIUS only server, without having a registration, isolation, portal interface. In case you want to use PacketFence for your 802.1x authentication only. 
...
The concept of having a RADIUS enforcement is to not use registration and isolation, nor the portal. Everything here is for 802.1x authentication only. By default your management interface will be your RADIUS interface, if needed you can add another one via 'Network Configuration->Networks->Interfaces' adding an `Other` type of interface and the additionnal daemon `RADIUS`.
