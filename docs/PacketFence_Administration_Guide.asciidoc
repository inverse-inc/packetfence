...
Once a source is defined, it can be used from *Configuration -> Connection Profiles*. Each connection profile has a list of authentication sources to use.
...
* Redirect URL under *Configuration -> Connection Profile -> Portal Name*
...
Connection profiles should be managed from PacketFence's Web administrative GUI - from the *Configuration -> Connection Profiles* section. Adding a connection profile from that interface will correctly copy templates over - which can then be modified as you wish.

* Filters under *Configuration -> Connection Profile -> Portal Name -> Filters*
...
* Advanced filter under *Configuration -> Connection Profile -> Portal Name -> Advanced Filter*
...
Next, head to 'Configuration -> Connection Profiles', select the connection profile you use (most probably `default`) and then under 'Root Portal Module', assign `My first root module` then save your profile. If you were to access the captive portal now, an error would display since the Root module we configured doesn't contain anything.
...
Moreover, don't forget to add Google as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add Facebook as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add GitHub as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add Instagram as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add LinkedIn as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add Pinterest as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add Twitter as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Moreover, don't forget to add WindowsLive as a registration mode from your connection profile definition, available from *Configuration -> Connection Profiles and Pages*.
...
Go in Configuration -> Connection Profiles -> Edit a Portal -> Add Scan
...
Self-registration is enabled by default. It is part of the captive connection profile and can be accessed on the registration page by clicking the *Sign up* link.
...
Available registration modes are defined on a per-portal-profile basis. These are configurable from *Configuration -> Connection Profiles*. To disable the self-registration feature, simply remove all self-registration sources from the connection profile definition. Notice however that if your default connection profile has no source, it will use all authentication sources.
