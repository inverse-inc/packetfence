...
The outgoing interface should be specified by adding in `/usr/local/pf/conf/pf.conf` the option `interfaceSNAT` in inline section. It is a comma-delimited list of network interfaces like eth0,eth0.100. It's also possible to specify a network that will be routed instead of using NAT by adding in `conf/networks.conf` an option `nat=no` under one or more network sections.
...
It mean that when you add a new switch configuration in PacketFence's administration interface you don't have to restart the RADIUS service.
