...
You can force VPN users to authenticate first on the captive portal and based on
the role of the device allow it and/or set dynamic ACL.

In this example we assume that the Cisco ASA have 2 interfaces, one Management
(192.168.2.1) where the VPN is activated and another one Registration
(192.168.1.6) that is facing the PacketFence server (192.168.1.5).

Before trying to configure PacketFence with the Cisco ASA first be sure that
when you connect with AnyConnect and when the VPN is up that your device is able
to reach Internet.
...
In this section we will cover the OpenVPN installation on a Debian 11 machine
...
Then apply this patch:
 ...
 diff -ruN openvpn-radiusplugin-master.orig/Config.cpp
 openvpn-radiusplugin-master/Config.cpp
...
On the PacketFence side the only thing you need to do is to create a new switch
as type OpenVPN with the ip address 192.168.0.6 and with the shared secret
'secret'.
And enable "CLI Access Enabled" in the switch too to enable the radius-cli to
start.
