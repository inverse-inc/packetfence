...
NOTE: When doing MAC Authentication, there is a known issue with some Dell
switches. If you get errors where the device is using EAP type MD5, but
PacketFence is expecting PEAP, you will need to edit the line *default_eap_type
= peap* under the section *eap* in the file
*/usr/local/pf/conf/radiusd/eap.conf* to *default_eap_type = md5*.
...
On the interface, enable MAC based authentication mode, enable MAB, and set the
order of authentication to 802.1X followed by MAC authentication. Also enable
...
PacketFence supports this switch using RADIUS, MAC-Authentication, 802.1x and
VoIP
...
Important, even if "authentication allow-srcipanyacl enable" has been enable on
the switch, it doesn't support ACL with source ip and the ACL direction are only
in.
...
The configuration needs to be done is the one above (N1500 Series (FW >=
6.6.0.17))
