...
The configuration above enables authentication on port 1/1/1 - make sure you
change this to the ports where you want to perform enforcement.
...
While configuring the switch in PacketFence, ensure you set at least the
following values:
...
In order to enable VoIP, you first need to enable LLDP then define the network
policy for tagging VoIP traffic on the ports where PacketFence is enabled.
...
If you want to use the server PacketFence to authenticate users on the Brocade
switch.
...
NOTE: Make sure to have a local account in case the switch can not reach the
PacketFence server
