...
Note: The "Procurve" module is developed for the ProCurve 2500 Series switches using AOS Switch version 16.x and also this module inherits all its capabilities from the old "ProCurve 25
00 Series" module.
...
*Finally, make sure to change the value of the $VOICEVLANAME variable in the ArubaOS Switch 16.x (old Procurve 5400) module's source code.*
...
To use this feature, first configure RADIUS and the authentication method on your switch. Next, in the admin interface, go to _Configuration -> Policies and Access Control -> Network Devices -> Switches_. Click on the switch you want, then on the 'Roles' tab, and check 'Role by access list'. Now you are able to add ACLs for each role.
