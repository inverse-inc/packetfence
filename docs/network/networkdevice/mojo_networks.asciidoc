...
* Then click on _Configuration -> Device Configuration -> RADIUS Profiles ->
  Add a RADIUS Profile_
...
* Login to the https://dashboard.mojonetworks.com
* Go to *Wireless Manager*
* Then click on _Configuration -> Device Configuration -> SSID Profiles ->
  Add a new Profile -> WLAN_ 
...
To enable the external captive portal, go to the *SSID Profiles* page in *Device
Configuration*. Add a new Wi-Fi profile with the following attributes:
...
image::mojo-network-vlan.png[scaledwidth="100%",alt="Mojo Network"]
   ...
   Authentication with “http://IP_OR_HOSTNAME_OF_PACKETFENCE/Mojo” and the
   RADIUS shared secret. Click on *RADIUS Settings* to select PacketFence as
   authentication and accounting server.

image::mojo-external-splash.png[scaledwidth="100%",alt="Mojo External Splash
Page"]

   On the right, add the IP address or hostname of PacketFence to the Walled
   Garden Sites.
...
* Finally _Radio 1 - 2x2 b/g/n Configuration -> Add SSID Profile -> Select
  your SSID profile previously created(802.1X or Web authentication profile) ->
  Ok_
...
Add a Switch with the IP address of the Access Point (AP) with the following
configuration:
...
Add a switch with the IP address fo the Access Point (AP) with the following
configuration:
...
IMPORTANT: Clone the newly created switch and enter *192.0.2.254* or the MAC
address of the AP.
