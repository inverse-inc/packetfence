...
Avaya bought Nortel's wired networks assets. So Avaya switches are, in effect,
re-branded
Nortels. See <<_nortel,Nortel section>> of this document for configuration
instructions.

==== 802.1X with MAC Authentication Bypass and VoIP

NOTE: The configuration below requires an ntp server. We use the PacketFence
server as the NTP server but any other one will do. If you want to use the
PacketFence server for NTP, make sure you install the appropriate service and
open port 123 in [filename]`/usr/local/pf/conf/iptables-custom.conf.inc` (more
information available on Installation -> Advanced Topics -> Iptables).
