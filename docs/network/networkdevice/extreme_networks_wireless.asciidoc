...
On the Extreme Cloud IQ Controller, there should already be two built-in IP
Firewall policies:
...
Still, on the Extreme Cloud IQ Controller, create a wireless network with the
...
**** Allow user profile assignment using RADIUS attributes in addition to the
three tunnel RADIUS attributes: Enabled
