...
Use most secure feature available for your switch model in this order:
...
https://www.cisco.com/c/en/us/td/docs/switches/lan/catalyst3750/software/release/12-2_55_se/release/notes/OL23054.html[Minimal
IOS required for CoA is 12.2(52)SE]. Latest available IOS for 3550 is
...
the ACLs centrally in PacketFence without configuring them in your switches and
...
different ACLs for your registration VLAN, production VLAN, guest VLAN, etc.
...
Before continuing, configure your switch to be in MAC authentication bypass or
802.1X.
...
Now if for example, your normal users are placed in the 'default' category and
your guests in the 'guest' category.

If for example the 'default' category uses the network 192.168.5.0/24 and your
...
Or also you could restrict your users to use only your DNS server where
192.168.5.2 is your DNS server
