...
CAUTION: For 802.1X and MAB configurations, refer to <<Catalyst_RADIUS, this
section below>>.
...
image::cisco-downloadable-acl-cross-network.png[scaledwidth="100%",alt="Cross
network deny ACL"]

Could also only prevent the guest users from using shared directories

image::cisco-downloadable-acl-deny-shares.png[scaledwidth="100%",alt="Deny
shares ACL"]

Or also could restrict the users to use only the DNS server where
192.168.5.2 is the DNS server

image::cisco-downloadable-acl-force-dns.png[scaledwidth="100%",alt="Force DNS
ACL"]


==== Cisco IOS 15.5

CAUTION: For 802.1X and MAB configurations, refer to <<Catalyst_RADIUS_15_5,this
section below>>.
