...
This setup has been tested using VMWare ESXi 4.0 & 5.0 with 8192MB RAM dedicated to the virtual machine. It might work using other VMWare products.  You need a CPU that support long mode. *In other words, you need to have a 64-bit capable CPU on your host.*
...
* a regular, registration and isolation VLAN for wireless visitors (VLAN numbers and subnets)
...
First, execute the following commnand in an SSH session.
