...
This setup has been tested using VMWare ESXi 4.0 & 5.0, Fusion 3.x and Workstation 7.x with 8192MB RAM dedicated to the virtual machine. It might work using other VMWare products.  You need a CPU that support long mode. *In other words, you need to have a 64-bit capable CPU on your host.*

We build two separate virtual machine, one to run on ESXi 5.0 (OVF format) and one to run on VMWare Fusion/Workstation (VMX/VMDK format).
...
* We use a DHCP server on the PacketFence ZEN box which will take care of IP address distribution in VLANs 2 and 3
...
Newer version of VMWare Player handles the VLAN trunking a lot better.  Having that said, we can use a single interface on the VM.  So, you need to ensure that your VM host is plugged into a physical trunk port with VLAN 1,2 and 3 as the allowed VLAN.

IMPORTANT: We tested most with VMWare Player for Linux. We cannot support VMWare Fusion anymore, there are some issues when we need to do routing through Mac OS X VLAN interfaces.
...
In this guide we will show you how to configure the VLAN (out-of-band) mode. If you want to configure the INLINE (in-band) mode please refer to the guide PacketFence Inline Deployment Quick Guide using ZEN.
...
NOTE: If you don't setup a cluster, don't check High availability field.
