...
NOTE: This installation mode is not suggested unless you use the SSO for informationnal purposes (no enforcement). PacketFence will use easily spoofable UDP packets to communicate with the Palo Alto firewall. If you require encryption and origin validation of the SSO messages, please use the XML API.
