...
* a switch port in the inline network for the PacketFence ZEN box (for eth1) which needs to be configured in access mode and in the same access VLAN as every switchport on which devices will be plugged.
...
Make sure that there is two virtual network cards created. Assign the first card (eth0) to you management network (Production) and assign the second one (eth1) to the Inline network.
...
The inline enforcement is a very convenient method of performing access control on older network hardware that is not capable of doing VLAN enforcement or that is not supported by PacketFence.
...
By default the inline traffic is forwarded through the management but it should be possible to specify another one by adding in pf.conf the option interfaceSNAT in inline section. It is a comma delimited list of network interfaces like eth0,eth1.2. It's also possible to specify a network that will be routed instead of using NAT by adding in `conf/networks.conf` an option nat=no under one or more network sections (take care of the routing table of the PacketFence server).
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Before booting your VM, make sure that the PC is correctly plugged in the management network and that the link is up.
...
For the Inline interface, we plug this card in a switch port access VLAN 2.
...
Note that if you have a routed network that need to be take into account by PacketFence as an Inline subnet, you will need to add this via `Routed Network` and select Inline L3. This configuration is available only in the Administration GUI not in the configurator.
