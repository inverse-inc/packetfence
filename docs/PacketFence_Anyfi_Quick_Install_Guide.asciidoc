...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Now that you have a fully functional PacketFence installation, you'll need to add the Anyfi Gateway to the PacketFence switches database for correct integration.
