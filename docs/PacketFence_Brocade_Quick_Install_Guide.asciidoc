PacketFence: Brocade Quick Installation Guide
============================================
:encoding: UTF-8
:lang: en
:doctype: book
...
Configuring the Brocade equipement in PacketFence
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Now that you have a fully functionnal PacketFence installation, you'll need to add the Brocade switch and wireless controller to the PacketFence switches database for correct intergration.
...
PacketFence configuration is now done. You may now reboot the demonstation PC (leave the USB key in). Once rebooted, the web browser should open in the PacketFence web administation interface.
...
When done, we need to activate it on the switchport level.  For exemple, let's do it for port 10.  In "configure terminal", type :
...
Finally, you need to enable it at the switchport level.  Again, we will use port 10.  In configure termnial, type :
