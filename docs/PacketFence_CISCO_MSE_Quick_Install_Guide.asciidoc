...
* An account has been created on the MSE in order to use rest. (Read only for localization , write to create notifications)
...
This part can be done directly on the MSE web admin GUI but we include a perl script in addons/mse-subscribe.pl that will help you to create one by the rest api.

So let say that you have a write username and password on the MSE (mswrite/password) now go in /usr/local/pf/addons then run:

 ./mse-subscribe.pl --username=mseuser --password=password --url=http://192.168.1.6:8083 --target-ip=192.168.1.5 --target-port=9292 --url-path=/mse/ --zone=Campus>Building>Level>Zone --notification-name=Zone1
...
In `Configuration -> Portal Profiles -> Add profile -> Advanced filter` specify an advance filter like that:
...
Now go in `Nodes` and click on a mac address and you will see that a new tab appear, then if you click on it you will be able to retreive the map and the localization of the device.
