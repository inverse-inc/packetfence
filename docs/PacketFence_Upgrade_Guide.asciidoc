...
PacketFence installation can be upgraded in two ways:
...
For all PacketFence versions prior to 12.0.0, upgrading steps are described in the <<_archived_upgrade_notes,Archived upgrade notes>>.
...
The first steps are the same as in <<PacketFence_Clustering_Guide.asciidoc#_performing_an_upgrade_on_a_cluster,Performing an upgrade on a cluster>>
but at the step <<PacketFence_Clustering_Guide.asciidoc#_upgrade_node_c>> for node C,
follow the upgrade instructions <<PacketFence_Upgrade_Guide.html#_pre_automation_upgrade_standalone_redhat_el8>>.
