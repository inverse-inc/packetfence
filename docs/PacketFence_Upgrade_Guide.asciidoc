...
[source,bash]
----
/usr/local/pf/addons/upgrade/to-10.3-roles-acls.pl
----
...
`inline_accounting` table will be removed by upgrade of database schema because it has been replaced by `bandwidth_accounting` table since v10.
...
PacketFence now depends of the MariaDB version 10.2.
In order to upgrade the MariaDB version you need to execute the following steps before upgrading PacketFence.
...
If the error "Recovering after a crash using tc.log" appear during the mysql_upgrade command then delete the file tc.log in /var/lib/mysql/
...
sed -i "s/xtrabackup/mariabackup/g" sed -i /usr/local/pf/conf/mariadb/mariadb.conf.tt
----
