...
In the case of a standalone-server running this script will extract the current configuration from Samba and convert it to the NTLM_AUTH_API format.
In the case of a cluster the script will raise an error and the domain join will need to be configured manually.

Currently NTLM_AUTH is supported in cluster-mode, but every machine joined to the domain will use the exact same password.
The machine accounts in the active directory will need to be deleted and each server in the cluster joined one-by-one through the admin UI.

In the admin - using the VIP - go to Configuration -> Policies and Access Control -> Active Directory Domains.
Create a New Domain and specify the Server Name as %h (very important - each server will use its own hostname), fill all the remaining information and specify the Machine Account Password (keep it as it will have to be reused).
Click Save and you should be able to see the Machine Account created in the Active Directory Domain.

Next, on each remaining cluster member - go to Status -> Services -> API Redirect, and choose the 2nd+ server) and re-perform the previous step by visiting the newly created Domain, replace the Machine Account Password hash with the plain-text password and click Save. Continue with this step on the all the remaining cluster members.
