...
*CRITICAL UPGRADE REQUIREMENT*: PacketFence upgrades *MUST* be performed one major version at a time. Direct upgrades across multiple major versions (e.g., 11.x to 14.x) are *NOT SUPPORTED* and will result in configuration corruption and service failures.

*Required Upgrade Path Example*:
- From 11.x → 12.x → 13.x → 14.x (correct)
- From 11.x → 14.x (incorrect - will fail)

*Always upgrade incrementally*: 11.0 → 11.1 → 11.2 → 12.0 → 12.1 → 12.2 → 13.0 → 13.1 → 13.2 → 14.0 → 14.1 → latest
...
**Version Stepping Requirement**: All PacketFence upgrades must proceed one major version at a time to ensure database schema integrity and configuration compatibility.

=== Supported Upgrade Paths

Starting from PacketFence 11.0.0, choose the appropriate upgrade method:

**1. Maintenance Patches** (within same major.minor version)
- Example: 14.0.0 → 14.0.1 → 14.0.2
- Method: <<#_apply_maintenance_patches,Apply maintenance patches>>
- **Safe to skip patch versions**: You can upgrade directly from 14.0.0 → 14.0.5

**2. Minor Version Upgrades** (within same major version)
- Example: 14.0.x → 14.1.x → 14.2.x
- Method: <<#_sequential_version_upgrades_major_and_minor,Sequential Version Upgrades>>
- **Must upgrade sequentially**: Each minor version must be installed in order

**3. Major Version Upgrades** (cross-version upgrades)
- Example: 13.x → 14.x → 15.x
- Method: <<#_sequential_version_upgrades_major_and_minor,Sequential Version Upgrades>>
- **CRITICAL**: Must be performed one major version at a time
- **Never skip major versions**: Attempting to upgrade from 11.x directly to 14.x will fail

=== Pre-11.0.0 Systems

For PacketFence versions prior to 11.0.0:
1. **Upgrade to 10.3.0** first (if not already)
2. **Export/import to 11.0.0** (OS migration required)
3. **Continue sequential upgrades**: 11.0 → 11.1 → 11.2 → 12.0 → etc.
...
== Sequential Version Upgrades (Major and Minor)

**IMPORTANT**: This section covers upgrading between different PacketFence versions. All upgrades must follow sequential version order - never skip major versions.
...
**Example Upgrade Sequence** (13.1 → 14.1):
1. Upgrade 13.1 → 13.2 (complete all 13.2 steps)
2. Upgrade 13.2 → 14.0 (complete all 14.0 steps) 
3. Upgrade 14.0 → 14.1 (complete all 14.1 steps)

**Never attempt**: 13.1 → 14.1 directly
...
2. **Follow cluster guide**: See https://www.packetfence.org/doc/PacketFence_Clustering_Guide.html#performing_an_upgrade_on_a_cluster[Performing an upgrade on a cluster]
...
**Prerequisites**:
1. **Source system**: Must be PacketFence 10.3.0 or later
2. **Target system**: Fresh OS installation (Debian 11+ or RHEL 8+)
3. **Review limitations**: Read https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#assumptions_and_limitations[assumptions and limitations]

**After 11.0 Migration**: Continue with sequential upgrades (11.0 → 11.1 → 11.2 → etc.)

=== Step 1: Prepare Source System for Export

==== Systems Before 10.3.0

**Sequential Upgrade Required**:
1. **Identify current version**: Run `/usr/local/pf/bin/pfcmd version`
2. **Upgrade path to 10.3.0**: Follow version-by-version upgrades
   - Example: 9.3.x → 10.0.x → 10.1.x → 10.2.x → 10.3.0
3. **Verify 10.3.0 functionality**: Ensure all services work correctly
...
==== Systems Running 10.3.0 or Later

**Ready for Export**:
1. **Verify version**: Confirm you're running 10.3.0+
2. **Begin export**: Follow https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#export_on_current_installation[export process]
3. **Validate export**: Ensure export completes without errors

=== Import (on new installation)

Follow instructions related to https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#import_on_new_installation[import process].
...
**Upgrade Sequence Example** (11.0 → 13.0):
1. Complete "Upgrading from 11.0 to 11.1" procedures
2. Complete "Upgrading from 11.1 to 11.2" procedures  
3. Complete "Upgrading from 11.2 to 12.0" procedures
4. Continue until reaching 13.0
...
https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#eap_local_user_authentication[EAP
...
If ProxySQL causes issues in the deployment, revert back to haproxy-db by following https://www.packetfence.org/doc/PacketFence_Clustering_Guide.html#database_via_proxysql_or_haproxy_db[these instructions]
...
PacketFence 14.0.0 introduces changes to supported operating systems:
...
1. Use the automated upgrade process: <<#_sequential_version_upgrades_major_and_minor,Sequential Version Upgrades (major or minor)>>
...
If mariadb-backup is not installed, follow the standard export process at https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#export_on_current_installation[export on current installation].

**Prerequisites**: Read https://www.packetfence.org/doc/PacketFence_Installation_Guide.html#assumptions_and_limitations[assumptions and limitations] before proceeding.
...
To build a new cluster or rebuild an existing cluster, follow instructions in https://www.packetfence.org/doc/PacketFence_Clustering_Guide.html#cluster_setup[Cluster setup section].
...
**Cluster-specific procedure** - Follow the cluster upgrade guide: https://www.packetfence.org/doc/PacketFence_Clustering_Guide.html#performing_an_upgrade_on_a_cluster[Performing an upgrade on a cluster]
