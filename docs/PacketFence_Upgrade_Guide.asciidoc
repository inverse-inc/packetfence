...
Take a complete backup before upgrading.
...
Stop all running PacketFence services:
...
Follow the <<PacketFence_Upgrade_Guide.asciidoc#_automation_of_upgrades,automation of upgrades>> instructions.

=== For a cluster

Refer to the <<PacketFence_Clustering_Guide.asciidoc#_performing_an_upgrade_on_a_cluster,Performing an upgrade on a cluster>> section in the <<PacketFence_Clustering_Guide.asciidoc#,PacketFence Clustering Guide>>.
...
Follow the <<PacketFence_Installation_Guide.asciidoc#_export_on_current_installation,export process>> instructions.

=== Import (on new installation)

Follow the <<PacketFence_Installation_Guide.asciidoc#_import_on_new_installation,import process>> instructions.
...
Run this command to upgrade the database schema:
...
TLS 1.0 and TLS 1.1 are now disabled by default. If supplicants are currently using these protocols, move to TLS 1.2. If TLS 1.2 is not possible adjust
...
PacketFence now allows enabling or disabling local authentication for 802.1X directly in the web admin.
...
Cluster upgrades are not automated. Follow these instructions to upgrade configuration and database schema.
...
Use the provided SQL upgrade script to upgrade from 11.0 to 11.1 schema.
...
The database schema has changed.
Use the provided SQL upgrade script to upgrade from 11.1 to 11.2 schema.
...
Check filter conditions (VLAN, RADIUS, Switch, DNS, DHCP, and Profile) that use the ```not equals`` operator.
Verify the logic handles null/undef values correctly.

Add an additional defined condition to filters that must ensure a value is defined.

=== Notification on certificates expiration in pfpki

When using `pfpki` with PKI templates created without email attribute, set a value for this attribute.

This enables `pfpki` to use email addresses from PKI templates for certificate expiration notifications on certificates without emails.

== Upgrading from a version prior to 12.0.0

=== Tenant code deprecated

PacketFence tenant management code has been removed. Stay on releases prior to 12.0 if tenants are required.
...
Bandwidth accounting tracking is disabled by default.
To enable bandwidth reports or security events:
1. Go to _Configuration -> System Configuration -> RADIUS -> General_
2. Enable 'Process Bandwidth Accounting'
3. Restart the `pfacct` service to apply changes.

=== Fix permissions and checkups deprecated

API calls for fixing permissions and performing checkups from web admin are deprecated.
Containerization of services made these calls obsolete.
...
NOTE: This applies when a RADIUS authentication source is configured in PacketFence. Ignore this section if PacketFence is used as a RADIUS server without a RADIUS authentication source.

RADIUS authentication sources previously used the packet source IP in the NAS-IP-Address field. This behavior is deprecated in favor of using the management IP address (or VIP in clusters). Configure alternative NAS-IP-Address values directly in the RADIUS authentication source if needed.

=== Log files names updated

Some log file names have changed:
...
If successful, restart services using the <<PacketFence_Upgrade_Guide.asciidoc#_restart_packetfence_services,restart PacketFence services>> instructions.


===== Additional steps to build or rebuild a cluster

To build or rebuild a cluster, follow the <<PacketFence_Clustering_Guide.asciidoc#_cluster_setup,Cluster setup>> instructions.
...
API::
  <<#_upgrading_from_a_version_prior_to_12_0_0,Upgrading from a version prior to 12.0.0>>

Authentication::
  <<#_upgrading_from_a_version_prior_to_11_1_0,Upgrading from a version prior to 11.1.0>>, <<#_upgrading_from_a_version_prior_to_12_0_0,Upgrading from a version prior to 12.0.0>>

Automation::
  <<#_automation_of_upgrades,Automation of Upgrades>>

Backup::
  <<#_general_upgrade_tips,General Upgrade Tips>>, <<#_upgrading_from_a_version_prior_to_12_0_0,Upgrading from a version prior to 12.0.0>>

Bandwidth::
  <<#_upgrading_from_a_version_prior_to_12_0_0,Upgrading from a version prior to 12.0.0>>

Certificate::
  <<#_upgrading_from_a_version_prior_to_12_1_0,Upgrading from a version prior to 12.1.0>>

Cluster::
  <<#_upgrade_to_another_version_major_or_minor,Upgrade to another version (major or minor)>>, <<#_upgrading_from_a_version_prior_to_11_1_0,Upgrading from a version prior to 11.1.0>>

Database::
  <<#_general_upgrade_tips,General Upgrade Tips>>, <<#_upgrading_from_a_version_prior_to_11_1_0,Upgrading from a version prior to 11.1.0>>

DHCP::
  <<#_upgrading_from_a_version_prior_to_13_0_0,Upgrading from a version prior to 13.0.0>>

EAP::
  <<#_upgrading_from_a_version_prior_to_11_1_0,Upgrading from a version prior to 11.1.0>>

Filters::
  <<#_upgrading_from_a_version_prior_to_11_2_0,Upgrading from a version prior to 11.2.0>>

Firewall::
  <<#_upgrading_from_a_version_prior_to_13_1_0,Upgrading from a version prior to 13.1.0>>
...
NTLM::
  <<#_upgrading_from_a_version_prior_to_11_0_0,Upgrading from a version prior to 11.0.0>>

Packages::
  <<#_general_upgrade_tips,General Upgrade Tips>>
...
Schema::
  <<#_upgrading_from_a_version_prior_to_11_1_0,Upgrading from a version prior to 11.1.0>>, <<#_upgrading_from_a_version_prior_to_11_2_0,Upgrading from a version prior to 11.2.0>>
