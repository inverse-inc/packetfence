...
In this release of PacketFence we moved from Samba to a new NTLM_AUTH_API service.
In order to upgrade the domain join perform the following:
...
Running the previous script will be enough and it will extract the current configuration from Samba and convert it to the NTLM_AUTH_API format.

==== Cluster

The script will detect if you are running PacketFence in a cluster environment and will detect 2 scenarios:

1. If the script detect that the Samba machine name match the hostname then the script will migrate the configuration to the NTLM_AUTH_API format and remplace the machine name to %h.

2. If the script detect that the Samba machine name doesn't match with the hostname then you will have to delete the machine accounts in the AD and reconfigure the join manually.

In both cases the NTLM_AUTH_API is supported in a cluster mode but each machine joined to the domain need to own the same exact password.

Depending of the action of the script, you should see or no a configuration for your domain(s) under _Configuration -> Policies and Access Control -> Active Directory Domains_

Create or edit a Domain and really important, specify the Server Name as %h (it will use the hostname of the server), fill all the informations and specify the Machine account password (keep it you will have to reuse it) and the credentials of a admin account who is able to join a machine to the Domain..
And click Save and you should be able to see the Machine account created in the Active Directory Domain.

Next go on the server 2 (Status -> Services -> API Redirect and choose the server 2) and do the same thing that you did on the server 1 (the Domain will already there) and this time the Machine Account Password will be a hash, retype the same password you set on the previous server.
