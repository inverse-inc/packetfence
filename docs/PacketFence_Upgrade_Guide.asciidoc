...
In place upgrades are supported for Redhat EL8. You can follow up the current <<PacketFence_Upgrade_Guide.html#_upgrade_to_another_version_major_or_minor,Upgrade to another version (major or minor)>>.

=== Debian 12

Starting from PacketFence 14.0.0, Debian 11 support is dropped in benefit of Debian 12.
In place upgrades are not supported for Debian. You will have to provision new operating system(s) in order to migrate from Debian 11 to Debian 12 or From EL8 to Debian 12.

To simplify upgrade process to PacketFence 14.0.0 and future versions, we continue to rely on an export/import mechanism.

By default, mariadb-backup is installed on any cluster. It could be used on a standalone too.

To know if you are using mariadb-backup, check if the package is installed or if the binary is there:
...
If you are not using it, please just follow the default export process explained there <<PacketFence_Installation_Guide.html#_export_on_current_installation,export on current installation>>.

To export from Debian 11 or from RedHat EL8 with a Mariadb backup installed to Debian 12 (only), you will need to change mariadb-backup on the current installation.

Before doing anything else, be sure to read <<PacketFence_Installation_Guide.asciidoc#_assumptions_and_limitations,assumptions and limitations>> of this mechanism.

==== Export DB with mariadb-backup installed to PacketFence 14.0 on Debian 12

===== From PacketFence version 11.1 to 13.2

* On Debian 11

The main idea will be to install mariadb-backup used in Debian 12 in the current VM then do the db backup then restore the current mariadb-backup version.

To do the database backup in order to export it later, please use the following steps and command lines:

** Install the next version of mariabd-backup
...
mv /root/mariadb-backup/usr/bin/mariabackup /usr/bin/mariabackup
...
** reinstall the correct version of mariadb-backup

----
apt reinstall mariadb-backup -y
----

* on Red Hat EL8

This one is require more steps on the EL8 first and then on the Debian 12.
...
The name should be something like that packetfence-db-dump-innobackup-2024-09-20_08h31.xbstream.gz
You should find it in `/root/backup` directory.

** Prepare backuped database on Debian 11

----
gunzip packetfence-db-dump-innobackup-2024-09-20_08h31.xbstream.gz
mkdir -p /root/backup/restore/
pushd /root/backup/restore/
mv packetfence-db-dump-innobackup-2024-09-20_08h42.xbstream /root/backup/restore/
cd /root/backup/restore/
mbstream -x < packetfence-db-dump-innobackup-2024-09-20_08h42.xbstream
rm packetfence-db-dump-innobackup-2024-09-20_08h42.xbstream
mariabackup --prepare --target-dir=./
----

 => then send files with scp to the new Debian 12 server with Packetfence 14.0 already installed.

----
ssh root@DEBIAN_12_IP mkdir -p /root/backup/restore/
scp /root/backup/restore/* root@DEBIAN_12_IP:/root/backup/restore/
----

** Prepare files to be used on Debian 12
...
The command above will create your export archive in /tmp/export.tgz. You will now need to copy this file to your new server using SCP or your prefered mechanism.
