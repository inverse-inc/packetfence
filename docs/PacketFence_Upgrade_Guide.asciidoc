...
=== Automated upgrade (available starting from PacketFence 10.3.0)

You can use our
<<PacketFence_Installation_Guide.asciidoc#_exportimport_mechanism,export/import
mechanism>> if your current PacketFence version is at least 10.3.0.
...
=== NTLM cache background job deprecated

The option `NTLM cache background job` has been deprecated. If you previously
used it on one of your domains, you should remove it by editing
[filename]`/usr/local/pf/conf/domain.conf` and set
`ntlm_cache_batch=disabled`.

IMPORTANT: You could still use `NTLM cache background job individual fetch`
