...
* <<#_upgrade_to_a_patch_version_x_y_patch,Upgrade to a patch version (X.Y.PATCH)>>
* <<#_upgrade_to_a_major_or_minor_version_major_minor_z,Upgrade to a major or minor version (MAJOR.MINOR.Z)>>

For all PacketFence versions prior to 11.0.0, follow the steps described in the <<_upgrade_procedure,Upgrade procedure>>.

== Upgrade to a patch version (X.Y.PATCH)
...
== Upgrade to a major or minor version (MAJOR.MINOR.Z)

IMPORTANT: This type of upgrade is only available starting from PacketFence 11.0.0

Follow instructions related to <<PacketFence_Installation_Guide.asciidoc#_automation_of_upgrades,automation of upgrades>>.

== Upgrade procedure

A PacketFence upgrade consists of the following steps:

. stop all PacketFence services (see <<_stop_all_packetfence_services,Stop all PacketFence services>>)
. preliminary step(s) to run before packages upgrades (see sections for your upgrade path)
. packages upgrades (see <<_packages_upgrades,Package upgrades>>)
. configuration migration(s) (see sections for your upgrade path)
. database schema upgrade(s) (see sections for your upgrade path)
. restart all PacketFence services (see <<_restart_all_packetfence_services,Restart all PacketFence services>>)
. regenerate Monit configuration (optional) (see <<_regenerate_monit_configuration,Regenerate Monit configuration>>)

Upgrade notes for a given upgrade path are cumulative. That is to say, upgrading from version 7.0 to version 9.0 all of the following must be applied in order for each sequential version (7.0, 8.0 and 9.0):

. preliminary step(s) to run before packages upgrades
. configuration migration(s)
. database schema upgrade(s)

IMPORTANT: Before starting packages upgrades read the upgrade
instructions specific to the upgrade path taken since some preliminary steps may be
required *BEFORE* the packages are upgraded.
...
==== Cluster upgrade before 10.3

There is a specific cluster upgrade that must be performed if the current version is prior to 10.3. Follow the steps described in <<_mariadb_upgrade_to_10_2,MariaDB upgrade to 10.2>>.

=== Stop all PacketFence services

It is recommended to stop all the `packetfence-config` and `packetfence` services before proceeding any further:
...
If you reboot a standalone server or any server member in a cluster after services
...
This will ensure services do not restart after a reboot.

After rebooting revert to the previous target:
...
Update PacketFence with [command]`yum`:

// subs=attributes allow to substitute {release_minor} in code block
[source,bash,subs="attributes"]
----
yum localinstall -y https://www.inverse.ca/downloads/PacketFence/RHEL8/packetfence-release-{release_minor}.el8.noarch.rpm
...
To *ONLY* upgrade your RHEL / CentOS system to 7.7, perform the following steps *BEFORE* you perform your OS update with [command]`yum`.

.Disable `systemd-logind` service (known issues with `systemctl isolate`):
[source,bash]
----
...
This prevents network disconnects while [command]`yum` is performing the update. The last command ensures the current process is removed from the `user-0.slice`.
...
Update PacketFence with [command]`apt`:

[source,bash,subs="attributes"]
----
echo "deb http://inverse.ca/downloads/PacketFence/debian/{release_minor} bullseye bullseye" > /etc/apt/sources.list.d/packetfence.list
apt update
apt install packetfence
----

PacketFence relies heavily on Fingerbank, it is recommended to use the latest version:
...
PacketFence and Fingerbank are now upgraded. However, there may be additional steps that are required depending on the version being upgrading from.
...
Once all the <<_steps,steps>> of the upgrade procedure are completed, restart the `packetfence-config` and `packetfence` services:
...
To upgrade and regenerate the Monit configuration:

.For PacketFence versions prior to v11.1.0:
[source,bash]
----
/usr/local/pf/addons/monit/monit_build_configuration.pl 'email(s)' 'subject' 'configurations' 'mailserver'
----

.For PacketFence versions v11.1.0 and later:
[source,bash]
----
/usr/local/pf/bin/pfcmd generatemonitconfig
----

. Enable and start `monit` (if stopped earlier):
...
=== Regenerate domain(s) configuration

Active Directory fail-over have been improved. To benefit from such
improvements, you need to regenerate domain(s) configuration with following
commands:
...
IMPORTANT: During a cluster upgrade, you need to run these commands on each cluster member.
...
The `pf-maint.pl` script used to get maintenance patches has been deprecated. You can now get maintenance patches using your package manager, see <<#_upgrade_to_a_patch_version_x_y_patch,Upgrade to a patch version (X.Y.PATCH) section>>.
...
PacketFence now allow to enable or disable local authentication for 802.1X directly in web admmin.
...
If you rely on bandwidth reports, security events or online/offline then enable it by doing the following.
