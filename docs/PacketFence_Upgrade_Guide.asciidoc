...
NOTE: Starting from PacketFence 11.0.0, this step is not necessary if you are
doing an
...
Starting from PacketFence 11.0.0, you can upgrade your PacketFence installation in two ways:
...
For all PacketFence versions prior to 11.0.0, you should follow <<_upgrade_procedure,steps described in Upgrade procedure section>>.
...
WARNING: All changes to files not considered as configuration files will be overwritten by new packages.
...
If you run a version prior to 10.3, cluster upgrade for this version is specific. You need to follow steps described in <<_mariadb_upgrade_to_10_2,MariaDB upgrade to 10.2>> section.
...
. Enable and start monit (if you stopped it at beginning of upgrade):
...
Then enable 'Record accounting in SQL tables'
