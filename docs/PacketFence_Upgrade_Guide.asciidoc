...
The new version provide a dynamic iptables rules creations. If files /usr/local/pf/conf/iptables-input.conf.inc or /usr/local/pf/conf/iptables-input-management.conf.inc contain lines, it should be manually translated in a new json file: /usr/local/pf/conf/iptables-custom.conf.inc.
