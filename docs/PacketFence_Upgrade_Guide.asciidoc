...
In place upgrades are not supported, in order to upgrade to PacketFence 11.0.0, you need to follow next steps.

=== Steps to perform on current installation (PacketFence 10.3.0)

Current installation needs to run on PacketFence 10.3.0. You need to follow upgrade path to PacketFence 10.3.0 before doing anything else.

==== Export

To simplify upgrade process from PacketFence 10.3.0 to PacketFence 11.0.0, we
provide an export script which will be used to export your 10.3.0 configuration.
...
[source,bash,subs="attributes"]
----
wget -O http://packetfence.org/downloads/PacketFence/debian/packetfence-export_{release_minor}.deb
dpkg -i packetfence-export_{release_minor}.deb
----

===== Execution

FIXME: link to JSN process

=== Steps to perform on new installation (PacketFence 11.0.0)
