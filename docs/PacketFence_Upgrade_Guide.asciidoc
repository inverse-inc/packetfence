...
**Never attempt**: Major version upgrades like 13.x → 14.x (not supported if the OS changed)
...
according to general and specific upgrades steps, like <<#_mariadb_specific_upgrades,MariaDB Specific Upgrades>>.
...
Certain PacketFence upgrades **require following specific procedures**, particularly when
upgrading MariaDB across major versions. Each PacketFence release is validated against
...
same major version. Be aware that even minor version upgrade can have specific
upgrade steps.
====
...
[options="compact"]
 ...
 . **Follow cluster guide**: See <<PacketFence_Clustering_Guide.asciidoc#performing_an_upgrade_on_a_cluster,Performing an upgrade on a cluster>>
