...
|**Yes** - from Debian 9/CentOS 7

|Prior to 11.0
|Debian 9, CentOS 7 (deprecated)
|**Yes** - to continue receiving updates
|===
...
=== MariaDB Incremental Upgrade Requirement

PacketFence **requires sequential version upgrades** because MariaDB must be
upgraded incrementally through major versions. Each PacketFence version is
tested with specific MariaDB versions, and skipping PacketFence versions can
result in incompatible MariaDB schema changes.

*Why Sequential Upgrades are Mandatory*:
- MariaDB cannot safely skip major version upgrades
- Database schema changes are cumulative and version-dependent
- Configuration changes may depend on specific MariaDB features
- Skipping versions can cause irreversible database corruption
...
[IMPORTANT]
====
**Sequential Upgrades Required**: All PacketFence upgrades must be performed
sequentially. See <<#_mariadb_incremental_upgrade_requirement,MariaDB Incremental Upgrade Requirement>>
for the technical reasons why skipping versions will cause database corruption.
====
...
packages to ensure compatibility and security.
...
# Check if reboot is required after kernel/core updates
if [ -f /var/run/reboot-required ]; then
    echo "Reboot required after OS updates"
    # Reboot and continue upgrade process after restart
fi
----
...
needs-restarting -r
# If reboot needed, restart system before continuing
----

*Critical Components Updated*: OS package updates ensure MariaDB, Apache,
FreeRADIUS, and other system dependencies are current and compatible with the
PacketFence version being installed.
...
**IMPORTANT**: This section covers upgrading between minor versions within the
same major version. Major version upgrades are not supported - use migration
procedures instead.

[TIP]
====
**OS Migration Required?** Check the <<#_operating_system_compatibility,Operating System Compatibility>>
matrix first. If your target version requires a different operating system,
use <<#_upgrading_from_packetfence_10x_to_110,export/import migration>> instead
of these in-place upgrade procedures.
====

**Before Starting**:
1. **Update OS packages**: Follow <<#_update_operating_system_packages,OS package update procedures>>
2. **Verify current version**: Run `/usr/local/pf/bin/pfcmd version` to confirm your starting version
3. **Plan upgrade path**: Determine all intermediate versions needed to reach your target
4. **Review breaking changes**: Check upgrade notes for each version in your path
...
[NOTE]
====
**Before Using Automation**: Review the <<#_operating_system_compatibility,Operating System Compatibility>>
...
[WARNING]
====
**CLUSTER ENVIRONMENTS**: Export/import procedures on clusters require
**additional coordination steps** beyond standalone systems. **Data loss can
occur** if cluster-specific procedures are not followed correctly. All cluster
nodes must be properly synchronized during the migration process.

**Cluster-Specific Requirements**:
- All nodes must be stopped in correct sequence
- Database synchronization must be verified before export
- Import must be performed on primary node first
- Additional cluster reconfiguration required after import
====
...
4. **For clusters**: Review <<PacketFence_Clustering_Guide.asciidoc#performing_an_upgrade_on_a_cluster,cluster upgrade procedures>>
...
**Upgrade Type**: Minor version upgrade (configuration + database changes)
...
**Upgrade Type**: Major version upgrade (may require OS migration)
...
- **RedHat EL8**: Version varies by repository (typically 10.3 or 10.5)

**Alternative Export Method**: If mariadb-backup is not installed, use the
standard export process at <<PacketFence_Installation_Guide.asciidoc#export_on_current_installation,export on current installation>>.

**Additional Prerequisites**: Review <<PacketFence_Installation_Guide.asciidoc#assumptions_and_limitations,assumptions and limitations>> before proceeding.
...
in-place upgrades**. The export/import migration procedure handles database
schema changes automatically during the migration process.
====
