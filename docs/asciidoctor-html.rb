...
class PfHtml5Converter < (Asciidoctor::Converter.for 'html5')
  ...
  def convert_admonition node
    ...
    color = name == 'caution' ? 'var(--color-orange)' : 'var(--color-primary)'
    bgColor = name == 'caution' ? 'var(--color-orange-opacity)' : 'var(--color-primary-opacity)'
    %(<div#{id_attr} class="alert" style="background-color:#{bgColor};border-left:3px solid #{color}">
  <div class="media pt-2 pb-2">
    <div class="mr-3 align-self-start" style="color:#{color}">
  ...
  def convert_ulist node
    result = []
    id_attribute = node.id ? %( id="#{node.id}") : ''
    div_classes = ['ulist', node.style, node.role, (node.option? 'compact') ? 'compact' : ''].compact
