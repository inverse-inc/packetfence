...
If nodes fail to rejoin the cluster or remain in read-only mode unexpectedly, see <<#_checking_the_mariadb_sync,Checking the MariaDB sync>> and <<#_cluster_database_recovery,Cluster Database Recovery>> in the Troubleshooting section for Galera synchronization issues.
