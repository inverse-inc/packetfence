...
much in-depth knowledge, but if you want to use the captive portal with a layer
3 cluster, this will definitely make your setup more complex and will certainly
...
This section will describe the changes to do on your [filename]`cluster.conf`
when dealing with layer 3 clusters but doesn't cover all the cluster
installation. In order to install your cluster, follow the instructions in
<<_cluster_setup>> and refer to this section when reaching the step to configure
your [filename]`cluster.conf`.

=== Simple RADIUS only cluster

In order to configure a RADIUS only layer 3 cluster, you will need at least 3
...
When you are at the step where you need to configure your cluster.conf during
the cluster setup, refer to the example below to build your cluster.conf.
...
cluster, you will need at least 3 servers (5 are used in this example) with 2
...
The schema below presents the routing that needs to be setup in your network in
...
NOTE: You should use the configuration above to perform the cluster setup and
complete all the steps required to build your cluster. You should only continue
these steps after it is fully setup and running.

==== Servers network configuration

After you've finished configuring your cluster, on one of the servers, add the
...
Then, to complete the client network configuration, you will need to override
...
you've performed the configuration:
...
In cluster layer3 configuration you can configure a remote MariaDB server in
...
the main cluster. If you do not know which server is
...
The "MySQL master ip address" is the ip address of the master server where you
created the backup file. Not the VIP of the primary cluster.
...
Then you need to be sure that the root user exist in the remote database and
...
At the end it you want to check the status of the slave server for debug
purposes you can run the follwing command:
