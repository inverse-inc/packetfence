...
WARNING: *aMuchMoreSecurePassword* is only for example purpose, a custom password needs to be defined. This user should have a password that contains only alphanumeric characters (letters, numbers and importantly, *no spaces*).
...
If the configurator fails or cluster setup encounters issues, see <<#_service_startup_failures,Service Startup Failures>> and <<#_checking_the_mariadb_sync,Checking the MariaDB sync>> in the Troubleshooting section.
...
In order for PacketFence to communicate properly with the MariaDB cluster, change the following.
...
If database connectivity issues persist beyond expected, see <<#_database_connectivity_issues,Database Connectivity Issues>> and <<#_checking_the_mariadb_sync,Checking the MariaDB sync>> in the Troubleshooting section.
...
In order to create a new cluster, configure [filename]`/usr/local/pf/conf/cluster.conf` *on the first server* of the cluster.

Configure it with the server hostname. Use : [command]`hostname` command (without any arguments) to get it.
...
Please refer to <<PacketFence_Installation_Guide.asciidoc#domain_joining_on_a_packetfence_cluster_v14_x,Domain joining on a PacketFence cluster v14.x>> for detailed steps.

If domain joining fails on cluster nodes, see <<#_network_connectivity_issues,Network Connectivity Issues>>, <<#_authentication_failures,Authentication Failures>>, and <<#_radius_debugging,RADIUS Debugging>> in the Troubleshooting section.
