...
IMPORTANT: In a three nodes cluster, you can offer service with at least one
node.

==== Using a clean shutdown

If you stop the `packetfence-mariadb` service properly on a node or if you
shutdown your node properly, the cluster will detect this shutdown and continue
to
operate.
...
For cluster maintenance issues or service failures, see <<#_service_startup_failures,Service Startup Failures>>, <<#_checking_the_mariadb_sync,Checking the MariaDB sync>>, and <<#_cluster_database_recovery,Cluster Database Recovery>> in the Troubleshooting section.
