...
RHEL / CentOS
----
yum localinstall http://packetfence.org/downloads/PacketFence/RHEL6/`uname -i`/RPMS/packetfence-release-1.2-5.1.noarch.rpm -y
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
----
gpg --keyserver keyserver.ubuntu.com --recv E3A28334
----
...
Replace the monit addon directory
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
----
...
Where :
[options="compact"]
* `email(s)`: CSV (no spaces) of recipient email addresses.
* `subject`: Email subject line (example: 'Packetfence Monit Alert').
...
NOTE: This command will create configuration scripts in `/etc/monit.d/`, but it will not remove old(er) scripts from an earlier installation. Unused `/etc/monit.d/*.conf` will need to be removed manually.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
This step is only required during an upgrade.
----
...
This step is only needed if `monit summary` is stuck `Waiting` on `packetfence-etcd`.

First stop `etcd` on all nodes in the cluster:

----
systemctl stop packetfence-etcd
----

Once packetfence-etcd is stopped on all nodes, remove the contents on the `etcd` directory.
----
rm -Rf /usr/local/pf/var/etcd/*
----

Restart `etcd` on all nodes in the cluster.
----
systemctl stop packetfence-etcd
----
