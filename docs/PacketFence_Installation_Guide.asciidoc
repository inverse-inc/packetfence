...
This step is only needed if `monit summary` is stuck `Waiting` on `packetfence-etcd`. This error indicates that the cluster is currently in 'Split Brain'. Please follow the instructions in the PacketFence Clustering Guide on how to resolve this. 
