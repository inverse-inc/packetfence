...
(merge_return means that the original answer of PacketFence will be merge with the filter answer automatically):
  ...
  [Ethernet-EAP-Accept]
  status=enabled
  description=Return Access-Accept when the connection is Ethernet-EAP and when there is no security event
  scopes=returnRadiusAccessAccept
  radius_status=RLM_MODULE_OK
  merge_answer=no
...
preProcess scope can be useful when you want to create internal RADIUS attributes that will be used internally, like in the authentication rules.

For example if we want to add the attribute TLS-Stripped-UserName by extracting the first portion of the TLS-Client-Cert-Common-Name attribute value:
