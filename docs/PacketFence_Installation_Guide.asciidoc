...
There is some default vlan filters defined in the configuration you can use to achieve the following goal:

.EXAMPLE_Reject_between_11am_2pm will revent a device from connecting when its role is "default", the SSID is "SECURE", the current time is between 11am and 2pm, from Monday to Friday and as a registered device

.EXAMPLE_Trigger_event_if_user will create a security event if the SSID is OPEN and the owner is igmout (the security event needs to have a custom trigger with the value 12345)

.EXAMPLE_Autoregister_if_user will autoregister the device and assign the role staff to each device where the username is igmout.

.EXAMPLE_Autoregister_windows_devices will autoregister all Windows devices and assign them the role default.

.EXAMPLE_Reject_specific_MAC will filter a MAC address and reject it by assigning the role REJECT.

.EXAMPLE_Detect_VOIP will auto set Avaya and Polycom as phones by matching vendor MAC, and set to default role

.EXAMPLE_Reject_User_Unless_Machine will refuse user authentication without prior machine authentication

.EXAMPLE_Autoregister_Printer_Scanner will autoregister printers and scanners and add a note.
...
ldapfilter action override the internal ldap filter packetfence create internally (uid=$username) so you can create a custom filter that match your needs.

Per example something like this (search for the user and check to see if it's permitted based on some criteria):
...
set_role_on_not_found is a way to define a role is the rule doesn't match, let's take the ldapfilter example above.

If we add the action set_role_on_not_found = REJECT so it mean that the device will be rejected if the ldap filter doesn't return anything. (if it match then set_role action will be applied)

role_from_source action
^^^^^^^^^^^^^^^^^^^^^^^

role_from_source will check to see if the ldap attribute exist and will add it in the ldap_attribute context (available in the radius filters)

So per example if you want to take the value of the ldap attribute customRadius and add it in the radius answer you needs to do the following.
...
Note, this support multiples ldap attributes, like customRadius:Airespace-Interface-Name=internet , customRadius:Aruba-User-Vlan=666

Append search attributes ldap filter
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This option will add as a & condition to the ldap filter generated by PacketFence,
So per example the ldapfilter generated by PacketFence will be the following:
...
By doing that, event you wont don't need the "Search Attributes" feature, you will be able to store the user dn in the PacketFence-UserDN attribute.
...
You can define a Realm with a regex in order to match multiples format.

Per example in the ACME realm we define the regex like this:

  .*\.acme\.com$

It mean that if you have a user coming with this username mickey@la.acme.com , FreeRADIUS will define the realm as la.acme.com (it will be included in the radius request) and PacketFence will map the user to the ACME Realm.
...
(merge_return means that the original answer of PacketFence will be merge with the filter answer automatically):
  ...
  [Ethernet-EAP-Accept]
  status=enabled
  description=Access Accept when the connection is Ethernet-EAP and when there is no security event
  scopes=returnRadiusAccessAccept
  radius_status=RLM_MODULE_OK
  merge_answer=no
...
by the value of _portalURL defined in the switch config):
  ...
  [Ethernet-EAP-Accept]
  status=enabled
  description=Access Accept with VSA when the connection is Ethernet-EAP and when there is no security event
  scopes=returnRadiusAccessAccept
  radius_status=RLM_MODULE_OK
  merge_answer=yes
  condition=not_defined(security.id, "") && contains(connection_type, "Ethernet-EAP")
...
Here is an example on filtering by switch IP addresses and using MAC authentication, and then changing session and idle timeouts.
  ...
  [Session-timeout Idle-Timeout and terminate action]
  answer.1=reply:Idle-Timeout = 240
  status=enabled
  answer.0=reply:Session-Timeout = 10620..12600
  top_op=and
  description=Change the session timeout and
  scopes=returnRadiusAccessAccept
  merge_answer=no
...
Fingerbank can also be used to target specific devices as seen below. We can then apply Cisco ACLs to them.
  ...
  top_op=and
  description=Detect ipad
  scopes=returnRadiusAccessAccept
  radius_status=RLM_MODULE_OK
  merge_answer=yes
  condition=contains(fingerbank_info.device_hierarchy_names, "Apple iPad")

preProcess scope can be usefull when you want to create internal radius attributes that will be used internally, like in the authentication rules.

per example we want to add the attribute TLS-Stripped-UserName by extracting the first portion of the TLS-Client-Cert-Common-Name attribute value:
...
This rule will add the TLS-Stripped-UserName radius attribute in the request and you will be able to use it in the authentication/administrations rules.
...
RADIUS EAP Profiles allow you to select a specific eap profile in FreeRADIUS based on the Realm of the user.
...
Per example the Realm ACME.COM needs to use the CA certificate from ACME CA and the other Realm needs to use the default one.
...
Then create the EAP profile in 'Configuration -> System Configuration -> RADIUS -> EAP Profiles' and create a new EAP profile and select the TLS profile created before (PEAP Profile per exemple)
...
Restart radiusd to generate the new RADIUS configuration.
