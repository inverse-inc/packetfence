...
Download the new monitoring directory
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
----
...
Move the old monit directory and replace with the new one
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
----
...
----
[options="compact"]
* `email(s)`: CSV of recipient email addresses (no spaces).
* `subject`: Email subject line (example: 'Packetfence Monit Alert').
* `configurations`: CSV of configurations (example: 'packetfence,active-active,os-checks').
* `mailserver`: SMTP server, use 'localhost' is SMTP relay is not required.
...
patch
^^^^^
This updates only once a week. It is normal to see status `Waiting`.


packetfence-etcd
^^^^^^^^^^^^^^^^

If stuck waiting on `packetfence-etcd`, perform the following on all nodes in the cluster:
----
systemctl stop packetfence-etcd
----

Once packetfence-etcd is stopped on all nodes:
----
