...
image::docs/images/getting-started.png[scaledwidth="65%",alt="Getting Started"]

Going through the configurator
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

First open PacketFence's configurator - you can access it from https://@ip_of_packetfence:1443/configurator

 * Step 1 - Enforcement, choose 'RADIUS enforcement' - this will make PacketFence act as simple RADIUS server.
 * Step 2 - Networks, make sure you define only one interface with the Management type. That network interface will be the one for which the Cisco 2960 access switch will talk to. The management interface of PacketFence and the Cisco 2960 should normally be in the same network.
