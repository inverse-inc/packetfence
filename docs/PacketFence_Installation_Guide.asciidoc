...
EXAMPLE: lets define a rule that prevents a device from connecting when its category is the "default", when the SSID is "SECURE" and when the current time is between 11am and 2pm:
...
EXAMPLE: Refuse User Auth without machine Auth
...
Here is an example on filtering by switch IP addresses and using MAC Auth, and then changing session and idle timeouts.
