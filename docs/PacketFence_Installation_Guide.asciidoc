...
Once you have configured your self service portal policy, you need to assign
it to the appropriate connection profile in _Configuration->Policies and
Access Control->Connection Profiles_.

As long as your self service portal policy is not assigned to a connection profile, users will not be able to
...
By default all users can manage their devices through this self service
portal. You can specify which roles can manage their devices on this page by
configuring a self service portal policy in _Configuration->Advanced Access Configuration->Self Service Portal_.
Then, make sure you assign this policy to the appropriate connection profile
in _Configuration->Policies and Access Control->Connection Profiles_.

By default, this page is accessible in PacketFence managed networks
(registration, isolation, inline) and on any interface which has the `portal`
daemon enabled. You can prevent this page from being served in PacketFence
managed networks by enabling the parameter *Status URI only on production network*
in _Configuration->Advanced Access Configuration->Captive Portal_.

Page will be accessible at https://YOUR_PORTAL_HOSTNAME/status.
...
If you chose to enable 'NTLM cache background job' in one of your domains, you will need to enable the pfcron job that will periodically cache the credentials. This can be configured in 'Configuration->System Configuration->Main Configuration->Maintenance->populate_ntlm_redis_cache'. It is advised to set the interval of this task to half the expiration of the credentials you have set in the domain configuration. This will ensure you have an optimal cache hit. Once done, restart the `pfcron` service.
...
Once enabled, restart the pfdhcplistener and pfcron services.
pfcron will query via SNMP all the switches to create a map (MAC <-> switch)
...
for all networks (not only inline networks) instead of the ones defined in
networks.conf.
