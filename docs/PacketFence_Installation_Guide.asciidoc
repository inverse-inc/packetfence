...
## screenshot user create 3

## screenshot user create 4
...
PacketFence changes to handle Rapid7 syslog information
+++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
* Create the fifo pipe file that PacketFence will use to get data from Rapid7
...
TIP: You can see if the Nexpose server is sending to the right server by monitoring both sides traffic, with a 'tcpdump -i any dst host YOUR_PACKETFENCE_SERVER_IP' on your Rapid7 Nexpose server and 'tcpdump -i any port 514' on the PacketFence server.
