...
For security reasons you may need to encrypt a part or the whole database. Starting from PacketFence 8.1 you can encrypt the database.

First you need to choose between two Key Manager, the first one is the File Key Manager ( https://mariadb.com/kb/en/library/encryption-key-management/#file-key-management-plugin ) and the other one is
the AWS Key Management ( https://mariadb.com/kb/en/library/aws-key-management-encryption-plugin-setup-guide/ ).

Follow the previous link and after that go in Configuration -> System Configuration -> Database -> Encryption and fill the appropriates fields.

Once done restart your database ( systemctl restart packetfence-mariadb ) and connect to it.

If you want to encrypt a table then do :

 ALTER TABLE person ENCRYPTED=YES encryption_key_id=1;
...
[appendix]
Restoring a Percona XtraBackup or MariaDB Backup dump
-----------------------------------------------------

If you enabled Percona XtraBackup or MariaDB Backup for your nightly backup, you can use the following instructions to restore it. In this example we will be restoring `/root/backup/packetfence-db-dump-innobackup-2016-12-20_00h31.xbstream.gz`
