...
Facebook
^^^^^^^^
To use Facebook as an authentication source you will need to setup a Facebook app. Go to: <https://developers.facebook.com/apps>.
...
NOTE: Go to *Configuration -> System Config -> General Configurtation* for the `HOSTNAME` and `DOMAIN` to use throughout the Facebook OAuth configuration (example: *HOSTNAME*=`packetfence.moc` *DOMAIN*=`pf810`, therefore *Site URL*=`https://pf810.packetfence.moc/`).
...
Navigate to *Main Menu -> Settings -> Basic*. Note the *App ID* and *App Secret*. In *App Domains* enter the following URI: [`HOSTNAME`].[`DOMAIN`] (example: *HOSTNAME*=`packetfence.moc` *DOMAIN*=`pf810`, therefore *App Domains*=`pf810.packetfence.moc`).
