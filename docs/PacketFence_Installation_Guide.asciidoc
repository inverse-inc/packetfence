...
First stop `packetfence-etcd` on all nodes in the cluster:

----
systemctl stop packetfence-etcd
----

Once `packetfence-etcd` is stopped on all nodes, remove the contents on the `etcd` directory on all nodes.
----
rm -Rf /usr/local/pf/var/etcd/*
----

Restart `packetfence-etcd` on all nodes in the cluster.
----
systemctl start packetfence-etcd
----
