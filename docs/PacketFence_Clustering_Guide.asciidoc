...
Then restart PacketFence's mariadb on the first server:
  ...
  # systemctl packetfence-mariadb restart
...
Now, reload the configuration and restart packetfence-config. You will see errors related to a cache write issue but you can safely ignore it for now. These appear because packetfence-config cannot connect to the database yet.
  ...
  # systemctl packetfence-config restart
  # /usr/local/pf/bin/pfcmd configreload hard
  # systemctl packetfence-haproxy start
...
Still on the first server, start MariaDB forcing it to create a new cluster. This command will not return until you break it, so leave it running in the background and continue on.

 # systemctl packetfence-mariadb stop
 # /usr/local/pf/bin/pfcmd generatemariadbconfig
 # /usr/local/pf/sbin/pf-mariadb --force-new-cluster

NOTE: If the cluster is unable to get created properly, it may be because the other nodes have their MariaDB service running. Make sure you stop it on all nodes except the first one for now (`systemctl packetfence-mariadb stop` and `systemctl mariadb stop`).
...
Once all servers are synced, go on the first server that should still be running with the `--force-new-cluster` option, break the command and then start packetfence-mariadb normally:
  ...
  # systemctl packetfence-mariadb start
...
Once both nodes have completely synced (try connecting to it using the MySQL command line), then you can break the cluster election command you have running on node C and start node C normally (using `systemctl packetfence-mariadb start`).
