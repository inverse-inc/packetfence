...
Remote server in slave mode
~~~~~~~~~~~~~~~~~~~~~~~~~~~

In cluster layer3 configuration you can configure a remote server in slave mode, it means that the db will be replicated from the central DC and replicate to the remote site.
When all is ok the remote server use directly the db of the main site but in the case the remote server loose the link to the master one then this server will use his local db in read only mode.

In order to make this configuration you need at least 3 servers in the main site and one on a remote site.

Prepare the configuration
+++++++++++++++++++++++++

First you need to configure PacketFence to enable the master/slave mode, to do that go in Configuration -> System Configuration -> Database -> Advanced and set the "Master/Slave mode" to ON.

Next you need to configure your cluster.conf during the cluster setup, refer to the example below to build your cluster.conf.
...
And we defined the type and the enforcement on all the interfaces.

Prepare and start the master/slave replication
++++++++++++++++++++++++++++++++++++++++++++++

First you need to have a backup of the current database with the position of the master in the binary log.
To do that you first need to restart packetfence-mariadb on each servers of the main cluster.

Once done run the /usr/local/pf/backup-and-maintenance.sh script on all 3 servers, at the end you will have a db backup file (packetfence-db-dump-innobackup...) in /root/backup on only one of the servers.
Transfert this file on the remote server in /root/backup.

Next, connect on the remote server and do the following:
...
Last thing to do is to launch the script that will start the slave replication.
To do that go on the remote server and launch:

 /usr/local/pf/addons/makeslave.pm

 Enter the MySQL root password: password
 Enter the MySQL master ip address: 192.168.1.11


Or if prefer to do the procedure to start the slave replication manually:

Edit the file /var/lib/mysql/xtrabackup_binlog_info and note the file name and the position:

 mariadb-bin.000014      7473

Go back on the server where you got the backup file and launch the following command:
...
Return on the remote server and do the following (pfcluster and clusterpf are replication username and password you will find in the pf.conf file and 192.168.1.11 is the server where you fetched the backup):

 SET GLOBAL gtid_slave_pos = '22-2-10459';
 GRANT REPLICATION SLAVE ON *.* TO  'pfcluster'@'%';
 FLUSH PRIVILEGES;
