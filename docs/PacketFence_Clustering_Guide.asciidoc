...
First, you will need to install Percona Xtrabackup for the synchronization to work correctly.
  ...
  # yum install percona-xtrabackup socat
...
First, ensure you now start packetfence-mariadb. Again this can take up to 5 minutes to start since its an initial cluster creation.
...
If one or more of the nodes was gracefully shutdown, this process will not work and you will need to elect one of the nodes manually as the master with `/usr/local/pf/sbin/pf-mariadb --force-new-cluster` while making sure you delete `/var/lib/mysql/gvwstate.dat` on all the servers. Given the different amount of cases that can occur, selecting the node to elect as master is not documented in this guide. You should ensure you are electing the node with the latest data.
...
If the management node is not part of the alive nodes, select one of the servers of your cluster and create a new cluster using `/usr/local/pf/sbin/pf-mariadb --force-new-cluster`. Wait for another server to connect to this new cluster, break the command and start packetfence-mariadb like you normally would.
