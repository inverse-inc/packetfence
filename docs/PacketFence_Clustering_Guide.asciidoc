...
If you use the `/usr/local/pf/addons/database-backup-and-maintenance.sh` script, this will now enabled the Percona Xtrabackup mode. In order for it to work correctly, you should configure your installation as described in section '15.3.6. Using Percona XtraBackup' of the Administration guide.
...
NOTE: If the cluster is unable to get created properly, it may be because the other nodes have their MariaDB service running. Make sure you stop it on all nodes except the first one for now (`systemctl stop packetfence-mariadb`).
...
Now, flush any MySQL data you have on this node and restart packetfence-mariadb so that the servers join the cluster.
