...
After the installiation, you need to create the extra partition for DRBD. Using fdisk, create your new partition and save the table. You will probably need to reboot your server after this step.
...
CAUTION: Initializing, configuring and troubleshooting DRBD is not straight forward! We strongly recommend that you read the online documentation available ifrom the DRBD website so you have a better idea about how it works.
...
NOTE: If you are using CentOS 7, you need to make sure the interfaces name will be the same on both servers. See 'Setting the interfaces name on CentOS 7' in the Appendix section of this document.
...
^^^^^^^^^^^^^^^^^^^
 ...
 node pf1_server_name \
         attributes standby=off
 node pf2_server_name \
         attributes standby=off
 ...
 primitive DRBD_fs Filesystem \
         ...
         op stop interval=0 timeout=180s \
         op monitor interval=120s timeout=40s
 primitive MySQL lsb:mariadb \
         op start interval=0 timeout=180s \
         op stop interval=0 timeout=180s \
         op monitor interval=120s on-fail=restart timeout=40s
 group data_group DRBD_fs MySQL \
         meta target-role=Started is-managed=true
 ms DRBD_ms DRBD \
         meta master-max=1 master-node-max=1 clone-max=2 clone-node-max=1 notify=true target-role=Started is-managed=true
 location DRBD_ms-master-on-pf1_server_name data_group \
         rule $role=master #uname eq pf1_server_name
 colocation fs_on_drbd inf: DRBD_fs DRBD_ms:Master
 order DRBD_fs-after-DRBD_ms inf: DRBD_ms:promote DRBD_fs:start
