...
Configure packetfence-config
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Now that your cluster is configured, you should point packetfence-config to use 127.0.0.1 as the database IP so it uses haproxy to find the database server.

On the first, change the following `conf/pfconfig.conf` :
...
Then, sync your cluster:

  # /usr/local/pf/bin/cluster/sync --as-master

And restart packetfence-config on all of your servers:
