...
Once both nodes have completely synced, you should be able to connect to it using the MariaDB command line:
...
Then you can break the cluster election command you have running on node C and
start node C normally (using `systemctl start packetfence-mariadb`).
