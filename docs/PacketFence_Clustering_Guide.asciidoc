...
CAUTION: This user should *NOT* have a password that contains spaces
...
NOTE: If the cluster is unable to get created properly, it may be because the other nodes have their MariaDB service running. Make sure you stop it on all nodes except the first one for now (`systemctl stop packetfence-mariadb` and `systemctl stop mariadb`).
...
NOTE: If you reboot any of the nodes you're joining, you will need to stop all the PacketFence services (/usr/local/pf/bin/pfcmd service pf stop) and restart the steps from here.

NOTE: If you reboot the management node, you will need to stop packetfence-mariadb (systemctl stop packetfence-mariadb) and start it with the new cluster option so the servers can join (/usr/local/pf/sbin/pf-mariadb --force-new-cluster)
...
Shutting down a PacketFence Active/Active cluster of tree nodes
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In order to shutdown the cluster in a proper way, here is the steps you need to follow.
We are here assuming that we are on a cluster of tree nodes, on Centos7.

We will shutdown the nodes, one by one, starting by one of the "Slaves" and end with the "Master".

Find the Master Server: ::
* Type *ip a* on each nodes, and look for the one which hold the VIP (the /32 address).

WARNING: *YOU TAKE THE RISK OF LOOSING SOME DATA IF YOU DO NOT IDENTIFY MASTER AND SLAVES NODES* 
 
Shutdown the "Slaves": ::
* Logon with an ssh terminal on the master and make a permanent ping on both slave.
* On the first slave, execute: *shutdown -h now*.
* Once the first slave down, do the same with the second slave.

Shutdown the "Master": ::
* On the master, execute: *shutdown -h now*.

Bringing up a PacketFence Active/Active cluster of tree nodes
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We want to bring up the cluster, as the same status it was before the shutdown, assuming we want to have the Master and the Slaves well synchronised.

Therefore, we will do the _Shutting down_ procedure, but in reverse.

We will start the nodes, from the Master to the Slaves.

Bring up the "Old Master"
^^^^^^^^^^^^^^^^^^^^^^^^^
...
The *Master* must now be ready.


Bring up the "Slaves"
^^^^^^^^^^^^^^^^^^^^^

Start the first slave, logon to the ssh terminal.

Check the PacketFence-Mariadb sync with the Master, type the command: ::
----
mysql -u root -p
mysql> show status like 'wsrep%';
----
...
The PacketFence servers have a daily backup done, each nights (0:30AM).

If you need to extrenalise those backups, they are in:

----
/root/backup
----
Files description: ::
* packetfence-db-dump-DATE_00h30.sql.gz are the sql dump of your Mariadb database.
...
As the daily automatic backup, you will find the files in:

----
/root/backup/
----

Two files will be available, tagged with the Date and Hour of your backup.
