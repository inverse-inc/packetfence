...
On second and third servers, make sure that 'iptables' is stopped:
...
Make sure that each server is binding to it's own management address. If it's not, verify the `/usr/local/pf/conf/cluster.conf` management interface configuration.
