...
And restart packetfence-mariadb so it runs without being in the cluster
...
Once one of the node has completed synced (try connecting to it using the MySQL command line), then you can break the cluster election command you have running on node C and start node C normally (using `service packetfence-mariadb start`).
