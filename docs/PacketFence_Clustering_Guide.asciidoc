...
|===
|In `/ect/sysconfig/network-scripts/`| 
|One Management Interface|ifcfg-*YourFirstInterfeceName*

|One Secondary Interface|ifcfg-*YourSecondInterfeceName*

|One Registration Interface|ifcfg-*YourSecondInterfeceName*.*YourRegistrationVLANID*

|One Isolation Interface|ifcfg-*YourSecondInterfeceName*.*YourIsolationVLANID*
|===
...
WARNING: This last command will not return until you break it, so leave it running in the background and continue on.
...
NOTE: You can check with 'íp a', on the first serveur, you need to find the *VIp* on the first ethernet interface. On the otehrs server, be sure to have the 'interface.VLANID' interfaces with the good IPs.
...
WARNING: *fence* is only for example purpose, you need to define your owne password.
...
NOTE: You can check if the service is down with 'ps -edf | grep mysql', this service can be a little long to stop and it is not recommended to do the nexts step before it.
