...
* General 802.1x/MAC-Auth configuration
...
* Finally for voice LLDP-MED do the following in the global config:
  ...
  lldp med network-policy application voice tagged vlan 200 priority 5 dscp 46 ports e1/1/1 to 1/1/48
...
* In the event you're using DHCP helpers at the switcht:

    ip bootp-use-intf-ip

[NOTE]

  Never tag any VLAN onto any PacketFence (802.1x/MAC-Auth) enabled
