...
* Set the 'Registration' role to 'registration' (If undefine then it will use the role name)
* Set Role by Url for registration to 'http://<your_captive_portal_ip>/$session_id'
* The Url can contain dynamic parameters, like the mac address ($mac)
...
For IOS 12.2 create this acl and assign it to the switch port interface:
...
It's possible to mix web authentication and downloadable ACLs starting from the IOS version 12.2, per example if you want to be able to redirect the device to the captive portal when a violation is triggered.
