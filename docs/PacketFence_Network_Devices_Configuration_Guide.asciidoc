...
N1500 Series (FW >= 6.6.0.2)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This configuration has been tested with firmware 6.6.0.2
...
802.1X interface configuration:
  ...
  switchport voice detect auto
  switchport mode general
  switchport general pvid 135
  switchport general allowed vlan add 2,135-136,999
  authentication host-mode multi-domain
  authentication periodic
  dot1x timeout quiet-period 10
  mab auth-type pap
...
First, you must configure the guest policy. Go in _Settings->Guest Control_ and configure it as shown below:
