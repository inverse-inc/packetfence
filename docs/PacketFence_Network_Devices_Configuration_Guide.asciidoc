...
Add the authentication for the Captive Connection Profile via `Security -> Authentication -> L3 Authentication -> Captive Portal Authentication Profile -> Server Group`:
...
Now add the newly created policy to the Captive Connection Profile, `Security -> User Roles`:
