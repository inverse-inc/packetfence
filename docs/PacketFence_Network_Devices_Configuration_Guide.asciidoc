...
PacketFence support Cisco ASA VPN with anyconnect.

You can force the VPN user to authenticate first on the captive portal and based on the role of the device allow it and/or set set dynamic acl.

In this example we assume that the Cisco ASA have 2 interfaces, one Management (192.168.2.1) where the VPN is activated and another one Registration (192.168.1.6) who is facing the PacketFence server (192.168.1.5).
