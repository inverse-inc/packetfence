...
# screenshot port-control 802.1x
...
If you are using MAC authentication on this switch, tyou must adjust the FreeRADIUS configuration so it transforms the EAP requests this switch sends into requests that PacketFence will interpret as MAC authentication. This is necessary because the 'MAC Based' mode of the switch still performs EAP requests.

To adjust it, go in `/usr/local/pf/conf/radiusd/packetfence` and uncomment the following line:

  packetfence-eap-mac-to-mab-policy
