...
CAUTION: There is currently no way to reevaluate the access of the device using PacketFence. This means that there is no real-time action when trying to isolate a device. The RADIUS reauthentication delay will determine the maximum response time of the access reevaluation. See below for it's configuration.

First create a new SSID and then edit it's settings.

Configure it as seen in the screenshot below.

screenshot top

NOTE: Make sure to configure your RADIUS server so it's accessible externally under a domain name or a public IP address. Replace `pf.domain.net` by what you will configure. Refer to the Meraki documentation for more details.
