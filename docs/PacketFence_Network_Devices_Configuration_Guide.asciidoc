...
PacketFence supports Cisco NEAT through EAP-MD5 and EAP-FAST authentication methods. This means that Cisco switches can be authenticated through RADIUS and as a result, have the port it is connected to be reconfigured dynamically to receive trunk access.
...
More specifically, here is what is required to be configured on authenticator and supplicant switches to support EAP-FAST authentication against PacketProtector.
