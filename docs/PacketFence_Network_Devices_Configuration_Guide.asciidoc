...
In order to configure VLAN enforcement on the Unifi controller, you need first to configure a RADIUS profile, then next a secure Wireless network

Important : You cannot re-use a VLAN ID for dynamic VLAN if it is set as a static value for another SSID on the same AP. So, if you have an SSID set to use VLAN 10, you cannot use VLAN ID 10 for RADIUS controlled VLAN users as those users will not get an IP.
...
There is a particularity when you want to be able to deauthenticate a device when it is connected on the secure SSID.

By default pmksa caching is enabled and applies even when a deauthentication request is sent to the controller, meaning you will not get a new RADIUS request afterwards
...
In this case the uri is https://192.168.1.6:8443/manage/site/4j4ee7x4/ so the site id is 4j4ee7x4
...
1 and 4 are the profile id configured on the access point, to be sure to have the correct id, you can connect on the access point via ssh or via the debug terminal and do:
...
Complicated isn't it !, feel free to ask the support directly in the administration interface there: https://community.ubnt.com/t5/UniFi-Wireless/Feature-request-disable-pmksa-caching/m-p/2112479#M257628
