...
In order to use PacketFence as an external captive portal for web authentication, you will to configure first your radius authentication and accounting server (see steps above).

[float]
Hotspot configuration
+++++++++++++++++++++

Then you will need to configure the Hotspot service profile to redirect devices to your PacketFence portal. Go on the ZoneDirector administration web page to the section *Configure->Hotspot Services->Create New*

image::docs/images/ruckus_hotspot_service.png[scaledwidth="100%",alt="Hotsport Service"]

 1 - Name of your Hotspot service
 2 - Login Page: Url of Packetfence management interface (http://192.168.1.5)
 3 - Start Page: redirect to the follwing URL: http://192.168.1.5
...
image::docs/images/ruckus_create_ssid.png[scaledwidth="100%",alt="Hotsport Service"]
 ...
 6 - Access VLAN: The VLAN ID where the devices should be after authentication
...
On the ZoneDirector configuration in PacketFence, you will need to speficy -1 as the registration vlan in order to display the captive portal to the end device. 

You will need to desactivate the force secure redirect on the captive portal under *Configuration->Captive Portal->Secure redirect->Unchecked*

The captive portal needs to listen on the management interface, so you will need to add 'portal' to the interface type of the management interface in /usr/local/pf/conf/pf.conf
...
To apply the configuration, restart packetfence with the command: service packetfence restart
