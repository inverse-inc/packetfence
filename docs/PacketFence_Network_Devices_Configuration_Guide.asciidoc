...
Those switches are supported using 802.1X for networks with or without VoIP. Tested on Firmware Version 08.0.60.
...
* Global config settings:
  ...
  radius coa host 192.168.1.5 key useStrongerSecret

* VLAN configuration (create all the VLANs necessary but only tag ports for VOIP VLAN, also note you must tag at least one port in each VLAN, the assumption is that the uplink interface is port 1/1/48, adjust as necessary):

  vlan 1 name Access by port
   tag ethe 1/1/48
  vlan 2 name Registration by port
   tag ethe 1/1/48
  vlan 3 name Isolation by port
   tag ethe 1/1/48
  vlan 100 name Voice by port
   tag ethe 1/1/1 to 1/1/48

802.1X/MAC Authentication
+++++++++++++++++++++++++

* General 802.1X/MAC Authentication configuration
  ...
  dot1x timeout supplicant 30
  dot1x enable ethe 1/1/1 to 1/1/47
  mac-authentication enable
  mac-auth enable ethe 1/1/1 to 1/1/47
  reauth-period 259200
  re-authentication

* Run all discovery protocols:
...
* Interfaces configuration:
  ...
  int e1/1/1 to 1/1/47
   dot1x port-control auto
   voice-vlan 100

* Finally for voice LLDP-MED do the following in the global config:

  lldp med network-policy application voice tagged vlan 100 priority 5 dscp 46 ports e1/1/1 to 1/1/48

* For devices in routing mode (Layer 3) the following applies when VLAN 1 is your management VLAN:

    ip radius source-interface ve 1

* In the event you're using DHCP helpers at the switch:

    ip bootp-use-intf-ip

[NOTE]

  Never tag any VLAN onto any PacketFence (802.1X/MAC Authentication) enabled
  ports excepting the voice VLAN.
