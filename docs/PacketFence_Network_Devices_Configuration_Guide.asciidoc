...
To configure the Aruba in webauth there will be few steps. 

First of all you will need to configure a guest VLAN.

image::docs/images/aruba-config-guest-vlan.png[scaledwidth="100%",alt="Guest VLAN"]

Now you will need to configure a RADIUS server.
...
Next you will need to create the policy that will redirect user to the PacketFence captive-portal while not authenticated. Go to `Security > Authentication > L3 Authenticaion > Captive Portal Authentication Profile`.
...
Now create the policy for the guest access, for instance internet only.

You can add as many policies as you want, manage the way you attribute them from PacketFence.
...
image::docs/images/aruba-config-aaa-profile1.png[scaledwidth="100%",alt="AAA profile 1"]
...
image::docs/images/aruba-ap-config1.png[scaledwidth="100%",alt="Config AP 1"]
...
The next step will be to configure the Webauth in PacketFence, add the switch with the model choice `Aruba Network`, 

Tick the case `External Portal Enforcement`, in the Roles section, chosse `Role by Switch Role`, as the registration role, enter your default role: `packetfence-portal` and choose the policy matching roles, for instance guest: `internet-only`.
...
Once you have configured the Meraki, you will need to configure it in the PacketFence switches. You will need to create an entry for the public IP address of your cloud controller and one for the MAC address of your AP. Here is the resulting configuration for switches.conf.
  ...
  [00:11:22:33:44:55]
  description=Meraki local access point
  type=Meraki::MR
  radiusSecret=useStrongerSecret
  mode=production
