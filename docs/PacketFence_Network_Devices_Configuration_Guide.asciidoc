...
Add the authentication for the Connection Profile via `Security -> Authentication -> L3 Authentication -> Captive Portal Authentication Profile -> Server Group`:
...
Now add the newly created policy to the Connection Profile, `Security -> User Roles`:
