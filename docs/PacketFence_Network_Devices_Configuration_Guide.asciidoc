...
Next, in PacketFence, enable 'Role by Switch Role' your the switch configuration and assign the policies to the roles there. They will be returned inside the Filter-Id attribute.
...
Web-auth profile
  ...
  configure dns-client add domain-suffix extr.local
...
Then once done, restart pfmon service and run that to fill the PacketFence cache:
