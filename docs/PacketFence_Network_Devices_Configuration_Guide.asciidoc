...
[NOTE]
.dot1x on various models of 2960
============================================================================
...
We have found couple of commands that are working great or MAB:
...
The principe is to forward the device to the captive portal with an ACL if the device is in an unreg state and 
allow the device to reach internet by changing the ACL.
In the unreg state, the WLC will intercept the http traffic and forward the device to the captive portal.

In this sample configuration, the captive Portal use the ip address 172.16.0.250, the administration interface
use the ip address 172.16.0.249 and the WLC use the ip address 172.16.0.248.
The DHCP and DNS server are not managed by PacketFence (WLC DHCP Server, Production DHCP Server)
...
* Then you have to create two ACL, one to deny all except the minimum to hit the portal (Pre-Auth-For-WebRedirect ) and the other
one to allow any (Authorize_any) .
