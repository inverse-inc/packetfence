...
In version 6 or later of the HiveOS, we do return VLAN ID matching the number that the *User Profile* have. Create your *User Profile* in the HiveManager as usual, assign the matching VLAN, and in PacketFence configuration add the wanted VLAN ID in the section *Roles by VLAN*.
