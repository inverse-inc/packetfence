...
These ACLs are defined per node category like the VLANs which means you can define different ACLs for your registration VLAN, production VLAN, guest VLAN, etc.
...
This does not cover the configuration of the controller.
  ...
  service {
      anyfi {
          gateway anyfi-hotspot {
              ...
              bridge br0
              controller <the controller's ip address>
              isolation
              nas {
                  identifier anyfi
                  port 3799
...
Note that for the moment, the Anyfi gateway doesn't support RADIUS assigned VLANs when using 802.1X. The devices will be assigned the native VLAN on eth1.
PacketFence will only do the authentication of the 802.1X credentials and accept or deny the user.
Since the 802.1X users cannot be assigned a VLAN, if you want portal based authentication on top of the 802.1X authentication, you will need to use the inline mode on the network your devices will be in.
