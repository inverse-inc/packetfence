...
To use this feature, first configure RADIUS and the authentication method on your switch. Next, in the PacketFence web admin interface, go to the 'Configuration' page, and then 'Switches'. Click on the switch you want, then on the 'Roles' tab, and check 'Role by access list'. Now you are able to add ACLs for each role.
...
This command configures MAC authentication on the switch and activates this feature on the specified ports.
...
Now, for example, your normal users are placed in the 'default' category and your guests in the 'guest' category.

The 'default' category uses the network 192.168.5.0/24 and 'guest' uses the network 192.168.10.0/24.
