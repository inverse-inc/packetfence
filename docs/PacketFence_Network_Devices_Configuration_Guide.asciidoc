...
Starting from version 15.2(1)E (IOS) and 3.4.0E (IOSXE) , Cisco introduce the Identity Based Networking Services.
It mean that you can create a authentication workflow on the switch and create interfaces templates.
...
If the radius server goes up then it reinitialize the authentication if the port is in
IN_CRITICAL_VLAN.
