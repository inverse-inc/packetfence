...
Before you start the configuration, a hotfix is necessary due to a Microsoft issue. After restarting the ADCS service, the server cannot enroll new certificate and display the following error message: "The RPC Server is unavailable". The hotfix is available here: https://support.microsoft.com/en-us/kb/2633200
...
For the deployment of ADCS you will need to configure Network Enrollment Device Service (NDES). This subrole will allow us to exchange certificate with the MSPKI server via Simple Certificate Exchange Protocol (SCEP).
...
Using the PKI of Microsoft include that all your certificates will be deliver by the root CA of MSPKI. The public key of this certificate is necessary on PacketFence. 
...
In the output search for `X509v3 extensions:`. When using the scep enroll command you will need the "CEP Enryption" certificate as an arugment for -e and the "Enrollment agent" certificate as an argument for -c. -d is use for the debug output. -l is the local file where your certificate will be save.
