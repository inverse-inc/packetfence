...
PacketFence provides repositories for **RHEL 8** and **Debian 12** (bookworm) with all required dependencies.
...
echo "deb [signed-by=/etc/apt/keyrings/packetfence.gpg] http://inverse.ca/downloads/PacketFence/debian/{release_minor} bookworm bookworm" > /etc/apt/sources.list.d/packetfence.list
