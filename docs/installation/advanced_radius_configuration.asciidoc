...
In this section we will explain how to proxy a RADIUS request based on advanced criteria.

First you have to create RADIUS authentication source like above and create per example 2 REALMs "to_NPS" and "to_ISE" (Associate the RADIUS sources in the REALMs)
...
At this step restart the radius-auth service.

Here some examples of what you can do with the RADIUS filters (the content of the radius_filters.conf file):

Proxy to the to_NPS Realm the RADIUS request if the Calling-Station-Id or Colubris-AVPair attribute match the regex ACME$
...
Proxy to the to_ISE Realm the RADIUS request if the Calling-Station-Id or Colubris-AVPair attribute contains ACME_Admin$ and add the Realm to_ISE in the RADIUS request (can be per example use as a filter in a connection profile)
...
Proxy to the NULL Realm if the Calling-Station-Id or Colubris-AVPair attribute match the regex Guest$
...
Proxy to the to_ISE Realm the RADIUS request if the Calling-Station-Id attribute match the regex ACME$ 
...
CAUTION: Those examples can be added in /usr/local/pf/conf/radius_filters.conf and run /usr/local/pf/bin/pfcmd configreload hard
