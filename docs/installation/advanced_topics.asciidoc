...
`/usr/local/pf/conf/pf.conf` contains PacketFence general configuration,
including VLAN isolation mode settings.

Default parameters and descriptions are in
`/usr/local/pf/conf/pf.conf.defaults`.

Override default parameters by defining them in `/usr/local/pf/conf/pf.conf`.
...
Define reports in `report.conf` creating SQL queries to view PacketFence
database tables. Reports appear under _Reports_ menu in admin interface.

PacketFence includes preloaded reports optimized for common production use-cases
in `reports.conf.defaults`. Don't modify this file; use for working examples.
...
Configure reports by editing `/usr/local/pf/conf/report.conf` and adding a
report section. Run `/usr/local/pf/bin/pfcmd configreload hard`.
