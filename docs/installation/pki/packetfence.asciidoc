...
Optionally if you want the CA to be a sub CA of another PKI then you can click on 'Generate CSR' (top right of the CA form), change the information if one of them needs to be modify and click 'Generate CSR'.
Copy the CSR and provide it to the external CA to be able to retrieve the certificate signed.
...
You could also enable 'SCEP Server Enabled' and define on which external SCEP server you want to proxy the request.
