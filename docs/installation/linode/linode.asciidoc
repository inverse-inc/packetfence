...
First, you need to create three Linodes in the same region. Dedicate 16GB of memory per instance and make sure Private IP are enabled for each instance. Once done, make sure to configure the firewall policy similar to the following screenshot:
