...
This section has been created to give a quick overview to configure MFA integration with PacketFence.
...
You have to create a rule with a condition like "memberof equals cn=otp_user,dc=acme,dc=com" and assign a Action:
...
Once the user authenticate, he receive a push notification on his phone and have to validate it in order to grant the access.
...
Once the user receive the code by sms he will need to reauthenticate with his username and password and append the sms code. (like password,smscode)
...
In this user scenario the VPN client present a login page with one username, password and a second password field.
...
Once the user receive the code by sms he will need to reauthenticate with his username and password and set the code received by sms in the 2nd password field.
...
Once the user receive the code by phone call he will need to reauthenticate with his username and password and set the code received by phone in the 2nd password field.
