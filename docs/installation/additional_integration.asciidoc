...
The DHCP remote sensor consists of a lightweight binary installed on the
production DHCP server to replicate all DHCP traffic (1-to-1) to the PacketFence
server. This solution is more reliable than DHCP relaying since PacketFence
receives a copy of all the DHCP traffic including broadcast traffic. Supported
DHCP servers include Microsoft DHCP server and CentOS 6 and 7.

These sensors capture low-level packets on the DHCP server and forwards them to
the PacketFence management interface.

==== Microsoft Remote Sensor

The PacketFence-Forwarder is an optimized version of the udp-reflector, which
installs easily and only forwards DHCPREQUESTS and DHCPACK packets from the
source to the destination as well optionally mirroring DNS traffic for
integration with the Fingerbank Collector

Download the
https://inverse.ca/downloads/PacketFence/windows-packetfence-forwarder/PacketFence-Forwarder-Installer.exe[DHCP Forwarder installer].

This installs `nPCAP`, `nssm`, launches a configurator for the interface, IP and
port, saves the configuration, and finally installs and launches the
DHCP-Forwarder service.
...
[options="compact"]
* where `pcap0` is the pcap interface where the DHCP server listens on.
  (List them using `udp_reflector -l`)
...
Create the script `unreg_node_deleted_account.ps1` on the Windows Server with
...
NOTE: Change `@IP_PACKETFENCE` to the IP address of the PacketFence server
and change the `$username` and `$password` so they match the credentials
defined in the Web admin interface under
_Configuration -> Integration -> Web Services_.
...
With this filter in place, LDAP search filters generated for authentication lookups, such as username-based authentication and matching, will exclude disabled accounts and help prevent them from authenticating.

NOTE: This is a *preventive* approach that blocks authentication at LDAP search time. It applies to LDAP filters generated through the authentication source's append-search-attributes mechanism, rather than globally to every search path supported by the source. To apply the check at the level of individual authentication rules instead, see the next section.

===== Rejecting Disabled Accounts in Authentication Rules
...
In Active Directory, bit 2 (`0x2`) of `userAccountControl` corresponds to `ACCOUNTDISABLE`: when this bit is set, the account is disabled. A rule that matches disabled accounts and assigns the `REJECT` role is therefore the correct way to block them.

To create a rule that rejects disabled accounts:

. Go to _Configuration -> Policies and Access Control -> Authentication Sources_.
. Edit your Active Directory authentication source and create a new authentication rule (place it *above* your other rules so it is evaluated first).
. Add a condition: `userAccountControl` -> *has bit* -> `2`.
. In the rule actions, set _Role_ to `REJECT` and set an _Access duration_ (for example, `1 hour`).
. Save the rule.

This rule matches any account that has the `ACCOUNTDISABLE` bit set (bit 2), regardless of what other flags are present in `userAccountControl`, and rejects the authentication.
...
PacketFence currently supports Cisco switches which must be configured using the following guide: https://www.cisco.com/c/en/us/support/docs/security-vpn/remote-authentication-dial-user-service-radius/116291-configure-freeradius-00.html. From the PacketFence admin interface, configure an Admin Access role (_Configuration -> System Configuration -> Admin Access_) that contains the action 'Switches CLI - Read' or 'Switches CLI - Write' and assign this role to an internal user or with an Administration rule in an internal source.
...
Digitally signed scripts are included in the monit configuration which are fetched from https://inverse.ca/downloads/PacketFence/monitoring-scripts/v1/. These scripts will be updated and run at regular intervals to ensure the environment follows the best practices defined by Inverse and to email alerts of any important changes that may need to be performed.
