...
Go to "Configuration->Integration->Multi-Factor Authentication", click new MFA, select Akamai.
...
If MFA authentication fails, verify configuration and check <<#_radius_audit_log,RADIUS Audit Log>> to trace authentication flow.
