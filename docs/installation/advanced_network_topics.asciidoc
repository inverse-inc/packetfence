...
Provide the routed networks to PacketFence through the admin interface in _Configuration -> Network Configuration -> Networks_ or manually in [filename]`/usr/local/pf/conf/networks.conf`.
