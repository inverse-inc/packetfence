...
- Complete the following using your own information, Common Name must match the captive portal hostname in _Configuration -> General Configuration -> Hostname
...
- The Subject Alternative Name must exactly match the captive portal hostname in _Configuration -> General Configuration -> Hostname_ .
...
[source, shell]
----
openssl pkcs12 -in $PATH/YOUR_CA.p12 -clcerts -nokeys -out /usr/local/pf/conf/ssl/server.crt -passin pass:secret
openssl pkcs12 -in $PATH/YOUR_CA.p12 -nocerts -nodes -out /usr/local/pf/conf/ssl/server.key -passin pass:secret
----
...
- Complete the following using your own information, Common Name must match the captive portal hostname in _Configuration -> General Configuration -> Hostname_
