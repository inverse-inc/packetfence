...
instead of the URL the user originally intended to visit. For these browsers,
...
Registration and Isolation where DNS is black-holed. It is recommended to allow
users to reach the PacketFence server with the PacketFence LAN IP.
...
Connection profiles should be managed from the admin interface - from the _Configuration -> Policies and Access Control -> Connection Profiles_ section. Adding a new connection profile will make a copy of the default templates - which can then be modified as desired.
 ...
 * *SSID:* Guest-SSID
 * *VLAN:* 100
 * *Time period:* wd {Mon Tue} hr {1pm-3pm} -- See https://metacpan.org/pod/release/PRYAN/Period-1.20/Period.pm
...
Thus in the case of username mickey@le.zammitcorp.com, the realm is defined as _le.zammitcorp.com_ - which is included in the RADIUS request - and the user is mapped with the _ZAMMITCORP_ realm.
