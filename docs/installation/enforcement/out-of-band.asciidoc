...
Finally, from _Status->Services_, restart the `haproxy-portal`, `pfdhcp`, `pfdhcplistener`, `pfdns` services.


=== Network Devices

Now let's modify our switch configuration to enable our new registration and isolation VLANs. In the admin interface, go to _Configuration -> Policies and Access Control -> Network Devices -> Switches_, and click on our Cisco 2960 switch we added earlier (172.21.2.3).
...
In the admin interface, go to _Configuration -> Policies and Access Control -> Connection Profiles_, and click on 'Add Profile'. Provide the following information:
...
For VLAN assignment issues, verify switch configuration and trace RADIUS attributes as described in <<#_radius_debugging,RADIUS Debugging>> and <<#_network_connectivity_issues,Network Connectivity Issues>> in the Troubleshooting section.
