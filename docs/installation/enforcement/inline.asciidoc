...
In the admin interface, go to _Configuration -> Network Configuration -> Interfaces_, and click on the `ens192` logical name. Provide the following information:
...
For inline enforcement issues, check iptables rules and logs as described in <<#_log_files,Log Files>> and <<#_network_connectivity_issues,Network Connectivity Issues>> in the Troubleshooting section.
