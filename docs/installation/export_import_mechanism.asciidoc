...
restore, remove it if your backups were previously using mysqldump (default option):

NOTE: To know if your previous deployment used mysqldump or mariabackup, check the content of the archive (displayed during the import), if the file contains a file matching the pattern packetfence-db-dump-*.sql.gz, your backups were using mysqldump and you need to remove MariaDB-backup as described below.
