...
** [filename]`/usr/local/pf/conf/iptables.conf.tt` (but [filename]`/usr/local/pf/conf/iptables-custom.conf.inc` and [filename]`/usr/local/pf/conf/ip6tables-custom.conf.inc` are restored)
...
If all goes well, you can restart services using https://www.packetfence.org/doc/PacketFence_Upgrade_Guide.html#restart_packetfence_services[following instructions].

===== Additional steps to build or rebuild a cluster

If you want to build or rebuild a cluster, you need to follow instructions in https://www.packetfence.org/doc/PacketFence_Clustering_Guide.html#cluster_setup[Cluster setup section].
