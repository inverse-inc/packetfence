...
^^^^^^^^^^^^^^^^^^^^^^^^^
For the scope of this guide, the following network setup will be configured. Please note that a more complex setup can easily be used but will not be covered in this guide.
...
This guide assume that CoovaChilli is installed on the access point. If it's not, we suggest you Google around and make sure to find the appropriate way to do so since there's just to much vendor and network equipment that supports CoovaChilli to document this step here.

The guide also assume that you have an SSID configured on the access point.
...
Also note that manipulation on the OpenWRT access point are done using SSH shell access.

Please note that any interface name reference might be different from one hardware to another.
 ...
 * There is a DHCP server running on the LAN which can potentially wreak havoc

The followings needs to be done:
...
Configure chilli
++++++++++++++++
chilli configuration might differ from one hardware to another. Just make sure to follow theses configuration guidelines and you should be all-set.
...
* Edit the following parameters to integrate with PacketFence
    ...
    HS_UAMFORMAT    http://PACKETFENCE_MANAGEMENT_IP
    HS_SSID         SSID name
...
A startup script maybe required depending on the hardware. Again, a quick search on Google might be the best solution to find the best one
...
To do so, login to the PacketFence web administration interface if it is not already done. Click on the 'Configuration' tab and select the 'Switches' menu option under the 'NETWORK' section on the left hand side.
