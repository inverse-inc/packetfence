...
configure_and_check() {
    ...
    ANSIBLE_EL_GROUP=${ANSIBLE_EL_GROUP:-common_el}
    ANSIBLE_EL8_GROUP=${ANSIBLE_EL8_GROUP:-devel_el8}
    ...
    ACTIVE_BUILDS=${ACTIVE_BUILDS:-'pfbuild-el-8,pfbuild-bullseye'}
    PARALLEL=${PARALLEL:-2}
    PACKER_TEMPLATE=${PACKER_TEMPLATE:-pfbuild.json}

    declare -p GOVERSION PF_MINOR_RELEASE
    declare -p REGISTRY REGISTRY_USER 
    declare -p ANSIBLE_FORCE_COLOR ANSIBLE_EL_GROUP
    declare -p ANSIBLE_EL8_GROUP ANSIBLE_DEBIAN_GROUP ANSIBLE_RUBYGEMS_GROUP
    ...
    export ANSIBLE_FORCE_COLOR ANSIBLE_EL_GROUP ANSIBLE_EL7_GROUP
    export ANSIBLE_EL8_GROUP ANSIBLE_DEBIAN_GROUP ANSIBLE_RUBYGEMS_GROUP
}
