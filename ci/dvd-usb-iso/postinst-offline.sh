...
#
# This script runs inside the system the installer just laid down, so take the
# codename from it rather than hardcoding one. The on-ISO repo was generated by
# create-local-repo.sh under dists/${DEBIAN_CODENAME}, and the base DVD is that
# same release, so the two agree by construction.
. /etc/os-release
cat > /etc/apt/sources.list.d/packetfence-local.list << EOF
deb [trusted=yes] file:///media/cdrom/pf-repo ${VERSION_CODENAME} main
EOF
...
deb [signed-by=/etc/apt/keyrings/packetfence.gpg] https://inverse.ca/downloads/PacketFence/debian-branches/${PF_RELEASE_VERSION} ${VERSION_CODENAME} ${VERSION_CODENAME}
EOF
