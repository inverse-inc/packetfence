...
packetfence_upgrade_deploy() {
    ...
    for release_name in $(ls $RPM_RESULT_DIR); do
        ...
        dst_repo="$PUBLIC_REPO_BASE_DIR/RHEL$release_name"
        dst_dir="$DEPLOY_USER@$DEPLOY_HOST:$dst_repo"
        pf_export_rpm_file=$(basename $(ls $src_dir/packetfence-upgrade*))
        pf_export_rpm_dest_name=${PF_EXPORT_RPM_DEST_NAME:-"packetfence-upgrade-${PF_MINOR_RELEASE}.el${release_name}.noarch.rpm"}
    ...
    for release_name in $(ls $DEB_RESULT_DIR); do
        ...
        dst_dir="$DEPLOY_USER@$DEPLOY_HOST:$dst_repo"
        pf_export_deb_file=$(basename $(ls $src_dir/packetfence-upgrade*))
        pf_export_deb_dest_name=${PF_EXPORT_DEB_DEST_NAME:-"packetfence-upgrade_${PF_MINOR_RELEASE}.deb"}
