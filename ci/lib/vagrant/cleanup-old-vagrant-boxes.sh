#!/bin/bash
# Sweep prebaked/ on Linode before each upload, dropping pipeline bakes
# older than MAX_AGE_DAYS regardless of ref.
...
configure_and_check() {
    ...
    REMOTE_PREBAKED="s3:${BUCKET}/prebaked"
    ...
    declare -p BUCKET MAX_AGE_DAYS REMOTE_PREBAKED
}

cleanup_old() {
    log_section "Cleanup CI bakes older than ${MAX_AGE_DAYS} days from Linode"

    echo "Listing ${BUCKET}/prebaked/ before cleanup:"
    rclone ls "${REMOTE_PREBAKED}/" 2>/dev/null || echo "No ${BUCKET}/prebaked/ contents found"

    echo "Deleting files older than ${MAX_AGE_DAYS} days from ${REMOTE_PREBAKED}/"
    if rclone delete --min-age ${MAX_AGE_DAYS}d "${REMOTE_PREBAKED}/" --verbose; then
        echo "Old files deleted successfully"
    else
        echo "WARNING: Delete operation failed or no old files found (non-fatal)"
    fi

    echo "Listing ${BUCKET}/prebaked/ after cleanup:"
    rclone ls "${REMOTE_PREBAKED}/" 2>/dev/null || echo "No ${BUCKET}/prebaked/ contents remain"
}
