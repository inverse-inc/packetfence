	...
	case "$auth_type" in
		none)
			...
			wps_not_configured=1
                        ...
                        # Modifications below to support PF dynamic vlans
                        json_get_vars \
                                auth_server auth_secret auth_port \
                                dae_client dae_secret dae_port \
                                acct_server acct_secret acct_port \
                                vlan_naming vlan_tagged_interface \
                                vlan_bridge dynamic_vlan \
                                nasid iapp_interface
                        ...
                        #updates from wireless configuration

                        append bss_conf "vlan_file=/etc/config/hostapd.vlan" "$N"
                        append bss_conf "nas_identifier=$nasid" "$N"
                        # MAC Authentication
                        append bss_conf "macaddr_acl=2" "$N"
                        append bss_conf "vlan_naming=1" "$N"
