...
for my $section ( grep { /\S/ } $ini->Sections()) {
    ...
    next if ($old_value ne $sms_carriers);

    print "Updating $section\n";
    $ini->setval($section, 'sms_carriers', $new_value);
    $i |= 1;
}
