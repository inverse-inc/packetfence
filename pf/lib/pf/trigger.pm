...
BEGIN {
  ...
  our (@ISA, @EXPORT);
  @ISA    = qw(Exporter);
  @EXPORT = qw(trigger_db_prepare trigger_view trigger_view_enable trigger_view_all trigger_delete_all 
...
sub trigger_db_prepare {
  ...
  $trigger_view_vid_sql=$dbh->prepare( qq[ select tid_start,tid_end,class.vid,description from `trigger`,class where class.vid=`trigger`.vid and class.vid=?]);
  $trigger_view_tid_sql=$dbh->prepare( qq[ select tid_start,tid_end,class.vid,type,description from `trigger`,class where class.vid=`trigger`.vid and tid_start<=? and tid_end>=? ]);
  $trigger_view_all_sql=$dbh->prepare( qq[ select tid_start,tid_end,class.vid,type,description from `trigger`,class where class.vid=`trigger`.vid ]);
  $trigger_view_type_sql=$dbh->prepare( qq[ select tid_start,tid_end,class.vid,type,description from `trigger`,class where class.vid=`trigger`.vid and type=?]);
  $trigger_exist_sql=$dbh->prepare( qq [ select vid,tid_start,tid_end,type from `trigger` where vid=? and tid_start<=? and tid_end>=? and type=?]);
...
sub trigger_scan {
  ...
  if (!$tids || $tids=~/^all$/i){
    foreach my $row (@ref) {
      for (my $i=$row->{'tid_start'};$i<=$row->{'tid_end'};$i++){
        $pluginlist.="$i;";
      }
    }
    chop($pluginlist);
  ...
  my $trigger = Net::Nessus::ScanLite->new(  host            => $host,
                                            port            => $port,
                                            ssl             => $ssl,
                                          );
...
sub trigger_scan_add {
  ...
  if (defined $Config{'scan'}{'live_tids'} && grep({ $_ eq $tid } split(/\s*,\s*/, $Config{'scan'}{'live_tids'}))){
