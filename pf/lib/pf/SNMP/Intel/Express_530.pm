...
sub getVersion {
    ...
    $logger->trace("SNMP get_request for es530AgentRuntimeSwVersion: $oid_es530AgentRuntimeSwVersion");
    my $result = $this->{_sessionRead}->get_request(
        -varbindlist => [$oid_es530AgentRuntimeSwVersion]
    );
    my $runtimeSwVersion = ($result->{$oid_es530AgentRuntimeSwVersion} || '');
...
sub _setVlan {
    ...
    {                
        ...
        $logger->trace("SNMP set_request for dot1qPvid, dot1qVlanStaticUntaggedPorts and dot1qVlanStaticEgressPorts");
        $result = $this->{_sessionWrite}->set_request(
            -varbindlist => [
            "$OID_dot1qPvid.$dot1dBasePort", Net::SNMP::INTEGER, $newVlan,
...
sub setAdminStatus {
    ...
    $logger->trace("SNMP get_request for es530HwPortEncodingFactor: $OID_es530HwPortEncodingFactor");
    my $result = $this->{_sessionRead}->get_request(
        -varbindlist => ["$OID_es530HwPortEncodingFactor"]
    );
    my $es530PortEncodingFactor = $result->{"$OID_es530HwPortEncodingFactor"};
    my $es530PortCtrlUnitIndex = 1 + int($ifIndex / $es530PortEncodingFactor);
    ...
    $logger->trace("SNMP set_request for es530PortCtrlAdminState: $OID_es530PortCtrlAdminState");
    $result = $this->{_sessionWrite}->set_request(
        -varbindlist => [
        "$OID_es530PortCtrlAdminState.$es530PortCtrlUnitIndex.$es530PortCtrlModuleIndex.$es530PortCtrlIndex", Net::SNMP::INTEGER, ($enabled ? 3 : 2),
        ]
    );
    return (defined($result));
}
