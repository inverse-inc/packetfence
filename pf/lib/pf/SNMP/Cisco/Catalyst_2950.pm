                    ...
                    if (defined $portVlan) { # skip port with no VLAN
                        ...
                        if (($port_type == 1) || ($port_type == 4)) { # skip non static
                            ...
                            if ( grep(/^$portVlan$/, @{$this->{_vlans}}) != 0 ) {   # skip port in a non-managed VLAN
                                ...
                                push @nonUpLinks, $ifNames->{"$oid_ifName.$1"};
                            }
                        }
                    }
                }
...
sub clearMacAddressTable {
    ...
    my $ifNames = $this->{_sessionRead}->get_request(
        -varbindlist => ["$oid_ifName.$ifIndex"]
    );
...
sub getAllSecureMacAddresses {
    ...
    $logger->trace("SNMP get_table for cpsSecureMacAddrRowStatus: $oid_cpsSecureMacAddrRowStatus");
    my $result = $this->{_sessionRead}->get_table(
        -baseoid => "$oid_cpsSecureMacAddrRowStatus"
    );
    foreach my $oid_including_mac (keys %{$result}) {
        if ($oid_including_mac =~ /^$oid_cpsSecureMacAddrRowStatus\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)$/) {
...
sub isDynamicPortSecurityEnabled {
    ...
    $logger->trace("SNMP get_table for cpsSecureMacAddrType: $oid_cpsSecureMacAddrType");
    my $result = $this->{_sessionRead}->get_table(
        -baseoid => "$oid_cpsSecureMacAddrType.$ifIndex"
    );
    foreach my $oid_including_mac (keys %{$result}) {
...
sub getSecureMacAddresses {
    ...
    $logger->trace("SNMP get_table for cpsSecureMacAddrRowStatus: $oid_cpsSecureMacAddrRowStatus");
    my $result = $this->{_sessionRead}->get_table(
        -baseoid => "$oid_cpsSecureMacAddrRowStatus.$ifIndex"
    );
    foreach my $oid_including_mac (keys %{$result}) {
        if ($oid_including_mac =~ /^$oid_cpsSecureMacAddrRowStatus\.$ifIndex\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)$/) {
...
sub authorizeMAC {
    my ($this, $ifIndex, $deauthMac, $authMac, $vlan, $authVlan) = @_;
    my $logger = Log::Log4perl::get_logger("pf::SNMP::Cisco::Catalyst_2950");
    my $oid_cpsSecureMacAddrRowStatus = '1.3.6.1.4.1.9.9.315.1.2.2.1.4';  
...
sub getMaxMacAddresses {
    ...
    if ((! exists($result->{"$OID_cpsIfPortSecurityEnable.$ifIndex"})) || ($result->{"$OID_cpsIfPortSecurityEnable.$ifIndex"} eq 'noSuchInstance')) {
    ...
    $logger->trace("SNMP get_request for cpsIfMaxSecureMacAddr: $OID_cpsIfMaxSecureMacAddr.$ifIndex");
    $result = $this->{_sessionRead}->get_request(
        -varbindlist => [
        "$OID_cpsIfMaxSecureMacAddr.$ifIndex"
        ]
    );
    if ((! exists($result->{"$OID_cpsIfMaxSecureMacAddr.$ifIndex"})) || ($result->{"$OID_cpsIfMaxSecureMacAddr.$ifIndex"} eq 'noSuchInstance')) {
