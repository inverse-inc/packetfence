...
sub ifoctetslog_db_prepare {
  ...
  $ifoctetslog_history_user_start_end_sql=$dbh->prepare( qq [ select ifoctetslog.switch,ifoctetslog.port,read_time,ifoctetslog.mac,ifInOctets,ifOutOctets from ifoctetslog, node where ifoctetslog.mac=node.mac and pid=? and read_time >= from_unixtime(?) and read_time <= from_unixtime(?) order by mac asc, read_time desc ]);
  ...
  $ifoctetslog_history_switchport_start_end_sql=$dbh->prepare( qq [ select switch,port,read_time,mac,ifInOctets,ifOutOctets from ifoctetslog where switch=? and port=? and read_time >= from_unixtime(?) and read_time <= from_unixtime(?) order by read_time desc ]);
...
sub ifoctetslog_history_mac {
  ...
  if (exists($params{'start_time'}) && exists($params{'end_time'})) {
    $ifoctetslog_history_mac_start_end_sql->execute($mac, $params{'start_time'}, $params{'end_time'}) || return(0);
  ...
  foreach my $line (reverse @raw_data) {
    ...
    if (exists($previousLine->{'read_time'})) {
      my $timeDiff = str2time($line->{'read_time'}) - str2time($previousLine->{'read_time'});
      if (($previousLine->{'switch'} eq $line->{'switch'}) && ($previousLine->{'port'} == $line->{'port'})) {
        $line->{'throughPutIn'} = calculateThroughPut($line->{'ifInOctets'}, $previousLine->{'ifInOctets'}, $timeDiff);
        $line->{'throughPutOut'} = calculateThroughPut($line->{'ifOutOctets'}, $previousLine->{'ifOutOctets'}, $timeDiff);
      }
    }
    push @data, $line;
    $previousLine = $line;
...
sub ifoctetslog_history_user {
  ...
  my @data;
  if (exists($params{'start_time'}) && exists($params{'end_time'})) {
    $ifoctetslog_history_user_start_end_sql->execute($user, $params{'start_time'}, $params{'end_time'}) || return(0);
...
sub ifoctetslog_graph_switchport {
  ...
  return convertHistoryToGraph($params{'start_time'}, $params{'end_time'}, @tmp_data);
}
...
sub ifoctetslog_history_switchport {
  ...
  my @data;
  if (exists($params{'start_time'}) && exists($params{'end_time'})) {
    $ifoctetslog_history_switchport_start_end_sql->execute($switch,$params{'ifIndex'},$params{'start_time'},$params{'end_time'}) || return(0);
    @raw_data = db_data($ifoctetslog_history_switchport_start_end_sql);
  } else {
    $ifoctetslog_history_switchport_sql->execute($switch,$params{'ifIndex'}) || return(0);
...
sub calculateThroughPut {
  ...
  if ($currentOctets - $previousOctets >= 0) {
    return sprintf("%.2f", ($currentOctets - $previousOctets) / $timeDiff);
  } else {
...
sub convertHistoryToGraph {
  ...
  for ($current_timestamp = $start_time; $current_timestamp < $end_time; $current_timestamp += $timeDiffBetweenPoints) {
    $start_timestamp = $current_timestamp;
    $end_timestamp = $current_timestamp + $timeDiffBetweenPoints;
    my ($firstPosInRange,$lastPosInRange) = getFirstAndLastPosInRange($start_timestamp, $end_timestamp, $pos, @tmp_data);
    ...
    if ($lastPosInRange != -1) {
      ...
      if ($firstPosInRange == $lastPosInRange) {
        push @data, {'throughPutOut' => $tmp_data[$pos]->{'throughPutOut'},
          'throughPutIn' => $tmp_data[$pos]->{'throughPutIn'},
          'mydate' => $tmp_data[$pos]->{'read_time'}
        };
      } else {
        push @data, {'throughPutOut' => (($tmp_data[$lastPosInRange]->{'ifOutOctets'} - $tmp_data[$firstPosInRange]->{'ifOutOctets'}) / (str2time($tmp_data[$lastPosInRange]->{'read_time'}) - str2time($tmp_data[$firstPosInRange]->{'read_time'}))),
          'throughPutIn' => (($tmp_data[$lastPosInRange]->{'ifInOctets'} - $tmp_data[$firstPosInRange]->{'ifInOctets'}) / (str2time($tmp_data[$lastPosInRange]->{'read_time'}) - str2time($tmp_data[$firstPosInRange]->{'read_time'}))),
          'mydate' => $tmp_data[$lastPosInRange]->{'read_time'}
        };
      }
    } else {
      push @data, {'throughPutOut' => 0,
        'throughPutIn' => 0,
        'mydate'  => POSIX::strftime("%Y-%m-%d %H:%M:%S", localtime($current_timestamp))
      };
    }
  }
  return @data;
...
sub getFirstAndLastPosInRange {
  ...
  while (($pos < scalar(@tmp_data)) && ($start_time > str2time($tmp_data[$pos]->{'read_time'}))) {
    $pos++;
  }
  if (($pos < scalar(@tmp_data)) && ($end_time >= str2time($tmp_data[$pos]->{'read_time'}))) {
    $firstAndLastPos[0] = $pos;
  }
  if ($firstAndLastPos[0] != -1) {
    while (($pos < scalar(@tmp_data)) && ($end_time > str2time($tmp_data[$pos]->{'read_time'}))) {
      $pos++;
    }
    if (($pos > 0) && ($pos-1 < scalar(@tmp_data)) && ($start_time <= str2time($tmp_data[$pos-1]->{'read_time'}))) {
