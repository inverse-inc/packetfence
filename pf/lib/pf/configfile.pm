...
our ($configfile_add_sql, $configfile_exist_sql, $configfile_exist_new_sql, $configfile_exist_old_sql, $configfile_update_sql, $configfile_view_sql, $configfile_db_prepared);

BEGIN {
  use Exporter ();
  our (@ISA, @EXPORT);
  @ISA    = qw(Exporter);
  @EXPORT = qw(configfile_db_prepare configfile_import configfile_export configfile_add configfile_update configfile_view configfile_exist);
}
...
sub configfile_db_prepare {
  ...
  $configfile_update_sql=$dbh->prepare( qq[ update configfile set filecontent=?, lastmodified=from_unixtime(?) where filename=? ]);
  $configfile_add_sql=$dbh->prepare( qq[ insert into configfile(filename,filecontent,lastmodified) values(?,?,from_unixtime(?)) ]);
  $configfile_exist_sql=$dbh->prepare( qq[ select filename from configfile where filename=? ]);
  $configfile_exist_new_sql=$dbh->prepare( qq[ select filename from configfile where filename=? and unix_timestamp(lastmodified) > ? ]);
  $configfile_exist_old_sql=$dbh->prepare( qq[ select filename from configfile where filename=? and unix_timestamp(lastmodified) < ? ]);
  $configfile_view_sql=$dbh->prepare( qq[ select filename,filecontent,lastmodified from configfile where filename=? ]);
...
sub configfile_import {
  ...
  if (! configfile_exist($filename)) {
    $logger->info("config file $filename does not exist in database; addind new database entry");
    configfile_add($filename);
  } else {
    if (configfile_db_is_old($filename)) {
      $logger->info("config file $filename is outdated in database; updating database entry");
...
sub configfile_export {
  ...
  my $logger = Log::Log4perl::get_logger('pf::configfile');
  if (! configfile_exist($filename)) {
    $logger->info("config file $filename does not exist in database; unable to export");
  } else {
