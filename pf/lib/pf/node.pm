...
sub node_db_prepare {
  ...
  $node_ungrace_sql=$dbh->prepare( qq [ update node set status="unreg" where status="grace" and now() - lastskip > from_unixtime( ] . $Config{'registration'}{'skip_reminder'} . qq[ ) ]);
  ...
  $node_expire_window_sql=$dbh->prepare(  qq [ update node set status="unreg" where status="reg" and regdate + from_unixtime( ] . $Config{'registration'}{'expire_window'} . qq[ ) < now() ]);
  ...
  $node_expire_session_sql=$dbh->prepare( qq [ update node n set n.status="unreg" where n.status="reg" and n.mac not in (select i.mac from iplog i where (i.end_time=0 or i.end_time > now())) and n.mac not in
(select i.mac from iplog i where end_time!=0 and now() - i.end_time < from_unixtime( ] . $Config{'registration'}{'expire_session'} . qq[ ) ]);
  $node_expire_lastarp_sql=$dbh->prepare ( qq [ select mac from node where last_arp < (now() - from_unixtime(?)) and last_arp!=0 ]);
