...
sub locationlog_db_prepare {
    ...
    $locationlog_node_update_location_sql
        = $dbh->prepare(qq [ UPDATE node SET switch=?, port=? WHERE mac=? ]);
    $locationlog_update_end_switchport_sql
        = $dbh->prepare(
...
sub locationlog_view_open_switchport {
    my ( $switch, $ifIndex ) = @_;
    locationlog_db_prepare($dbh) if ( !$locationlog_db_prepared );
    return db_data( $locationlog_view_open_switchport_sql, $switch,
        $ifIndex );
}
...
sub locationlog_insert_start {
    ...
    locationlog_db_prepare($dbh) if ( !$locationlog_db_prepared );
    if ( defined($mac) ) {
        $locationlog_insert_start_with_mac_sql->execute( lc($mac), $switch,
            $ifIndex, $vlan )
            || return (0);
        $locationlog_node_update_location_sql->execute( $switch, $ifIndex,
            lc($mac) );
    } else {
        $locationlog_insert_start_no_mac_sql->execute( $switch, $ifIndex,
            $vlan )
            || return (0);
    }
    return (1);
...
sub locationlog_synchronize {
    ...
    if ($mustInsert) {
        if ( defined($mac) ) {
            $locationlog_insert_start_with_mac_sql->execute( $mac, $switch,
                $ifIndex, $vlan )
                || return (0);
        } else {
            $locationlog_insert_start_no_mac_sql->execute( $switch, $ifIndex,
