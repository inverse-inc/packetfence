...
To disable the self-registration feature, modify the following line in `/usr/local/pf/conf/pf.conf`.
...
According to this case, it is possible to limit the number of SNMP traps coming in from a single switch port and take action if that limit is reached. Per example, if over 100 traps are received by PacketFence from the same switch port in a minute, the switch port will be shut and a notification email will be sent.
Here's the configs for the SNMP traps limit feature. Per default, PacketFence will log the abnormal activity after 100 traps from the same switch port in a minute.
...
// this can probably be generated at build time 
// with the drawback that it might not work when user try to render document 
// on their workstation
// TODO make sure this works
----
sys::[/usr/local/pf/bin/pfcmd]
----
...
// TODO make sure this works
----
sys::[/usr/local/pf/bin/pfcmd_vlan]
----
