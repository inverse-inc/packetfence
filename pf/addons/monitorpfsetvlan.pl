...
sub monitorSnmp {
    my $fh = new File::Tail ('name' => SNMPLOG_FILE,
            'interval' => 2,
            'maxinterval' => 2);
...
sub monitorSetVlan {
    my $fh = new File::Tail ('name' => LOG_FILE,
            'interval' => 2,
            'maxinterval' => 2);
    ...
    while (1) {
        {
            ...
            foreach my $trap (keys %trapsReceivedSNMP) {
                if (exists($trapsReceivedSetVlan{$trap})) {
                    my $timeDiff = $trapsReceivedSetVlan{$trap} - $trapsReceivedSNMP{$trap};
...
#                    print time() . "\n";
                    ...
                    print "cannot find trap $trap, $trapAge seconds old, in pfsetvlan\n";
                    if ($trapAge > 120) {
