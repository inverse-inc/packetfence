...
my $ifDescHashRef;
my $result = $switch->{_sessionRead}->get_table(
  -baseoid => $OID_ifDesc
);
...
if (grep(/^snmp-server host $snmp_server .+ port-security/i, @tmp) > 0) {
  ...
  if ($doit) {
    $session->cmd("conf t");
    $session->cmd("snmp-server host $snmp_server version 2c public port-security");
    $session->cmd("end");
  }
}
...
foreach my $ifIndex (sort { $a <=> $b } keys %$ifDescHashRef) {
  ...
  my $config = '';
  my $lineNb = 0;
  while (($lineNb < scalar(@tmp)) && (! ($tmp[$lineNb] =~ /^interface /))) {
    $lineNb++;
  }
  ...
  if (grep(/^$ifIndex$/, @uplinks) > 0) {
    ...
    $logger->info("ifIndex $ifIndex excluded since port security is already configured");
  } else {
    my @macArray = $switch->_getMacAtIfIndex($ifIndex);
    if (scalar(@macArray) > 1) {
      $logger->info("ifIndex $ifIndex excluded since several MACs are present");
    } else {
      my $macToSecure;
      if (scalar(@macArray) == 1) {
      ...
      if ($macToSecure =~ /02:00:00:00:00/) {
        push @modLines, "switchport access vlan " . $switch->{_macDetectionVlan};
      } else {
        if ($config =~ /switchport access vlan dynamic/) {
          my $node_info = node_view($macToSecure);
          if ($node_info->{'vlan'} ne '') {
            push @modLines, "switchport access vlan " . $node_info->{'vlan'};
          } else {
            push @modLines, "switchport access vlan " . $switch->{_registrationVlan};
          }
        }
      }
      push @modLines, "switchport port-security";
      push @modLines, "no switchport port-security";
      push @modLines, "switchport port-security mac-address " . convertMac($macToSecure);
      push @modLines, "switchport port-security";
      if (! ($switchType =~ /Cisco::Catalyst_(29|35)50/)) {
        push @modLines, "switchport port-security maximum 1 vlan access";
      }
      push @modLines, "switchport port-security violation restrict";
      $logger->debug("switchport configuration command lines:\n" . join("\n", @modLines));
      if ($doit) {
        $logger->info("securing MAC $macToSecure on ifIndex $ifIndex");
...
sub convertMac {
  my ($temp) = @_;
  return substr($temp,0,2) . substr($temp, 3,2) . "." . substr($temp,6,2). substr($temp, 9,2) . "." . substr($temp,12,2) . substr($temp,15,2);
}
