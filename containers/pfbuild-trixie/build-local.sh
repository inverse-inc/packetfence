#!/bin/bash
# Build the pfbuild-debian-trixie image locally for testing.
#
# Usage (from repo root):
#   bash containers/pfbuild-trixie/build-local.sh
#
# Override build args via environment variables, e.g.:
#   GOVERSION=go1.25.10 PF_MINOR_RELEASE=15.2 bash containers/pfbuild-trixie/build-local.sh
...
IMAGE="ghcr.io/inverse-inc/packetfence/pfbuild-debian-trixie:local-test"
...
docker build \
    ...
    -t "${IMAGE}" \
    -f containers/pfbuild-trixie/Dockerfile \
    "${REPO_ROOT}"
