...
sub init {
    ...
    $self->{child_resources} = [ 'resource::CaptivePortal', 'resource::Database', 'resource::fqdn', 'config::Pfdetect', 'resource::trapping_range', 'resource::stats_levels', 'resource::passthroughs', 'resource::isolation_passthroughs', 'resource::network_config', 'config::Roles' ];
    if(defined($host_id)){
...
sub build_child {
    ...
    for my $section (qw(admin_login self_reg_login)) {
        next if $Config{$section}{sso_base_url};
        my $scheme = isenabled($Config{'captive_portal'}{'secure_redirect'}) ? "https://" : "http://";
        $Config{$section}{sso_base_url} = $scheme.$Config{general}{hostname}.".".$Config{general}{domain};
    }
