package pfconfig::namespaces::resource::all_cluster_sections;

=head1 NAME

pfconfig::namespaces::resource::all_cluster_sections
...
sub build {
    ...
    for my $cluster_name (@$clusters) {
        my @cluster;
        push @cluster, @{$self->{cluster_resource}->{_servers}->{$cluster_name}};
        push @cluster, $self->{cluster_resource}->{_CLUSTER}->{$cluster_name};
        $servers{$cluster_name} = \@cluster;
    }
