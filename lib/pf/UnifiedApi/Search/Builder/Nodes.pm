...
our %ALLOWED_JOIN_FIELDS = (
    ...
    'online' => {
        ...
        column_spec   => "CASE IFNULL( (SELECT is_online from node_current_session as ncs WHERE ncs.mac = node.mac), 'unknown') WHEN 'unknown' THEN 'unknown' WHEN 0 THEN 'false' ELSE 'true' END|online"
    },
    'node_category.name' => {
...
sub rewrite_online_query {
    ...
    my $value = $q->{value};
    if (!defined $value || ($value ne 'true' && $value ne 'false' && $value ne 'unknown')) {
