...
sub item {
    ...
    return $self->cleanup_item($self->item_from_store($skip_inheritance));
}
...
sub item_from_store {
    my ($self, $skip_inheritance) = @_;
    if ($skip_inheritance) {
        return $self->config_store->readWithoutInherited($self->id, 'id')
    } else {
...
sub update {
    ...
    my $new_data = $self->validate_item($new_item);
    if (!defined $new_data) {
        return;
    }
    delete $new_data->{id};
