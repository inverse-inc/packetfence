package pf::WebAPI::RPC::JSON;
=head1 NAME

pf::WebAPI::RPC::JSON add documentation
...
our $ENCODER = JSON::XS->new;
our $DECODER = $ENCODER;

sub default_content_type { "application/json-rpc"  }

sub allowed_content_types { ( 'application/json-rpc', 'application/json', 'application/jsonrequest') }
...
sub handleRequest {
    ...
    eval {
        ...
        $response = {"jsonrpc" => "2.0" , result => \@results, id => $id};
    };
    if($@) {
        $response = {"jsonrpc" => "2.0" , error => { code => -32000, message => "$@" ,}, id => $id};
    }
...
sub parseRequest {
    ...
    my ($method,$args,$id) = @{$data}{qw(method params id)};
