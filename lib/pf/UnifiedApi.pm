...
sub setup_api_v1_config_roles_routes {
    ...
    $resource_route->register_sub_action({action => 'reassign', method => 'PATCH', auditable => 1});
    $resource_route->register_sub_action({action => 'bulk_reevaluate_access', method => 'POST', auditable => 1});
    $collection_route->register_sub_action({ action => 'bulk_update_acl', method => 'POST', auditable => 1});
