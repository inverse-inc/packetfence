...
sub zone_target {
  ...
  if (exists $zone_target_option{$v}){
    print "Target zone is $v";
    return create_string_for_xml("target",$v);
  } else {
    print "Unknown target zone\nApply %%REJECT%%";
...
sub zone_interface {
  ...
  if (length $v) {
    ...
    if (not exists $all_interfaces{$v}){
      print "Unknown interface zone\nApply management interface"; 
...
sub zone_sources {
  ...
  foreach $s ( $v.keys() ) {
    ...
    if (is_source_name($s_name){
      ...
      print "Name ($s_name) is not in available list (address MAC or ipset).\nSource is removed.";
    }
  }
  return $t;
}
...
sub zone_services {
  ...
  foreach $s ( $v.keys() ) {
    if (exists $available_services{$s}){
      ...
      print "Service $s does not exist.\nService is removed.";
    }
  }
  return %t;
}
...
sub zone_forward_ports {
  ...
  foreach $s ( $v.keys() ) {
    ...
    my $proto=$v{$s}{"protocol"};
    my $portid=$v{$s}{"portid"};
    if (length $proto && $portid){
      if (is_proto($proto)) {
        $t{$s}=$v{$s};
      } else {
        print "Forward Port Prototype ($proto) does not exist.\nForward Port is removed.";
      }
    } else {
      print "Forward Port Prototype ($proto) or Portid ($portid) is empty.\nForward Port is removed.";
    }
  }
  return %t;
}
...
sub zone_source_ports {
  ...
  foreach $s ( $v.keys() ) {
    ...
    if (is_proto($proto)){
      ...
      my $portid=$v{$s}{"portid"};
      print "Source Port Prototype ($proto) does not exist on $portid.\nSource Port is removed.";
    }
  }
  return %t;
}
...
sub zone_rules {
  ...
  foreach $r ($v.keys()) {
    ...
    if ( exists $v{$r}{"source"} ){
      ...
      if (not is_source_name($s_name){
        print "The Source under $r{"name"} does not have a correct name.";
        $flag=undef;
      }
    ...
    if ( exists $v{$r}{"match_rules"} ){
       ...
       foreach %match_rule (%match_rules.keys()) {
         if ($match_rule{"name"} eq "service"){
           my $s = $match_rule{"service"};
           my %available_services = get_all_firewalld_services();
           if (not exists $available_services{$s}){
             print "Service $s does not exist.";
             $flag=undef;
           }
         } elsif ($match_rule{"name"} eq "port"){
           if (not exists $match_rule{"portid"} || not exists $match_rule{"port_protocol"}){
             print "Port rule does not exist.";
             $flag=undef;
           } else {
             if (not is_proto($match_rule{"port_protocol"})) {
               print "Port rule does not have a known protocol.";
               $flag=undef;
             }
           }
         } elsif ($match_rule{"name"} eq "protocol") {
           if (not is_proto($match_rule{"protocol"})) {
             print "Protocol does not have a known protocol.";
             $flag=undef;
           }
         } elsif ($match_rule{"name"} eq "forward_port") {
           if (not exists $match_rule{"portid"} || not exists $match_rule{"protocol"}){
             print "A forward port rule needs a portid and a protocol";
             $flag=undef;
           } elsif (exists $match_rule{"protocol"} && not is_proto($match_rule{"protocol"})) {
             print "A forward port rule needs a known protocol";
             $flag=undef;
           } else {
