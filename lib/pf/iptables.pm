...
sub iptables_generate {
    ...
    if (is_inline_enforcement_enabled()) {
        ...
        $self->generate_inline_rules(
            \$tags{'filter_forward_inline'}, \$tags{'nat_prerouting_inline'}, \$tags{'nat_postrouting_inline'}, \$tags{'input_inter_inline_rules'});
...
sub generate_inline_rules {
    my ($self, $filter_rules_ref, $nat_prerouting_ref, $nat_postrouting_ref, $input_filtering_ref) = @_;
    my $logger = Log::Log4perl::get_logger('pf::iptables');
