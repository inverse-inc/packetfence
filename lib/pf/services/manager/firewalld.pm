...
sub _stop {
    ...
    pf_run("sudo iptables -N DOCKER");
    pf_run("sudo iptables -N DOCKER-ISOLATION-STAGE-1");
    pf_run("sudo iptables -N DOCKER-ISOLATION-STAGE-2");
    pf_run("sudo iptables -N DOCKER-USER");
    pf_run("sudo iptables -A FORWARD -j DOCKER-USER");
    pf_run("sudo iptables -A FORWARD -j DOCKER-ISOLATION-STAGE-1");
    pf_run("sudo iptables -A FORWARD -o docker0 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT");
    pf_run("sudo iptables -A FORWARD -o docker0 -j DOCKER");
    pf_run("sudo iptables -A FORWARD -i docker0 ! -o docker0 -j ACCEPT");
    pf_run("sudo iptables -A FORWARD -i docker0 -o docker0 -j ACCEPT");
    pf_run("sudo iptables -A DOCKER-ISOLATION-STAGE-1 -i docker0 ! -o docker0 -j DOCKER-ISOLATION-STAGE-2");
    pf_run("sudo iptables -A DOCKER-ISOLATION-STAGE-1 -j RETURN");
    pf_run("sudo iptables -A DOCKER-ISOLATION-STAGE-2 -o docker0 -j DROP");
    pf_run("sudo iptables -A DOCKER-ISOLATION-STAGE-2 -j RETURN");
    pf_run("sudo iptables -A DOCKER-USER -j RETURN");
