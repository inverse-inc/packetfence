package pf::firewallsso::WatchGuard;

=head1 NAME

pf::firewallsso::WatchGuard

=head1 SYNOPSIS

The pf::firewallsso::WatchGuard module implements an object oriented interface
to update the WatchGuard user table.
...
sub description {'WatchGuard Firewall'}
...
sub action {
    ...
    my $config = $ConfigFirewallSSO{$firewall_conf};
        ...
        && (grep $_ eq $node_info->{'category'}, @categories))
    {
        ...
        $username = $node_info->{'last_dot1x_username'} if ($config->{'uid'} eq '802.1x');
        return 0
          if ($config->{'uid'} eq '802.1x' && $node_info->{'last_dot1x_username'} eq '');
