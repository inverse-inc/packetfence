...
sub external_captive_portal {
    ...
    if (defined($switchId)) {
        ...
        if (defined($switch) && $switch ne '0' && $switch->supportsExternalPortal) {
            ...
            iplog_open($client_mac,$client_ip,3600) if (defined ($client_ip) && defined ($client_mac));
        ...
        if ($session_id{_session_id} eq $session) {
            ...
            iplog_open($session_id{client_mac},$r->connection->remote_ip,3600) if (defined ($r->connection->remote_ip) && defined ($session_id{client_mac}));
