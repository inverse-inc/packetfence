...
sub update_switch_role_with_mask {
    ...
    return undef unless exists $switch->{"_".$locationlog->{'role'}."NetworkFrom"} && defined($switch->{"_".$locationlog->{'role'}."NetworkFrom"}) && $switch->{"_".$locationlog->{'role'}."NetworkFrom"} eq "dynamic";
