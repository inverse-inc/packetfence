package captiveportal::DynamicRouting::Provisioning;

=head1 NAME

captiveportal::DynamicRouting::Provisioning
...
use pf::log;
use captiveportal::DynamicRouting::TLSEnrollment;
use pf::util;
...
sub execute_child {
    ...
    if( $self->is_eap_tls && !$self->session->{tls_enrollment_completed} ) {
        my $tls_module = captiveportal::DynamicRouting::TLSEnrollment->new(id => $self->id."_pki_module", parent => $self, app => $self->app, pki_provider_id => $provisioner->getPkiProvider()->id);
        $tls_module->execute();
    }
