...
sub signup : Local {
    ...
    if ($request->method eq 'POST') {
        ...
        $c->session->{source_id} = "local";
        $c->stash->{matchParams} = {username => $c->session->{username}};
        $c->stash->{info} = {pid => $c->session->{username}};
        $c->forward(Authenticate => 'setRole');
        $c->forward(Authenticate => 'setUnRegDate');
...
sub validate_new_user : Private {
    ...
    my @actions = (
        ...
        {type => 'unreg_date', value => $expiration},
    );
