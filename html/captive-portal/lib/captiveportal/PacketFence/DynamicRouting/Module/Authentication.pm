...
has '+actions' => (default => sub {{ "mark_as_sponsor" => [], "set_access_durations" => [], "set_access_level" => [], "trigger_portal_mfa" => [], "on_success" => [], "on_failure" => [], "destination_url" => [], "role_from_source" => [], "unregdate_from_source" => [], "time_balance_from_source" => [], "bandwidth_balance_from_source" => [], "unregdate_from_sponsor_source" => []}});
...
sub execute_actions {
    ...
    # Admin SSO only needs an access level. Self-registration SSO must still match a
    # regular authentication rule (role + access duration); mark_as_sponsor is optional on top of it.
    if ($self->app->isRootSSO and !$self->app->isSelfRegSSO and defined($self->new_node_info->{access_level})) {
