package captive::portal::Controller::Authenticate;
...
captive::portal::Controller::Authenticate - Catalyst Controller
...
sub modeToPath {
    ...
    if ( exists $action->{valid_modes}{$mode} ) {
        ...
        exists $c->config->{'captive::portal::Custom'}{Authenticate}{modes}
        {$mode}
        && defined $c->config->{'captive::portal::Custom'}
        ->{Authenticate}{modes}{$mode} ) {
        $path = $c->config->{'captive::portal::Custom'}
          ->{Authenticate}{modes}{$mode};
    }
    return $path;
}
