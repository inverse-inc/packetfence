...
const actions = {
  ...
  search: ({ state, getters, commit, dispatch }, page) => {
    ...
    let apiPromise = state.searchQuery ? api.searchAdminApiAuditLogs(Object.assign(body, { query: state.searchQuery })) : api.allAdminApiAuditLogs(body)
  ...
  getItem: ({ state, commit }, id) => {
    ...
    commit('ITEM_REQUEST')
    return api.getAdminApiAuditLog(id).then(data => {
