...
const setup = (props, context) => {
  ...
  const bind = computed(() => {
    ...
    if (placeholder.value) {
      ...
      switch (true) {
        case placeholder.value === 'no' && [options[0].value, options[1].value].includes(value.value):
          hints = [/*0, 1,*/[0, 1]]
          break

        case placeholder.value === 'yes' && [options[1].value, options[2].value].includes(value.value):
          hints = [/*1, 2,*/[1, 2]]
          break
      }
    }
...
export default {
  name: 'base-form-group-toggle-no-yes-default',
  inheritAttrs: false,
  components,
  props,
  setup
