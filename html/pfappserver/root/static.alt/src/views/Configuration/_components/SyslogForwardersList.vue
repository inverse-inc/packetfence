  ...
  <b-card no-body>
    ...
    >
      <template slot="pageHeader">
        <b-card-header>
          <h4 class="mb-3" v-t="'Syslog Entries'"></h4>
        </b-card-header>
      </template>
      <template slot="buttonAdd">
        <b-button variant="outline-primary" :to="{ name: 'newSyslogForwarder', params: { syslogForwarderType: 'server' } }">{{ $t('Add Syslog Entry') }}</b-button>
      </template>
      ...
      <template slot="buttons" slot-scope="item">
        <span class="float-right text-nowrap">
          ...
          <pf-button-delete  v-if="!item.not_deletable" size="sm" variant="outline-danger" :disabled="isLoading" :confirm="$t('Delete Syslog Entry?')" @on-delete="remove(item)" reverse/>
        </span>
      </template>
