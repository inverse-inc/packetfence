  ...
  <b-card no-body>
    ...
    >
      ...
      <template slot="buttonAdd">
        <b-button variant="outline-primary" :to="{ name: 'newFingerbankDhcpv6Fingerprint' }">{{ $t('Add Local DHCPv6 Fingerprint') }}</b-button>
      </template>
...
export default {
  ...
  methods: {
    clone (item) {
      this.$router.push({ name: 'cloneFingerbankDhcpv6Fingerprint', params: { id: item.id } })
    },
    remove (item) {
