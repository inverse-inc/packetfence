  ...
  <b-card no-body>
    ...
    >
      ...
      <template slot="buttonAdd" v-if="scope === 'local'">
        <b-button variant="outline-primary" :to="{ name: 'newFingerbankMacVendor', params: { scope: 'local' } }">{{ $t('Add DHCP Vendor') }}</b-button>
      </template>
...
export default {
  ...
  methods: {
    clone (item) {
      this.$router.push({ name: 'cloneFingerbankMacVendor', params: { scope: this.scope, id: item.id } })
    },
    remove (item) {
      this.$store.dispatch(`${this.storeName}/deleteMacVendor`, item.id).then(response => {
