  ...
  <b-card no-body>
    ...
    >
      ...
      <template slot="buttonAdd">
        <b-dropdown :text="$t('Add Provisioner')" variant="outline-primary" class="my-2">
      ...
      <template slot="buttons" slot-scope="item">
        <span class="float-right text-nowrap">
          ...
          <pf-button-delete  v-if="!item.not_deletable" size="sm" variant="outline-danger" :disabled="isLoading" :confirm="$t('Delete Provisioner?')" @on-delete="remove(item)" reverse/>
        </span>
      </template>
    </pf-config-list>
  </b-card>
