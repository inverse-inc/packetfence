...
export default {
  ...
  methods: {
    ...
    save () {
      const actionKey = this.actionKey
      this.$store.dispatch('$_fingerbank/updateDhcpFingerprint', this.form).then(response => {
        if (actionKey) { // [CTRL] key pressed
          this.close()
        }
      })
    ...
    remove () {
      this.$store.dispatch('$_fingerbank/deleteDhcpFingerprint', this.id).then(response => {
        this.close()
      })
    }
  },
