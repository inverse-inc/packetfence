  ...
  >
    <template slot="header" is="b-card-header">
      ...
      <h4 class="d-inline mb-0">
        <span v-if="!isNew && !isClone">{{ $t('Fingerbank User Agent {id}', { id: id }) }}</span>
        <span v-else-if="isClone">{{ $t('Clone Fingerbank User Agent {id}', { id: id }) }}</span>
    ...
    <template slot="footer">
      <b-card-footer @mouseenter="$v.form.$touch()">
        ...
        <pf-button-delete v-if="isDeletable && scope === 'local'" class="ml-1" :disabled="isLoading" :confirm="$t('Delete Fingerbank User Agent?')" @on-delete="remove()"/>
      </b-card-footer>
    </template>
  </pf-config-view>
</template>
