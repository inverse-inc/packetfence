  ...
  <b-card no-body>
    ...
    >
      <template v-slot:pageHeader>
        <b-card-header>
          <h4 class="mb-0">{{ $t('EAP Profiles') }}</h4>
        </b-card-header>
      </template>
      <template v-slot:buttonAdd>
        <b-button variant="outline-primary" :to="{ name: 'newRadiusEap' }">{{ $t('New EAP Profile') }}</b-button>
      ...
      <template v-slot:emptySearch="state">
        <pf-empty-table :isLoading="state.isLoading">{{ $t('No EAP profiles found') }}</pf-empty-table>
      </template>
