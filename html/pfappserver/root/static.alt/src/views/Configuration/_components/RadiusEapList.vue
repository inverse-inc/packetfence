  ...
  <b-card no-body>
    ...
    >
      <template v-slot:pageHeader>
        <b-card-header>
          <h4 class="mb-0">{{ $t('RADIUS EAP') }}</h4>
        </b-card-header>
      </template>
      <template v-slot:buttonAdd>
        <b-button variant="outline-primary" :to="{ name: 'newRadiusEap' }">{{ $t('New EAP Configuration') }}</b-button>
      ...
      <template v-slot:emptySearch="state">
        <pf-empty-table :isLoading="state.isLoading">{{ $t('No EAP configurations found') }}</pf-empty-table>
      </template>
...
export default {
  ...
  methods: {
    clone (item) {
      this.$router.push({ name: 'cloneRadiusEap', params: { id: item.id } })
    },
    remove (item) {
      this.$store.dispatch('$_radius_eap/deleteRadiusEap', item.id).then(response => {
