...
export default {
  ...
  computed: {
    ...
    view () {
      return view(this.form, this.meta) // ../_config/fingerbank/dhcpV6Fingerprint
    },
    invalidForm () {
      return this.$store.getters[`${this.formStoreName}/$formInvalid`]
    },
    isLoading () {
      return this.$store.getters['$_fingerbank/isDhcpv6FingerprintsLoading']
    },
    isDisabled () {
  ...
  methods: {
    init () {
      ...
      if (this.id) {
        // existing
        this.$store.dispatch('$_fingerbank/getDhcpv6Fingerprint', this.id).then(form => {
    ...
    clone () {
      this.$router.push({ name: 'cloneFingerbankDhcpv6Fingerprint', params: { scope: 'local' } })
    },
    create () {
      const actionKey = this.actionKey
      this.$store.dispatch('$_fingerbank/createDhcpv6Fingerprint', this.form).then(response => {
        if (actionKey) { // [CTRL] key pressed
          this.close()
        } else {
          this.$router.push({ name: 'fingerbankDhcpv6Fingerprint', params: { scope: this.scope, id: response.id } })
        }
      })
    },
    save () {
      const actionKey = this.actionKey
      this.$store.dispatch('$_fingerbank/updateDhcpv6Fingerprint', this.form).then(response => {
        if (actionKey) { // [CTRL] key pressed
          this.close()
        }
      })
    ...
    remove () {
      this.$store.dispatch('$_fingerbank/deleteDhcpv6Fingerprint', this.id).then(response => {
        this.close()
      })
    }
  },
