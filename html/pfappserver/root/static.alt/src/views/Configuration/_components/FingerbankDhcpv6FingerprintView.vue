<template>
  ...
  >
    <template slot="header" is="b-card-header">
      ...
      <h4 class="mb-0">
        <span v-if="!isNew && !isClone">{{ $t('Fingerbank DHCPv6 Fingerprint {id}', { id: id }) }}</span>
        <span v-else-if="isClone">{{ $t('Clone Fingerbank DHCPv6 Fingerprint {id}', { id: id }) }}</span>
    ...
    <template slot="footer">
      <b-card-footer @mouseenter="$v.form.$touch()">
        ...
        <pf-button-delete v-if="isDeletable" class="ml-1" :disabled="isLoading" :confirm="$t('Delete Fingerbank DHCPv6 Fingerprint?')" @on-delete="remove()"/>
      </b-card-footer>
    </template>
  </pf-config-view>
</template>
...
export default {
  ...
  methods: {
    ...
    clone () {
      this.$router.push({ name: 'cloneFingerbankDhcpv6Fingerprint' })
    },
    create () {
      ...
      this.$store.dispatch(`${this.storeName}/createDhcpv6Fingerprint`, this.form).then(response => {
        if (ctrlKey) { // [CTRL] key pressed
          ...
          this.$router.push({ name: 'fingerbankDhcpv6Fingerprint', params: { id: this.form.id } })
        }
      })
    },
    save () {
