...
import {
  view,
  validators
} from '../_config/fingerbank/device'
...
export default {
  ...
  computed: {
    ...
    isLoading () {
      return this.$store.getters['$_fingerbank/isDevicesLoading']
    },
    isDisabled () {
  ...
  methods: {
    init () {
      ...
      if (this.id) {
        // existing
        this.$store.dispatch('$_fingerbank/getDevice', this.id).then(form => {
    ...
    clone () {
      this.$router.push({ name: 'cloneFingerbankDevice', params: { scope: 'local' } })
    },
    create () {
      const actionKey = this.actionKey
      this.$store.dispatch('$_fingerbank/createDevice', this.form).then(response => {
        if (actionKey) { // [CTRL] key pressed
          this.close()
        } else {
          this.$router.push({ name: 'fingerbankDevice', params: { scope: this.scope, id: response.id } })
        }
      })
    },
    save () {
      const actionKey = this.actionKey
      this.$store.dispatch('$_fingerbank/updateDevice', this.form).then(response => {
        if (actionKey) { // [CTRL] key pressed
          this.close()
        }
      })
    ...
    remove () {
      this.$store.dispatch('$_fingerbank/deleteDevice', this.id).then(response => {
        this.close()
      })
    }
  },
