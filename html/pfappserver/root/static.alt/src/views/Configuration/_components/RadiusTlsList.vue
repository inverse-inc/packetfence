  ...
  <b-card no-body>
    ...
    >
      <template v-slot:pageHeader>
        <b-card-header>
          <h4 class="mb-0">{{ $t('TLS Profiles') }}</h4>
        </b-card-header>
      </template>
      <template v-slot:buttonAdd>
        <b-button variant="outline-primary" :to="{ name: 'newRadiusTls' }">{{ $t('New TLS Profile') }}</b-button>
      ...
      <template v-slot:emptySearch="state">
        <pf-empty-table :isLoading="state.isLoading">{{ $t('No TLS profiles found') }}</pf-empty-table>
      </template>
