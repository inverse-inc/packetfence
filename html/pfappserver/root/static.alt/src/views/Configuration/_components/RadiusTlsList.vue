  ...
  <b-card no-body>
    ...
    >
      <template v-slot:pageHeader>
        <b-card-header>
          <h4 class="mb-0">{{ $t('RADIUS TLS') }}</h4>
        </b-card-header>
      </template>
      <template v-slot:buttonAdd>
        <b-button variant="outline-primary" :to="{ name: 'newRadiusTls' }">{{ $t('New TLS Configuration') }}</b-button>
      ...
      <template v-slot:emptySearch="state">
        <pf-empty-table :isLoading="state.isLoading">{{ $t('No TLS configurations found') }}</pf-empty-table>
      </template>
...
export default {
  ...
  methods: {
    clone (item) {
      this.$router.push({ name: 'cloneRadiusTls', params: { id: item.id } })
    },
    remove (item) {
      this.$store.dispatch('$_radius_tls/deleteRadiusTls', item.id).then(response => {
