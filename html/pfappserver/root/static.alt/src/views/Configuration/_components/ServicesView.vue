...
import {
  pfConfigurationServiceViewFields as fields
} from '@/globals/configuration/pfConfigurationServices'
  ...
  methods: {
    init () {
      this.$store.dispatch('$_bases/optionsServices').then(options => {
        ...
        this.$store.dispatch('$_bases/getServices').then(data => {
    ...
    save () {
      ...
      this.$store.dispatch('$_bases/updateServices', form).then(response => {
        // TODO - notification
      })
    }
  },
