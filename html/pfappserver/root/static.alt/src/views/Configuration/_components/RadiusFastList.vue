  ...
  <b-card no-body>
    ...
    >
      <template v-slot:pageHeader>
        <b-card-header>
          <h4 class="mb-0">{{ $t('RADIUS Fast') }}</h4>
        </b-card-header>
      </template>
      <template v-slot:buttonAdd>
        <b-button variant="outline-primary" :to="{ name: 'newRadiusFast' }">{{ $t('New Fast Configuration') }}</b-button>
      ...
      <template v-slot:emptySearch="state">
        <pf-empty-table :isLoading="state.isLoading">{{ $t('No Fast configurations found') }}</pf-empty-table>
      </template>
