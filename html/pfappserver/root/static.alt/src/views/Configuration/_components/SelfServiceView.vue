...
export default {
  ...
  methods: {
    init () {
      this.$store.dispatch('$_self_services/options', this.id).then(options => {
        const { meta = {} } = options
        const { isNew, isClone } = this
        this.$store.dispatch(`${this.formStoreName}/setMeta`, { ...meta, ...{ isNew, isClone } })
        if (this.id) { // existing
          this.$store.dispatch('$_self_services/getSelfService', this.id).then(form => {
    ...
    close () {
      this.$router.push({ name: 'self_services' })
    },
    clone () {
