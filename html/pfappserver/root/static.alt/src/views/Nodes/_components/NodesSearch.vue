      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkCloseSecurityEvents`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Closed security events on {nodeCount} node(s).', { nodeCount: items.length }),
    ...
    applyBulkRegister () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkRegisterNodes`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Registered {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkDeregister () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkDeregisterNodes`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Deregistered {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkReevaluateAccess () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkReevaluateAccess`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Reevaluated access on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkRestartSwitchport () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkRestartSwitchport`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Restarted switch port on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkRefreshFingerbank () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkRefreshFingerbank`, { items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Refreshed fingerbank on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkRole (role) {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkApplyRole`, { category_id: role.category_id, items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Applied role on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkBypassRole (role) {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkApplyBypassRole`, { bypass_role_id: role.category_id, items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Applied bypass role on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkSecurityEvent (securityEvent) {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkApplySecurityEvent`, { vid: securityEvent.vid, items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Applied security event on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
    ...
    applyBulkBypassVlan () {
      ...
      if (macs.length > 0) {
        this.$store.dispatch(`${this.storeName}/bulkApplyBypassVlan`, { bypass_vlan: bypassVlan, items: macs }).then(items => {
          ...
          this.$store.dispatch('notification/info', {
            message: this.$i18n.t('Applied bypass VLAN on {nodeCount} node(s).', { nodeCount: items.length }),
            success: successCount,
            skipped: skippedCount,
            failed: failedCount
          })
