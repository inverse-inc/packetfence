 ...
 *    ]
 *
 *    `vuelidate` (vuelidate object) -- the local vuelidate model where this component attaches itself.
          ...
          >
            ...
            <b-col cols="4" class="text-left py-1" align-self="start">
              ...
              <pf-form-chosen
                ...
                :options="fields"
                :vuelidate="getTypeValidation(index)"
            ...
            <b-col cols="6" class="text-left py-1" align-self="start">
              ...
              <pf-form-chosen v-if="isFieldType(adminroleValueType, inputValue[index])"
                ...
                :vuelidate="getValueValidation(index)"
...
export default {
  ...
  methods: {
    ...
    getTypeInvalidFeedback (index) {
      let feedback = []
      if (this.vuelidate && index in this.vuelidate) {
        const vuelidate = this.vuelidate[index] /* use external vuelidate $v model */
        if (vuelidate.type) {
          const vuelidateModel = vuelidate.type
          if (vuelidateModel) {
            Object.entries(vuelidateModel.$params).forEach(([key, value]) => {
              if (vuelidateModel[key] === false) {
    ...
    getValueValidation (index) {
      if (this.vuelidate && index in this.vuelidate && 'value' in this.vuelidate[index]) {
        return this.vuelidate[index].value
      }
      return {}
    },
    getValueInvalidFeedback (index) {
      ...
      if (this.vuelidate && index in this.vuelidate) {
        const vuelidate = this.vuelidate[index] /* use external vuelidate $v model */
        if (vuelidate.value) {
          const vuelidateModel = vuelidate.value
          if (vuelidateModel) {
            Object.entries(vuelidateModel.$params).forEach(([key, value]) => {
              if (vuelidateModel[key] === false) {
                feedback.push(key.trim())
