...
<template>
  <div ref="svgContainer" :class="[ 'svgContainer', { [`highlight-${highlight}`]: highlight } ]">
    ...
    >
      ...
      <!-- links -->
      <path v-for="link in links" :key="`${link.source.id}-${link.target.id}`"
        v-bind="linkPathAttrs(link)"
        :class="[ 'link', { 'highlight': link.highlight } ]"
      />

      <!-- highlighted link text paths -->
      <text v-if="link.highlight" v-for="link in links" :key="`${link.source.id}-${link.target.id}`" class="linkText" dy="-2">
      ...
      <template v-for="(node, i) in nodes">
        ...
        <!--- router icon --->
        <use v-if="node.type === 'router'" :key="node.id"
          xlink:href="#router"
          width="32" height="32"
          :id="`node-${node.id}`"
          :x="coords[i].x - (32 / 2)"
      ...
      <!-- mini map -->
      <rect v-if="~~miniMapHeight > 0" class="innerMiniMap" v-bind="innerMiniMapProps" />
      <rect v-if="~~miniMapHeight > 0" class="outerMiniMap" v-bind="outerMiniMapProps"
...
export default {
  ...
  props: {
    ...
    miniMapHeight: { // set to 0 to disable (default: 0)
      type: Number,
      default: 0
    },
    tooltipDistance: {
