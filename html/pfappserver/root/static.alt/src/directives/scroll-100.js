...
export default Vue.directive('scroll-100', {
  update: function (el) {
    ...
    while (parentNode && 'classList' in parentNode) {
      parentNode.classList.add('scroll-100', 'h-100', 'overflow-hidden')
  ...
  unbind: function (el) {
    ...
    let parentNode = el.parentNode
    while (parentNode && 'classList' in parentNode) {
      parentNode.classList.remove('scroll-100', 'h-100', 'overflow-hidden')
      parentNode = parentNode.parentNode
    }
    // Element is detached, remove height constraint on all remaining nodes of document
    let parentNodes = Array.from(document.getElementsByClassName('scroll-100 h-100 overflow-hidden'))
