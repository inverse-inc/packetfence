<template>
  <b-card no-body>
    <b-card-header>
      <h4>{{ $t('Firewall SSO') }}</h4>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-dropdown :text="$t('New Firewall')" variant="outline-primary">
          <b-dropdown-item v-for="(text, firewallType) in types" :key="firewallType"
            :to="{ name: 'newFirewall', params: { firewallType } }"
