<template>
  ...
  >
    <form-group-sso-status namespace="sso_status"
                           ...
                           :text="$i18n.t('Whether or not sponsors validating a guest request can authenticate through single sign-on. The SSO login path must be served by a connection profile whose root module is a SelfRegSSO module containing the SAML or OAuth source to use.')"
    ...
    <form-group-sso-login-path namespace="sso_login_path"
                               ...
                               :text="$i18n.t('The portal path the sponsor is redirected to in order to perform the single sign-on. A connection profile with a URI filter on this path must select the SelfRegSSO root module.')"
    />

    <form-group-sso-login-text namespace="sso_login_text"
                               :column-label="$i18n.t('SSO Login Button Text')"
                               :text="$i18n.t('The text of the single sign-on button on the sponsor login page.')"
    />
    ...
    <form-group-allow-username-password namespace="allow_username_password"
                                        ...
                                        :text="$i18n.t('Whether the username/password form is still offered on the sponsor login page when SSO is enabled. Disabling this forces sponsors to use single sign-on.')"
                                        enabled-value="enabled"
                                        disabled-value="disabled"
    />
