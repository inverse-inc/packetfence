  ...
  <b-card no-body>
    <b-card-header>
      <h4 class="d-flex align-items-center">
        {{ $t('Floating Devices') }}
        <base-button-help class="text-black-50 ml-1" url="PacketFence_Installation_Guide.html#_floating_network_devices" />
      </h4>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-button variant="outline-primary" @click="goToNew">{{ $t('New Floating Device') }}</b-button>
      </base-search>
        ...
        <template #cell(buttons)="{ item }">
          <span class="float-right text-nowrap text-right">
            <base-button-confirm v-if="!item.not_deletable"
              ...
              :disabled="isLoading"
              :confirm="$t('Delete Floating Device?')"
