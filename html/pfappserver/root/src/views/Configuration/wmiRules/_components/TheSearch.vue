  ...
  <b-card no-body>
    <b-card-header>
      <h4 class="d-flex align-items-center mb-0">
        {{ $t('WMI Rules') }}
        <base-button-help class="text-black-50 ml-1" url="PacketFence_Installation_Guide.html#_wmi_rules_definition" />
      </h4>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-button variant="outline-primary" @click="goToNew">{{ $t('New WMI Rule') }}</b-button>
      </base-search>
      ...
      >
        ...
        <template #cell(on_tab)="{ item }">
          <icon name="circle"
            :class="{ 'text-success': item.on_tab === '1', 'text-danger': item.on_tab !== '1' }"
            v-b-tooltip.hover.left.d300 :title="$t(item.on_tab)" />
        </template>
      </b-table>
