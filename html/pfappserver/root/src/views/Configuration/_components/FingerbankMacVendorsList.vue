    ...
    >
      ...
      <template v-slot:cell(buttons)="item">
        <span class="float-right text-nowrap">
          <pf-button-delete size="sm" v-if="!item.not_deletable && localScope === 'local'" variant="outline-danger" class="mr-1" :disabled="isLoading" :confirm="$t('Delete MAC Vendor?')" @on-delete="remove(item)" reverse/>
...
export default {
  ...
  created () {
    this.$store.dispatch('$_fingerbank/macVendors').then(data => {
      this.data = data
    })
  },
  watch: {
