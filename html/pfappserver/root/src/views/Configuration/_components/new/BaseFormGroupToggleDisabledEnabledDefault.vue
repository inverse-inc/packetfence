    ...
    if (placeholder.value) {
      ...
      switch (placeholder.value) {
        case 'enabled':
          ...
          defaultLabel = i18n.t('Enabled')
          break
        case 'disabled':
        default:
          defaultLabel = i18n.t('Disabled')
          break
      }
      options = [ // recompose
        options[0],
      ...
      switch (true) {
        case placeholder.value === 'disabled' && [options[0].value, options[1].value].includes(value.value):
          hints = [/*0, 1,*/[0, 1]]
          break

        case placeholder.value === 'enabled' && [options[1].value, options[2].value].includes(value.value):
          hints = [/*1, 2,*/[1, 2]]
          break
      }
    }
...
export default {
  name: 'base-form-group-toggle-disabled-enabled-default',
  inheritAttrs: false,
  components,
  props,
  setup
