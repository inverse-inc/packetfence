    ...
    <b-card-header>
      ...
      <b-button-group class="mt-3">
        <b-button v-t="'All'" :variant="(scope === 'all') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankUserAgentsByScope', params: { scope: 'all' } }"></b-button>
        <b-button v-t="'Local'" :variant="(scope === 'local') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankUserAgentsByScope', params: { scope: 'local' } }"></b-button>
        <b-button v-t="'Upstream'" :variant="(scope === 'upstream') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankUserAgentsByScope', params: { scope: 'upstream' } }"></b-button>
      </b-button-group>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-button variant="outline-primary" @click="onNewClicked">{{ $t('New User Agent') }}</b-button>
      </base-search>
