    ...
    <b-card-header>
      ...
      <b-button-group class="mt-3">
        <b-button v-t="'All'" :variant="(scope === 'all') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankDhcpFingerprintsByScope', params: { scope: 'all' } }"></b-button>
        <b-button v-t="'Local'" :variant="(scope === 'local') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankDhcpFingerprintsByScope', params: { scope: 'local' } }"></b-button>
        <b-button v-t="'Upstream'" :variant="(scope === 'upstream') ? 'primary' : 'outline-secondary'"
          :to="{ name: 'fingerbankDhcpFingerprintsByScope', params: { scope: 'upstream' } }"></b-button>
      </b-button-group>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-button variant="outline-primary" @click="onNewClicked">{{ $t('New DHCP Fingerprint') }}</b-button>
      </base-search>
        ...
        <template #cell(buttons)="{ item }">
          <span class="float-right text-nowrap text-right">
            <base-button-confirm v-if="!item.not_deletable"
              ...
              :disabled="isLoading"
              :confirm="$t('Delete DHCP Fingerprint?')"
