  ...
  <b-card no-body>
    <b-card-header>
      <h4 class="d-flex align-items-center mb-0">
        {{ $t('Security Events') }}
        <base-button-help class="text-black-50 ml-1" url="PacketFence_Installation_Guide.html#_security_events" />
      </h4>
    </b-card-header>
    <div class="card-body">
      <base-search :use-search="useSearch">
        <b-button variant="outline-primary" @click="goToNew">{{ $t('New Security Event') }}</b-button>
      </base-search>
      ...
      >
        ...
        <template #cell(enabled)="{ item }">
          <toggle-status :value="item.enabled" :disabled="isLoading"
            :item="item" @input="item.enabled = $event" />
        </template>
        ...
        <template #cell(buttons)="{ item }">
          <span class="float-right text-nowrap text-right">
            <base-button-confirm v-if="!item.not_deletable"
              ...
              :disabled="isLoading"
              :confirm="$t('Delete Security Event?')"
            ...
            >{{ $t('Clone') }}</b-button>
            <b-dropdown :text="$t('Preview')"
              size="sm" variant="outline-primary" class="mr-1" right>
              <b-dropdown-header class="text-secondary">{{ $t('Connection Profile') }}</b-dropdown-header>
              <b-dropdown-item v-for="(connectionProfile, index) in connectionProfiles" :key="index"
                :href="`/config/profile/${connectionProfile.id}/preview/security_events/${item.template}.html`" target="_blank"
              >{{ connectionProfile.id }}</b-dropdown-item>
            </b-dropdown>
          </span>
        </template>
      </b-table>
...
const setup = (props, context) => {
  ...
  const connectionProfiles = ref([])
  $store.dispatch(`$_connection_profiles/all`).then(data => {
    connectionProfiles.value = data
  })
