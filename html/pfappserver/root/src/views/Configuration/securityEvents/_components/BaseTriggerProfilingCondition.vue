...
const setup = (props, context) => {
  ...
  const valueLookup = ref({})
  watch(type, () => {
    valueLookup.value = useNamespaceMetaAllowedLookup(`triggers.0.${type.value}`, meta)
  }, { immediate: true })

  const typeOptions = Object.keys(triggerFields)
    .filter(field => {
      const { category } = triggerFields[field]
      return category === triggerCategories.PROFILING
    })
    .map(value => {
    ...
    for (let t = 0; t < types.length; t++) {
      ...
      switch (component) {
        case componentType.SELECTONE:
          return BaseInputChosenOneSearchable
          // break
...
// @vue/component
export default {
  name: 'base-trigger-profiling-condition',
  inheritAttrs: false,
  components,
  props,
  setup
