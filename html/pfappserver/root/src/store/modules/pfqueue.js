...
const pollTaskStatus = ({ task_id, headers }) => {
  return apiCall.getQuiet(`pfqueue/task/${task_id}/status/poll`, { headers }).then(response => {
    ...
    else if (error.response && !['ERR_BAD_RESPONSE', 'ERR_NETWORK', 'ERR_CANCELLED'].includes(error.code)) { // The request was made and a response with a status code was received
      throw error
    }
    else {
      if (!(task_id in retries))
