...
sub options_sources {
    return map { { value => $_->id, label => $_->id, attributes => { 'data-source-class' => $_->class  } } } grep { $_->{'type'} eq "AD" or $_->{'type'} eq "LDAP" or $_->{'type'} eq "SQL" or $_->{'type'} eq "Htpasswd" or $_->{'type'} eq "RADIUS" } @{getInternalAuthenticationSources()};
}
