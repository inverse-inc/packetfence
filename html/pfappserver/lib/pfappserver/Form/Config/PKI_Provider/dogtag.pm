package pfappserver::Form::Config::PKI_Provider::dogtag;

=head1 NAME

pfappserver::Form::Config::PKI_Provider::dogtag

=head1 DESCRIPTION

Form to handle Fedora Dogtag PKI configuration
...
has_field 'url' =>
  (
   type => 'Text',
   tags => { after_element => \&help,
             help => 'The url used to connect to the Dogtag PKI service'},
  );
...
has_field 'crlURI' =>
  (
   type => 'Text',
   tags => { after_element => \&help,
             help => 'The URI of the Certificate Revocation List'},
  );

has_field 'challenge_password' =>
  (
   type => 'ObfuscatedText',
   tags => { after_element => \&help,
             help => 'Password to connect to the Dogtag PKI CA'},
  );
...
has_block definition =>
  (
    render_list => [qw(type url crlURI challenge_password country state locality organization organizational_unit cn_attribute cn_format ca_cert_path server_cert_path)],
  );
