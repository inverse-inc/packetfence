...
has_field 'disallowed_node_roles' =>
  (
   type => 'Select',
   multiple => 1,
   label => 'Disallowed node roles',
   options_method => \&options_roles,
   element_class => ['chzn-select'],
   element_attr => {'data-placeholder' => 'Click to add a role' },
   tags => { after_element => \&help,
...
sub options_actions {
    ...
    foreach my $role (@ADMIN_ACTIONS) {
        $role =~ m/^(.+?)(_(WRITE|READ|CREATE|UPDATE|DELETE|SET_ROLE|SET_ACCESS_DURATION|SET_ROLE_FROM_SOURCE|SET_UNREG_DATE|SET_ACCESS_LEVEL|SET_TIME_BALANCE|SET_BANDWIDTH_BALANCE|TRIGGER_RADIUS_MFA|TRIGGER_PORTAL_MFA|MARK_AS_SPONSOR|CREATE_MULTIPLE|READ_SPONSORED))?$/;
