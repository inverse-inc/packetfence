...
sub options_type {
    my $self = shift;
    local $_;
    return map { /^profile\.filter\.(.*)$/;{ value => $1, label => $self->_localize($_)} }
        map { "profile.filter.$_" } sort keys %pf::factory::condition::profile::PROFILE_FILTER_TYPE_TO_CONDITION_TYPE;
}
