...
sub initialize_mysql :Local :AdminRole('FINGERBANK_UPDATE') :AdminConfigurator {
    ...
    if($pid) {
        ...
        use POSIX();
        POSIX::setsid();
        # HACK alert #2 : we do the import through a bash script which will handle the alerting itself since when the httpd.admin process restarts, the e-mail will not be sent after the job.
