package pfappserver::PacketFence::Controller::Config::RadiusRemote;

=head1 NAME

pfappserver::Controller::Configuration::RadiusRemote - Catalyst Controller

=head1 DESCRIPTION

Controller for RadiusRemote configuration.
...
__PACKAGE__->config(
    ...
    action_args => {
        ...
        '*' => { model => "Config::RadiusRemote", form => "Config::RadiusRemote" },
    },
);
...
Show the 'view' template when creating or cloning radiusremote.
...
after [qw(create clone)] => sub {
    ...
    if (!(is_success($c->response->status) && $c->request->method eq 'POST' )) {
        $c->stash->{template} = 'config/radiusremote/view.tt';
    }
};
