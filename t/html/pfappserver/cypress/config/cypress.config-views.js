    ...
    setupNodeEvents: (on, config) => {
      on('before:run', async (details) => {
        await fs.readFile('/usr/local/pf/conf/unified_api_system_pass', 'utf-8', async (err, password) => {
          ...
          let response = await fetch('https://localhost:1443/api/v1/login', { method: 'POST', body: request });
          let body = await response.text();
          let { token } = JSON.parse(body);
          let headers = { 'Authorization': `Bearer ${token}` };

          // get maintenance tasks, write fixtures
          response = await fetch('https://localhost:1443/api/v1/config/maintenance_tasks?limit=1000', { method: 'GET', headers });
