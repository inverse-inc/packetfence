...
SKIP: {
    ...
    ok($driver->get_title() =~ /Administrator - PacketFence/, "Checking Title");
    $element = $driver->find_element("username", "id");
    $element->clear;
    $element->send_keys("admin");
    $driver->find_element("password",               "id")->clear;
    $driver->find_element("password",               "id")->send_keys("admin");
    $driver->find_element("button.btn.btn-primary", "css")->click;
    $driver->set_implicit_wait_timeout(500);
    $driver->find_element("i.icon-user.icon-white", "css");
    ok($driver->get_current_url =~ m#/admin/status$#, "We are at the status page and login");
    $driver->quit();
}
