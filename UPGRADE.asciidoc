...
Per example, if an existing rule is as follow:
...
That existing rule will defaulting to the 'authentication' class if none is being set. If that's the case, the first action "Set access level of Web admin" will then be ignored.
