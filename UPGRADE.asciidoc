...
The packaging should automatically rename the existing raddb directory to raddb-pre6.
...
The configuration files under conf/radiusd/*.example have also changed somewhat.
...
You may point it to any valid file by setting the value of "certificate_file" and "private_key_file".
...
Finally, the database schema for the radius accounting tables and stored procedures has changed.
...
If your existing radacct table is large (as is sometimes the case) that operation may take a long time and consume a significant amount of disk space.
...
You may also want to drop the "radacct2" table from the database as it will no longer be needed.
