...
In this version we need to have the kernel development package that match your current kernel version in order to build the Netflow kernel module.
...
manual changes to configuration files will be recorded, included passwords.
...
If you previously ran the PacketFence-pki you will need to migrate from the sqlite db to mysql.
...
Next edit the pki providers (Configuration -> PKI Providers) and redefined the profile to use.
...
You also need to disable access to configurator by putting:
...
in `[advanced]` section of [filename]`pf.conf` file. Then run:
...
Some queries now need CREATE TEMPORARY TABLE priviledge.
You will be prompted for the mysql root password when running this script;
