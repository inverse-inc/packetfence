...
=== Merge of all RPM packages into one
...
. Update [package]`packetfence-release` and [package]`packetfence` packages:
+
[source,bash]
----
yum update packetfence-release --enablerepo=packetfence
yum update packetfence --enablerepo=packetfence
...
At the end of the process, you should have only one RPM package called
...
=== New GPG key for Debian installations
...
[source,bash]
----
curl -L https://inverse.ca/downloads/GPG_PUBLIC_KEY | sudo apt-key add -
----
