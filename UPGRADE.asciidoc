...
Once the additionnal steps are completed, make sure you restart packetfence-config and packetfence

  service packetfence-config restart
  service packetfence restart

Also, make sure to reload configuration
...
You can take a backup of the pf directory with the following command:
  ...
  tar -C /usr/local -czf /root/packetfence.tar.gz pf --exclude='pf/logs'
...
If you are using web-auth enforcement (external captive-portal), you might want (actually, you need otherwise it will no longer works!) to add the 'portal' type to the existing 'management' interface.
