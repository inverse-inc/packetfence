...
The Active Directory domain join configuration is now part of PacketFence. 
A migration script has been made so you can migrate an existing join into this configuration.
Note that this step is not mandatory, as the old join method is still supported.

Simply execute the following script and follow it's instructions `/usr/local/pf/addons/AD/migrate.pl`
