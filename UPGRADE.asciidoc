...
. preliminary steps to run before packages upgrades
. packages upgrades
...
. Follow instructions as mentioned in <<standalone,standalone server upgrade procedure section>> and stop before upgrading PacketFence packages using [command]`yum`
...
. Upgrade PacketFence packages by following [command]`yum` instructions in <<standalone-rpm>> section
