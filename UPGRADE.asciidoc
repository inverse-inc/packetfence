...
The etcd storage backend has been removed in favor of storing the data directly inside the MySQL database. In order to disable the service permanently, execute the following commands:

  systemctl stop packetfence-etcd
  systemctl disable packetfence-etcd
  rm -f /usr/lib/systemd/system/packetfence-etcd.service
  rm -f /etc/systemd/system/packetfence-etcd.service
...
Upgrade sudoers file (all Linux distributions)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

`sudo` configuration will now hide commands starting with `/sbin/ip netns exec` in syslog file.
During the Packetfence upgrade, the package manager of your distribution will inform you that `/etc/sudoers.d/packetfence` has changed. You will need to merge the current and new `sudoers` file.


Database schema and privileges update (all Linux distributions)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Changes have been made to the database schema and to the backup script
`/usr/local/pf/addons/database-backup-and-maintenance.sh`. Starting from now,
we will dump stored routines with the PacketFence database. You will need to update:

 * database schema
 * privileges of the user configured in PacketFence (so he can dump stored routines)

An SQL upgrade script has been provided to upgrade the database from the X.X schema to X.Y.
To upgrade the database schema and privileges, run the following commands:

 PF_USER=$(perl -I/usr/local/pf/lib -Mpf::db -e 'print $pf::db::DB_Config->{user}')
 mysql -u root -p -e "GRANT SELECT ON mysql.proc TO $PF_USER@'%';" \
