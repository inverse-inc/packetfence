...
. stop all PacketFence services (see <<stop-services>> section)
. preliminary steps to run before packages upgrades (see sections for your upgrade path)
. packages upgrades (see <<pkg-upgrades>> section)
. configuration migrations (see sections for your upgrade path)
. database schema upgrade (see sections for your upgrade path)
. restart all PacketFence services (see <<restart-services>> section)
...
changes in between the two versions, including database schema changes.
...
steps required depending on the version you are upgrading from. Please review
the following notes about upgrading from an older release.
...
NOTE: This step need to be done *before* packages upgrade.
...
. Follow instructions mentioned in <<stop-services>> section and stop before starting packages upgrades
...
. Upgrade PacketFence packages by following instructions in <<pkg-upgrades-rpm,Packages upgrades>> section
