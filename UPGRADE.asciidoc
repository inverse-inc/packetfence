...
`sudo` configuration will now hide commands starting by `/sbin/ip netns exec` in syslog file. During upgrade of PacketFence package, package manager of your distribution will inform you that `/etc/sudoers.d/packetfence` has changed. You will need to merge current and new `sudoers` file.
