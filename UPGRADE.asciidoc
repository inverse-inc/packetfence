...
Due to a change in the API of Instagram please do the following if you are using an Instagram OAuth2 source:

    sed -i "s/scope=email/scope=basic/g" /usr/local/pf/conf/authentication.conf
...
In order to do so, change the following section in `/usr/local/pf/conf/mariadb/mariadb.conf.tt`:
