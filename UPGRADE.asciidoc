...
=== Disable monit alerts

Before starting the upgrade shut down monitoring to prevent alerts from
generating false-positives. To see if `monit` is installed and running issue a
monit summary.
...
When the upgrade is complete restart `monit` back up with
