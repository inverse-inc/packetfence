...
* Add category filter on portal profile

Enhancements
++++++++++++
* Adds the ability to search nodes by status
* Removed SSLv3 support from default httpd configuration to prevent POODLE exploit.
* Adds an option to display Bypass VLAN of a node in the Admin GUI.
* Adds nested groups support for Active Directory
* We keep track of EAP machine auth by keeping the hostname in the 'node' table.
* Detect the closest language to use on the portal.
* Device will avoid autocorrect / uppercasing the login field in the captive portal
* Display Bypass VLAN of a node
...
* Fixed invalid floating device creation where the MAC address was not cleaned.
