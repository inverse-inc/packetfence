...
* Leave clients.conf.inc empty if cluster mode is disabled

Bug Fixes
+++++++++
* No longer unregister a device in pending state if the device is hitting more than one time the portal while being in pending state.
* Fix broken violation release process
* Fix for when multiple lines at a time returning from pfconfig
* Fix undefined variables in portal template files.
