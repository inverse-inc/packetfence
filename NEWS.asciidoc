...
* The username on a captive portal will no longer be stripped unless the authentication sources used all need it to be stripped
* Improved UDP reflector documentation
* Improved vendor specific attributes in radius filters
* New ability to specify on which LDAP attribute we should match for SponsorEmail
* New ability to strip a username in LDAP source even if not present in RADIUS request
...
* Fixes incorrect IPv6 dhcp in pfdhcplistener 
