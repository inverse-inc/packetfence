...
Version X.Y.Z released on XXXX-YY-ZZ
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* Added ability to trigger violations based on the Suricata alert description
...
* Suricata alerts can now trigger a violation based on the alert category instead of only the ID of the alert
...
* Deny access on the portal interface if the device hit to much the captive portal using mod_evasive
* Added DHCPv6 fingerprinting though Fingerbank
* Radius filter engine to return custom attributes based on rules
* Security Onion integration
* Added Paypal payment to the captive portal
* Added Stripe payment and subscriptions to the captive portal


Enhancements
++++++++++++
* Detects Microsoft Windows based captive-portal detection mecanisms
...
* Use snmp to determine the ifindexbased on the Nas-Port-Id
* Speed up switch creation
* Move to the vlan filters to use the feature/filter-engine
* Improved metrics now track SQL queries, LDAP queries, and more granular metrics in RADIUS AAA.
* Better logic based on the definition of the VLAN ID, role or ACL to return the radius answer (radius filters)
...
++++++++++++++++++++++++++++++++++++++
* Fix logrotation issue with the carbon daemons.
* Fix LLDP Telephone detection if only Telephone capabilitie is enable. #964
* Fix keepalived and iptables configuration for portal interfaces
* Fix improper httpd status code being set
* Removed the node delete button.
* Fixed detection if the device ask for a portal per uri
