...
=== Bug Fixes
  ...
  - Role ACLs translated for the switch could allow more than the role. Aruba CX, Arista, AOS v16, FortiSwitch, Cisco and Dell N1500 dropped conditions their format cannot carry (the source address or port, the ICMP type, TCP flags, port operators other than `eq`): `permit ip host 10.1.1.1 any` became a permit of all the traffic of the endpoint, `gt 1024` became port 1024. A deny with TCP flags was skipped on Aruba CX, Arista and AOS v16 and let the lines after it through, AOS v16 dropped every destination port, and Dell N1500 sent no ACL at all. A permit that cannot be expressed is now skipped with a warning, and a deny that cannot be expressed ends the ACLs with a deny all. A non contiguous mask no longer breaks the translation on Aruba CX, Arista, AOS v16 and FortiSwitch
  ...
  - `radius_configuration.pfacct_rate_limit` had no effect: the toggle was decoded but never applied, so accounting was forwarded to httpd.aaa unconditionally
  - The pfacct accounting rate limiter no longer suppresses Accounting-Stop packets, which locationlog close, unreg_on_acct_stop and floating-device handling rely on
...
=== New Features
  ...
  - Historical log view — query active and rotated `.log.*.gz` files across the cluster with a time-range picker, served by the Go log-tailer plugin (RE2 filters, bounded scans, lossless byte-offset pagination) (#9091)
  - pftest in admin GUI — the `authentication` and `profile_filter` subcommands are exposed under Auditing > pftest, guarded by the new PFTEST_CREATE admin-role action, rate-limited per tested user, and fan out across the cluster on request (opt-in) (#9091)
  - RADIUS authentication and accounting through pfconnector — remote sites can proxy RADIUS to the cloud over pfconnector tunnels, with a dedicated accounting tunnel, routing by packet code and a unified shared secret (#8963)
...
=== Enhancements
  ...
  - Per-zone VRRP router id: in a multi-zone (layer 3) cluster, the keepalived `virtual_router_id` can now be set per zone in the zone's CLUSTER section of cluster.conf, instead of sharing the single `active_active` value across all zones (#9159) @casdr
  - pfdhcplistener performance — external DHCP events hashed by client MAC across dedicated workers, Fingerbank lookups skipped when the signature is unchanged, firewall SSO updates gated by a half-lease cache, and notifications run in-process (#9054)
...
=== Bug Fixes
  ...
  - Fix non-deterministic RADIUS filter merge when using the `reply:` prefix (#9063)
  - Refresh the Fingerbank collector authorization header at request time so a changed API key is picked up without a restart (#9105)
...
== Version 15.0.0 released on 2025-10-27
...
== Version 14.1.0 released on 2025-02-18
...
* Node deletion in consistent between the API and pf::node::node_delete (#5088)
...
* New pf::MAC class to manage MAC addresses.
