...
* Improved integration with social media networks (email address lookups from OAuth sources, kickbox.io support, etc.)
...
* Introduced a 'packetfence_local' PKI provider to allow the use of locally generated certificates to be used in a PKI provider / provisionner flow
* New filtering engine for the portal profiles allowing complex rules to determine which portal will be displayed
* Added e-mail address lookup for Github and Facebook sources
...
* Improve error message in the profile management page
...
* Added Kickbox.io authentication source which can allow a new Null type source with e-mail validation
