...
* New portal module to keep the previous role if this role is in a list (PR #2490).
...
* Added ability to update the Firewall SSO on accounting packet (PR #2662)
* Added a way to define a VLAN by role as a vlan pool using a VLAN range (PR #2675)

Enhancements
++++++++++++

* Added Cloning in Connection Profiles (PR #2814) (#2809)
* Can now configure read and write timeouts for LDAP connections (#2613) (PR #2614)
* Keepalived can be configured to detect its peers via unicast instead of multicast (PR #2794)
* Suggest VID in Admin Violations -> Add Violation (#2804) (PR #2807)
* Enhanced English i18n strings in admin CMS violation actions.
* Create a priority queue
* Move ReAssignVlan and desAssociate api calls to the priority queue
* Add Connection Profile SSID filter suggestions based on all the previous SSIDs that have been seen in the locationlog (#2758) (PR #2771).
* Adds a description to the switches in the Nodes side navigation (#2791) (PR #2795).
* Improve configuration of the captive portal timer bar (via the captive_portal section of pf.conf) (#383) (PR #2762)
* (AD Powershell scripts) Enforce use of TLS in the powershell scripts which is required with the last versions of PF (PR #2788)
* (AD Powershell scripts) Cycle through all the possible Active Directory usernames formats in PacketFence (PR #2788)
* Removed old authentication code sources (Fixes #2610)
* Added rule description in listing (Fixes #2619)
* Improved documentation (PR #2774 fixes #2773)
...
* Enhanced English i18n strings in admin CMS integration syslog parser violation actions. (PR #2759)

Bug Fixes
+++++++++
* Move add delete buttons to the left to avoid the being cutoff (Fixes #2678)
* Fixes "Admin: Multiple 'Device Type' options in Nodes tab" (Fixes #2789)(PR #2793)
* Configurator: when using a different DB name, the fingerbank.conf mysql section isn't updated (Fixes #2665)(PR #2787) 
