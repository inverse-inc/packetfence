...
=== New Features
  ...
  - Email authentication sources have a new `Wait for activation` option (`wait_for_activation`). When enabled, the device is not granted temporary network access after registering: it stays on the captive portal and is registered automatically once the activation link is clicked, typically from another device such as a phone. The email activation timeout then only bounds the validity of the link
