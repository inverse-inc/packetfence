...
* Reorder action violation to set role before auto registration
* Added hostapd configuration in PacketFence_Network_Devices_Configuration_Guide
...
* Open the port 443 on the management interface when gaming registration is enable
* Update perl SOAP::Lite to 1.0 to prevent freeradius crash
* Update permissions on the logs files to remove error on log action in violation
* Untaint chain in pfmailer and action_log to prevent error with violations
* Fix reevaluate access on action trap in violation
* Set configuration file in spec file and debian build file to prevent this file to be overwrite
