...
* Added the support for authenticating users through OpenID Connect (PR #2394)
...
* Only display readonly actions in readonly mode (PR #2384)
* Journald total file size is now capped at 1Gb (PR #2389)
* Allow sources to be cloned (PR #2395)
* Visually split Administration and Authentication rules when viewing sources (PR #2395)
* Ability to run "fixpermissions" from the web admin GUI (PR #2398)
* Add sd_notify support to most of the sbin/ daemons (PR #2387)
* Allowed haproxy captive portal rate-limiting to be configurable (PR #2422)
* Use the regular samba mechanisms to locate and select DCs (PR #2410)
...
* journald: limit logfile size #2380
* Limit the amount of tasks a worker processes to avoid memory from growing
* REJECT role isn't honored in inline and some web-auth (#2383)
...
* User login remaining isn't displayed correctly in admin interface (#2450)
* Pfqueue counters show a count of 0 although queue is full (#2420)
* node_discovered is triggered when node hasn't been created in DB (#2436)
