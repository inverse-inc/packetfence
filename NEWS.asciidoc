...
New Features
++++++++++++
* Added support Juniper EX2300 (JUNOS 18.2)
* New Authentication Source supported: Clickatell
...
* SSL Inspection Portal Module
* RADIUS Proxy integration from admin gui
* RADIUS Filter for pre_proxy/post_proxy/preacct/accounting/authorize
* DUMMY Source just to compute the rules
...
* Chose hashing algorythm when creating a local user from a source
...
* Issue with the dhcp server that give sometimes a duplicate ip addresse
* Adjusted CentOS and RHEL dependencies
* Fixed MAC filtered lookups are cached in pfdns (#3785)
