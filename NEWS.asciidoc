...
=== Bug Fixes
  ...
  - Role ACLs sent as RADIUS filter rules by the Aruba CX, Arista and AOS v16 modules could allow more than the role: the source address, source port, ICMP type and port operators other than `eq` were dropped (`permit ip host 10.1.1.1 any` became a permit from any, `gt 1024` became port 1024), a deny with TCP flags was skipped and let the lines after it through, and AOS v16 dropped every destination port. A permit that cannot be expressed is now skipped with a warning, and a deny that cannot be expressed ends the ACLs with a deny all
