...
* Support for Huawei S2700, S3700, S5700, S6700, S7700, S9700 
* Added support for LinkedIn and Windows Live as authentication sources.
* Support for 802.1X on Juniper EX2200 and EX4200
* Unreg a node if the username is locked or deleted in Active Directory 
* Added support for the Netgear M series
* Added support to define SNAT interface to use for passthrough
* Added nessus scan policy based on dhcp fingerprint
* Added support to unregister a node if the username is locked or deleted in Active Directory
* Update firewall user table from PacketFence to allow devices to passthrough the firewall
...
* Removed cgi based catalyst files
* For device registeration use the source used to authenticate for calculating the role and unregdate (bugid:1805)
