...
* New service pfcertmanager that generate certificate files based on the configuration.
...
* Choose which eap profile to use based on a radius filter.
* Basedn can be define in the authentication sources rules
* New hooks for the radius filter engine in eduroam virtual server.
* Redefined "restart" in the service manager to allow "PartOf" in systemcd scripts
...
* Support of multiples ldap servers in frreeradius ldap_packetfence configuration file
* copy outter user-Name attribute in PacketFence-Outer-User attribute to be able to use it in the authentication rules.
