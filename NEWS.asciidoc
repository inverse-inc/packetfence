...
Version 4.0.5 released on 2013-MM-DD
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* Authentication is not limited to internal sources
* DynamicTable widget doesn't allow to drag'n'drop under last row
* Connections on port 443 are not accepted for self-registration (#1679)
...
* Returned configured per switch role (if configured) for "Role mapping by switch role" rather than sending the user role.
