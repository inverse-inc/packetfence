...
Version X.Y.Z released on 2017-
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* Added the possibility to import switches from a CSV file.
...
* Redirect template for violation (PR #2400)
* Display vlan in locationlog view
* Allow the timezone to be selected.
* Add a task count jitter to reduce the chance that pfqueue workers exit at the same time.
* Updated ACE text editor to version 1.2.8
* Allow the ability reset the search forms for nodes and users (PR #2555)
* Allow saving configuration files in readonly mode except violation, switches, role (#2464) (PR #2566)
* Added parameter to have extended descriptions in the custom reports
* Add support for SSL and StartTLS for sending mail (PR #2446)
* Allow to ignore self signed cert errors for nessus 6 (PR #2568)
* Allow violations to be triggered by nessus 6 scanner (PR #2568)
* Improved device registration to be able to create profiles, this allow you to associate device registration profile by connection profile
* Allow to specify the format of the username that is being sent in firewall SSO (PR #2499)
* Monitor SSL certificates expiration and alert if they are expired (PR #2444)
* Content Security Policy (CSP) enforced through the browser for Admin and Portal interfaces. Can be disabled through Advanced System Configuration. (PR #2336)
...
* Fortigate 50E 802.1x support
* Adds the ability to normalize the computer authentication username when using EAP-TLS (PR #2414)
...
* Move the syslog parser from the section Compliance to Integration in the menu of the WebAdmin (#2467)
* pfsso now logs in packetfence.log (#2553) (PR #2557)
* httpd.dispatcher now logs in httpd.dispatcher.log (PR #2557)
* Fix incorrect inline sub type detection
* Fix ipset update with the incorrect ip address (the old one)
* Fix missing confirm prompt when restarting all services via the admin interface (#2365) (PR #2571)
* Fix the database has gone away error in services (#2580) (PR #2585)
* Fixes violation definition sync when removing a violation from the config
* Fixes incorrect Connection-Type when using EAP-TTLS (#2582)
* Fix voip logic to reduce the chance of duplicate locationlog entries (#2527)
...
* Use the system time instead of browser time when using preset time values (#2559)
...
* SSL certificates expiration monitoring (#2444)
