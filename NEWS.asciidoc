...
* Add support for Role Based Enforcement for Meraki wired devices (#7000)
* Added proxysql to distribute database reads when using PacketFence in cluster
* Sign a csr in PacketFence pki
* Added support to split database read and writes to different MySQL servers (#7055)
...
* Detect Cli-Access from Dell N1500 switch (#7070)
...
* Return VIP in DHCP request if `dns_on_vip_only` is enabled (#7035)
* Replace LF by CRLF at end of mails sent by PacketFence (SMS email has "Bare Line Feed Characters" Status code: 550 5.6.11  #5380)
