...
=== New Features
  ...
  - Juniper EX switches running Junos 18.x or later (`Juniper::Junos_v18_x`) can now enforce role ACLs. PacketFence translates the ACLs of the role into `Juniper-Switching-Filter` attributes and sends them inline in the Access-Accept, so no filter has to be pre-provisioned on the switch. Set `AccessListMap` to `Y` on the switch to enable it
