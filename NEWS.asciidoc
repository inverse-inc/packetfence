...
Version 8.0.1 released on XXXX-XX-XX
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* Put etcd in his own directory

Bug Fixes
+++++++++

* Fix issue with device profiling not being performed when an endpoint connects for the first time
* Fix missing timeout when performing RADIUS SSO (FortiGate, CheckPoint, WatchGuard)
* Fix issue with API frontend when initially configuring the webservices username and password
* packetfence-haproxy-portal and packetfence-tc systemd service in a wrong target.
* Custom routing w/ Inline - Fails Silently (#3215)
* Nessus6 scanner
* haproxy-db only listen on ipv6 (debian) (#3208)
* Fixed packetfence-local-auth
* Fixed dns passthrough for normal domain (was considered as a wildcard)
* Winbind fail to start because of a permissions on /var/run/samba/winbindd in the chroots
* Update from 7.4 to 8.0 audit log file not there (#3216)
* Fixed unreg on accounting stop (#3220)
* Allow nodes without roles to be modified when restricting allowed role (#3217)
* Fix speed issues with node search in the admin
* Fix missing timeout for RADIUS sources tests in pfstats
