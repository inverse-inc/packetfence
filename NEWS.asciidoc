...
=== New Features
  ...
  - pftest in admin GUI — the `authentication` and `profile_filter` subcommands are exposed under Auditing > pftest, guarded by the new PFTEST admin-role actions, rate-limited per tested user, and fan out across the cluster on request (opt-in)
