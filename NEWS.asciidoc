...
* An unregistered device can now be set into a 'parking' state, where it will have a longer DHCP lease time and will only access a lightweight portal
...
* Unregister a node from a secure connection to an unsecure one is now managed by the VLAN filters
...
* Trigger violation on source IP or destination IP address if they are in the trapping range networks
* Performance improvement for VoIP detection
* Added a new computed return in radius filter (random number in a range)
* Reinstating iplog (iplog_history and iplog_archive) rotation/cleanup jobs ran by pfmon
 
Bug Fixes (bug Id is denoted with #id)
++++++++++++++++++++++++++++++++++++++

* Compute unregdate for secure connections
