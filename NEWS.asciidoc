...
* pfdns filter engine. Added a way to return custom answer in pfdns
...
* Assign the voice role to VoIP devices when PacketFence detect it
* Renamed Vlan to Role in admin gui violation
* Deregister a node from a secure connection to a unsecure one is now managed by the vlan filters.
* Location history of a node show the role instead of the vlan id
* Documentation to configure cisco switch with Identity Networking Policy
...
* Added a new computed return in radius filter (random number in a range).
...
* Compute unregdate for secure connection
